...
| [`LANG-GMN.md`](LANG-GMN.md) | dialect charter | GMN, the GMEOW Model Notation — the engineered, token-compact dialect ladder (GMN-0 the narrow-waist normal form; GMN-1 the model surface; GMN-2 lossy cognitive compaction); the rate–fidelity contract over the LLM token channel; the record-initial sigil table, record and tabular forms, the in-band header, the encoding and versioning policy, and the nine-field envelope contract |

...
//! both the earlier `run_shacl` fixture cases (parameterized `rstest` rows) and
...
fn sssom_row_exists(subject_id: &str, object_id: &str) -> bool {
    ...
    for entry in std::fs::read_dir(&dir).unwrap_or_else(|e| panic!("read mappings dir: {e}")) {
...
fn no_frame_collapsing_projection_exists() {
    let queries = repo_root().join("generated").join("queries");
    assert!(
        queries.join("standpoint-owl2.rq").exists(),
        "lossless standpoint-owl2.rq projection must ship"
    );
    assert!(
        !queries.join("standpoint.rq").exists(),
...
fn bbc_projection_exists() {
    assert!(
        repo_root()
            .join("generated/queries/standpoint-bbc.rq")
            .exists(),

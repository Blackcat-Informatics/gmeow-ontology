...
use gmeow_rdf_core::{RdfDataset, RdfDatasetBuilder, RdfLiteral, TermFactory, TermId, TermValue};
...
struct DroppedReifier {
    ...
    has_annotation: bool,
    /// `true` if one of those dropped annotation predicates is `gmeow:accordingTo`.
    has_standpoint: bool,
}
...
fn collect_dropped_reifiers(
    ...
    for (reifier_var, inner) in reifiers {
        ...
        // those predicates `gmeow:accordingTo`?
            ...
            if let TermPattern::Variable(s) = &tp.subject {
                if s.as_str() == reifier_var {
                    ...
                    if let NamedNodePattern::NamedNode(n) = &tp.predicate {
                        if n.as_str() == GMEOW_ACCORDING_TO {
                            has_standpoint = true;
                        }
                    }
                }
...
/// the dropped reifier also lost annotations / a `gmeow:accordingTo` standpoint.
...
mod tests {
    ...
    fn reified_graph() -> Arc<RdfDataset> {
        ...
        // Annotation: :r :confidence "0.9" ; :r gmeow:accordingTo :sourceX .
    ...
    fn dropped_annotated_reifier_emits_annotation_and_standpoint_codes() {
        ...
        // gmeow:accordingTo annotation — all keyed off ?r, which the template drops.
        let ds = reified_graph();
        let mut ctx = EvalCtx::new(&ds);
        let where_pat = GraphPattern::Bgp {
            patterns: vec![
                ...
                // ?r gmeow:accordingTo ?stand  (the standpoint annotation)
                TriplePattern {
                    subject: var("r"),

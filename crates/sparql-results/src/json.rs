...
/// `gmeow` provenance extension when `provenance` is non-empty.
///
...
/// Write the full SRJ document (base object + optional `gmeow` extension).
///
/// The base object is written first, then — when provenance is non-empty — the
/// `gmeow` member is inserted just before the document's final closing `}` so
...
fn write_srj(
    ...
    out.push_str(",\"gmeow\":");
    write_gmeow(result, provenance, out);
    out.push('}');
    Ok(())
}
...
/// Write the additive `gmeow` extension object (the value of the top-level
/// `"gmeow"` member). Only present fields are emitted, in a fixed order.
fn write_gmeow(result: &SparqlResult, provenance: &ResultProvenance, out: &mut String) {
...
mod tests {
    ...
    fn populated_provenance_appends_valid_gmeow_member() {
        ...
        assert!(text.contains("\"gmeow\":{"), "missing gmeow member: {text}");
        assert!(
        ...
        assert!(
            text.contains("\"engine\":\"gmeow-sparql-eval\""),
            "missing engine: {text}"
        );
        assert!(

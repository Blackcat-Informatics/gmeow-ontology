...
const NON_CONFORMANT: &[&str] = &[
    ...
    // Standalone only, and NOT for the reason this entry used to give. It claimed the
    // math:NormalizationStrength individual was untyped in isolation; that was a real
    // defect in the ontology (a qualified cardinality that consulted a type the consumer
    // never has) and it is fixed at the source — the four QualifiedMinCount errors are
    // gone. What remains is ambient to validating any example alone: the
    // ontology-completeness shape selects `?this a owl:Ontology` under the gmeow
    // namespace and finds the slice IRI without the label/definition that lives in
    // module.ttl. Conformant in the merged bundle.
    "slices/grounding/math/examples/reference-ast-act.ttl",

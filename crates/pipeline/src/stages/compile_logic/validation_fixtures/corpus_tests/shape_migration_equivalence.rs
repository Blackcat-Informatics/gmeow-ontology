...
//! consuming the authenticated typed shapes and SHACL emitted by the real producer
//! over each exact source module. No test derives or projects a corpus shape.
//!
...
/// Read the producer's standalone typed shapes for this exact source module.
fn derived(rel: &str) -> Vec<ShapeProduct> {
    super::module(rel).shapes.clone()
}
...
fn class_shape<'a>(shapes: &'a [ShapeProduct], class: &str) -> &'a ShapeProduct {
    shapes
        .iter()
...
fn flag_with_shape(shape: &ShapeProduct, data_ttl: &str) -> Vec<String> {
    let doc = &shape.shacl;
    let shapes =
...
fn flag_with_procedural(module_rel: &str, data_ttl: &str) -> Vec<String> {
    let shapes = parse_shapes(&super::module(module_rel).procedural_shacl, None)
        .expect("consume producer-emitted procedural constraints");
    let data = purrdf::parse_dataset(data_ttl.as_bytes(), "text/turtle", None)
        .expect("fixture parses as Turtle");
    validate_dataset(&data, &shapes)
        .expect("validate")

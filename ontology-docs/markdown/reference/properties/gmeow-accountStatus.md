...
- Avoid for the service's liveness (use [`gmeow:serviceStatus`](gmeow-serviceStatus.md)) and avoid forcing one value in a merge — competing source- or standpoint-asserted statuses coexist, so never make it functional.

### How to use

- Point the [`OnlineAccount`](../classes/gmeow-OnlineAccount.md) at a [`gmeow:AccountStatus`](../classes/gmeow-AccountStatus.md) individual; mark a retired account historical with a past validity period, and carry conflicting observers' statuses on the statement layer rather than collapsing them.
...
- ex:lillithMastodon [`gmeow:accountStatus`](gmeow-accountStatus.md) [`gmeow:accountStatusActive`](../individuals/gmeow-accountStatusActive.md).

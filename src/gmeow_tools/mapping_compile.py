...
The three cross-layer consistency invariants (FnO type-mismatch, EDOAL→FnO
reference integrity, CONSTRUCT↔EDOAL↔SSSOM spec-drift) are checked natively by
``gmeow_slice.lint_projection`` (#854, subsuming the former Python
``projection_lint`` trio) and surfaced as ``mapping-compile.{fno-type,fno-ref,
spec-drift}`` findings (see :func:`compile_diagnostics_report`). ``--check``
...
def emit_fno(dsl: Dsl, onto: Graph) -> Graph:
    ...
    re-parses it into a fresh rdflib :class:`Graph` (so the Turtle writer is
    unchanged) and binds the canonical prefixes as the old code did. The result is
    graph-isomorphic to the historical Python emitter.
    """

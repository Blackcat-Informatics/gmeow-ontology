...
- Avoid for the display handle (use [`gmeow:accountName`](gmeow-accountName.md)) and for the Nostr identity layer (use [`gmeow:nostrPubkey`](gmeow-nostrPubkey.md) / [`gmeow:nip05`](gmeow-nip05.md)); the actor IRI is the ActivityPub-specific endpoint, not a generic homepage.

### How to use

- Carry the actor IRI as an [xsd:anyURI](http://www.w3.org/2001/XMLSchema#anyURI) on the [`OnlineAccount`](../classes/gmeow-OnlineAccount.md); keep the service it belongs to on [`gmeow:accountService`](gmeow-accountService.md) and the human handle on [`gmeow:accountName`](gmeow-accountName.md) so each identifier role stays distinct.

### Examples

- ex:lillithMastodon [`gmeow:activityPubActor`](gmeow-activityPubActor.md) "https://fosstodon.org/users/lillith"^^[xsd:anyURI.](http://www.w3.org/2001/XMLSchema#anyURI.)

...
//! ([`NativeSparqlEngine`](gmeow_sparql_eval::NativeSparqlEngine)) to JavaScript so a
...
//! the wasm tree stays clean because the evaluator's only non-portable edge (the
//! `ureq` SERVICE transport) is `cfg(not(wasm32))`-gated off.
//!
//! ## Federation is intentionally absent
//!
//! This binds the plain [`SparqlEngine::query`](gmeow_rdf_core::SparqlEngine::query)
//! entry — the one with **no** [`RemoteQuerySource`](gmeow_sparql_eval::remote)
...
//!   [`gmeow_sparql_results`].
//! - CONSTRUCT / DESCRIBE → **Turtle** via the `native_codecs` serializer (the one
//!   serialization seam; never `oxigraph::io`, never the `gmeow-gts` crate).

use gmeow_rdf::{serialize_dataset, SerializeGraph};
use gmeow_rdf_core::{SparqlEngine, SparqlRequest, SparqlResult};
use gmeow_sparql_eval::NativeSparqlEngine;
use gmeow_sparql_results::{serialize as serialize_results, ResultProvenance, SparqlResultsFormat};

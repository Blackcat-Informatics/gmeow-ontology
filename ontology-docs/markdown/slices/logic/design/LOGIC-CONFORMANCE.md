...
- **Source:** [`slices/core/logic/design/LOGIC-CONFORMANCE.md`](https://github.com/Blackcat-Informatics/gmeow-ontology/blob/main/slices/core/logic/design/LOGIC-CONFORMANCE.md)
...
> Status: implementation contract for [`logic:`](https://blackcatinformatics.ca/logic/). This is the **conformance** member of the
...
lets the slow, correct reference and the fast Rust engine coexist and *provably agree* ([Principle 7](https://github.com/Blackcat-Informatics/gmeow-ontology/blob/main/CONSTITUTION.md#principle-7)),
exactly as GTS conformance design promotes the GTS §18 vectors into a `conformance/` directory so any
implementation certifies against the same files. The reasoning corpus is that contract for [`logic:`](https://blackcatinformatics.ca/logic/),
...
The corpus lands under `conformance/logic/` once the [`logic:`](https://blackcatinformatics.ca/logic/) vocabulary exists (cases cannot be
...
|---|---|---|
| `foundation/` | `input.logic.ttl` with UFO⁺ types (rigidity, identity-supply, mediation) | `verdicts.json` reproduces the `reasoning_lint.py` verdicts exactly (isomorphic); the [gUFO](../../../external/ontologies.md#target-gufo) downcast passes every OntoUML anti-pattern check |
...
| `worlds-B/` | risk cascade, teleology, or norms source | `materialized.nq` proves **exactly zero [`Event`](../../../reference/classes/gmeow-Event.md) instances** are generated (the no-occurrence gate), with type-level force present |
...
| `projections/` | any `input.logic.ttl` | `projections/` (OWL DL/EL, Datalog, N3, [gUFO](../../../external/ontologies.md#target-gufo)) compare by isomorphism; the preservation ledger matches |
...
| Kind | Meaning |
|---|---|
| [`logic:ExactPreservation`](https://blackcatinformatics.ca/logic/ExactPreservation) | same answers as canonical for the declared query class |
| [`logic:SoundUnderApproximation`](https://blackcatinformatics.ca/logic/SoundUnderApproximation) | anything the projection entails is canonically valid, but it may miss answers |
| [`logic:CompleteOverApproximation`](https://blackcatinformatics.ca/logic/CompleteOverApproximation) | the projection may overgenerate but will not miss answers |
| [`logic:ValidationOnly`](https://blackcatinformatics.ca/logic/ValidationOnly) | detects some invalidity, but is not an entailment relation |
| [`logic:InconsistencyPreserving`](https://blackcatinformatics.ca/logic/InconsistencyPreserving) | a canonical inconsistency appears in the projection |
| [`logic:InconsistencyReflecting`](https://blackcatinformatics.ca/logic/InconsistencyReflecting) | a projection inconsistency implies a canonical inconsistency |
...
[`logic:SoundUnderApproximation`](https://blackcatinformatics.ca/logic/SoundUnderApproximation) for subclass entailments under profile `X`; PTIME."* This is what lets
the BFO/DOLCE bridge views ([LOGIC-MIGRATION.md](LOGIC-MIGRATION.md#foundation-projection-and-discipline))
be labelled honestly: a bridge view is typically [`logic:ValidationOnly`](https://blackcatinformatics.ca/logic/ValidationOnly) or carries no preservation
claim at all, never [`logic:ExactPreservation`](https://blackcatinformatics.ca/logic/ExactPreservation), unless a specific subfragment is certified. The

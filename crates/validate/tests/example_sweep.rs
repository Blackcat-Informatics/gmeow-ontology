...
const NON_CONFORMANT: &[&str] = &[
    ...
    "slices/core/notes/examples/annotations-and-notes.ttl", // gmeow:commentParent → Entity not typed standalone
    "slices/core/preference/examples/comparison-worked.ttl", // gmeow:vantage → a gmeow:Standpoint and the math: carrier objects (math:CellularSheaf/math:GlobalSection consensus frames) resolve through the module subclass chains only, and gmeow:confidence → a shared logic:QualityValue individual is untyped standalone — illustrative, validated unioned with the module by make validate
    ...
    "slices/core/preference/examples/hard-fails-soft-high.ttl", // reuses the norms gmeow:Assessment and the shared gmeow:verdictNotHeld / gmeow:constraintHardnessHard value individuals (typed in the modules) plus gmeow:vantage → a gmeow:Standpoint — all untyped when the fixture is loaded in isolation, conformant unioned with the ontology
    "slices/core/preference/examples/model-delta-worked.ttl", // reuses by reference the model-serving gmeow:ModelArtifact, norms gmeow:Rubric, ai gmeow:Prompt, gmeow:EvidenceSpan and the math:CliffordAlgebra/math:Multivector/math:MetricSignature geometry objects — shared vocabulary defined in the modules, untyped standalone; illustrative-alongside-the-ontology
    "slices/core/preference/examples/multi-evaluator-no-winner.ttl", // gmeow:vantage → a gmeow:Standpoint, gmeow:contextTask → a gmeow:Goal, and the math: sheaf/obstruction carrier objects resolve through module subclass chains only — illustrative standalone, conformant in the merged bundle
    "slices/core/preference/examples/pareto-incomparable.ttl", // gmeow:vantage → a gmeow:Standpoint and the math: carrier objects resolve through module subclass chains only; gmeow:onCriterion targets and gmeow:confidence → shared value individuals untyped standalone — illustrative, validated unioned with the module
    "slices/core/preference/examples/promotion-worked.ttl", // reuses the shared gmeow:verdictHeld value individual, gmeow:EvidenceSpan, and gmeow:vantage → a gmeow:Standpoint — typed in the modules, untyped when loaded in isolation; illustrative-alongside-the-ontology, conformant unioned with the bundle

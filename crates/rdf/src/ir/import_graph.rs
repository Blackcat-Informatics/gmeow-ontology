...
//! [`gmeow_gts::model::Graph`] **by value** and MOVES its owned term strings into
//! the interner (`std::mem::take` on `Term.value` / `Term.lang`), rather than
//! cloning them.
//!
//! Because `gmeow_gts::reader::read()` folds every segment into one append-order
...
//! [`super::import_gts_events`] (the event-sink path) instead.**
//!
...
/// MUST use [`super::import_gts_events`] (`import_sink.rs`) instead.
///
...
pub fn import_gts_graph(graph: Graph) -> Result<RdfBundle, RdfDiagnostic> {
    ...
    let lookaside = crate::gts::lookaside_from_graph(&graph);
...
mod tests {
    ...
    use gmeow_gts::model::{Term as GtsTerm, TermKind as GtsKind};

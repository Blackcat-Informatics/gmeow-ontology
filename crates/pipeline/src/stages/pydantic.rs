...
/// # Bump policy
///
/// Bump `owl:versionInfo` in `ontology/gmeow.ttl` and `make check` — never
...
fn render_about(version: &str) -> Vec<u8> {
    ...
    out.push_str("and run `make check` — never hand-edit this file or set `version`\n");
...
/// standalone `make check-sync SYNC_MODE=update SYNC_OUTPUTS=docs` entry, which runs post-pipeline against

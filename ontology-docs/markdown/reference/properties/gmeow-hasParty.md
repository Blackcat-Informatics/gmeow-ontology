...
Relates an agreement to an agent that is bound by it. NOT functional: an agreement binds two or more parties, and each is attached by a separate [`gmeow:hasParty`](gmeow-hasParty.md) statement.
...
- Avoid for the direction of a teleological promise (a [`Commitment`](../classes/gmeow-Commitment.md) carries its committed agent and beneficiary separately) and for an agent merely mentioned in the agreement; only the agents actually bound are parties, and the property is non-functional.

### How to use

- Attach one [`gmeow:Agent`](../classes/gmeow-Agent.md) per [`gmeow:hasParty`](gmeow-hasParty.md) statement; where the per-party direction of obligation matters, model each side as a [`gmeow:Commitment`](../classes/gmeow-Commitment.md) moment rather than overloading this membership-style link.

### Examples

- ex:partnership [`gmeow:hasParty`](gmeow-hasParty.md) ex:lillith.

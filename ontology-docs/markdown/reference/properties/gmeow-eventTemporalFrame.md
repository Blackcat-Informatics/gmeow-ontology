...
- Use to declare the temporal frame an event's date/interval is expressed in whenever it is NON-default — a non-Gregorian calendar, an astronomical scale — making the frame explicit ([Principle 11](https://github.com/Blackcat-Informatics/gmeow-ontology/blob/main/CONSTITUTION.md#principle-11)).

### Avoid when

- Avoid asserting it for the default [`gmeow:temporalFrameUTCGregorian`](../individuals/gmeow-temporalFrameUTCGregorian.md) (it is the assumed frame), and avoid stating two frames for one event — it is functional, one frame per event.

### How to use

- Point the [`gmeow:Event`](../classes/gmeow-Event.md) at one [`gmeow:TemporalFrame`](../classes/gmeow-TemporalFrame.md) via [`gmeow:eventTemporalFrame`](gmeow-eventTemporalFrame.md) (a subproperty of [`gmeow:hasReferenceFrame`](gmeow-hasReferenceFrame.md)) alongside its date triples; absent, the UTC-Gregorian default is assumed.

### Examples

- ex:festival [`gmeow:eventTemporalFrame`](gmeow-eventTemporalFrame.md) ex:hebrewCalendar.

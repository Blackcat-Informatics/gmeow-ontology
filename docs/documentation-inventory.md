...
| `docs/CLI_ARCHITECTURE.md` | CLI & MCP architecture: crate-selection decision record and the three-surface (consumer `gmeow`, developer `gmeow-dev`, MCP) Rust-port blueprint | Project-level tooling/DX doctrine; peer to `cli-extensions.md` and `PIPELINE_SPINE.md` | `keep-root` | Keep as the canonical CLI/MCP architecture reference. Update as the CLI→Rust port epic lands each surface. |

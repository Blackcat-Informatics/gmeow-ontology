...
| `test_level_ranks_are_zero_through_five` | `tests/test_awareness.py` | `conformance_awareness.rs::level_ranks_are_exactly_zero_through_five` | conformance twin | converted | closed numeric SET-EQUALITY over the six `gmeow:levelRank` values (a SPARQL ASK can assert each rank exists but not that the set is EXACTLY `{0,1,2,3,4,5}` and no others) → native Rust twin asserting the set both ways, hardened so exactly the six subjects carry a rank | `make check` |
| `test_manifest_depends_only_on_kernel_and_temporal` | `tests/test_awareness.py` | `conformance_awareness.rs::slice_depends_on_is_exactly_kernel_and_temporal` | conformance twin | converted | set-equality over `manifest.ttl`, which `run_structural_cell` never loads (store = module.ttl + examples/ only) → native Rust twin over the parsed manifest | `make check` |
...
**Awareness tally:** 12 converted (11 structural cells + 2 native conformance twins across 12 pytest functions), 1 deleted-covered-by-make-validate. The two former set-equality residue functions are now the native Rust twins in `crates/validate/tests/conformance_awareness.rs`; source file `tests/test_awareness.py` **deleted** (no residue remains).
...
| `test_manifest_depends_only_on_kernel` | `tests/test_imagination.py` | `conformance_imagination.rs::slice_depends_on_is_exactly_kernel` | conformance twin | converted | set-equality over `manifest.ttl`, which `run_structural_cell` never loads (store = module.ttl + examples/ only) → native Rust twin over the parsed manifest | `make check` |
...
**Imagination tally:** 11 converted (13 structural cells + 1 native conformance twin across 11 pytest functions), 1 deleted-covered-by-make-validate. The former set-equality residue function is now the native Rust twin in `crates/validate/tests/conformance_imagination.rs`; source file `tests/test_imagination.py` **deleted** (no residue remains).
...
| `extensions/sensory-environment` (gufo:) | 12 → 15 cells | 4 retained: 2 SSSOM mapping reads; 2 cross-slice (axis/frame-realm in places). The mixed `mental_reference_frame_requires_host` is fully migrated: its structural restriction arm → cell, its consistency + classification arm → the native RL harness. |
...
| `test_mental_reference_frame_requires_host` | `tests/test_sensory_environment.py` | `mental_reference_frame_hosted_instance_is_consistent` | RL-entailment + structural cell | converted | MIXED, split across two harnesses: the structural blank-node `someValuesFrom` restriction-axiom check → structural cell; the scoped OWL-RL consistency (no `owl:Nothing`) + `ReferenceFrame` classification → the native RL twin | `make logic-test` |

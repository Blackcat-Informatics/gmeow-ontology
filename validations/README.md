...
| [`classic-cross-check/`](./classic-cross-check/) | GMEOW's **native** reasoning / OWL 2 RL closure / RDF-1.2-statement pipeline agrees with — or strictly subsumes — the classical **ELK / HermiT / ROBOT / Jena / owlrl** oracles' own verdicts. The oracle half of the correspondence-calculus doctrine (`docs/APPLIED_CATEGORY_THEORY/take1.md` §10.2): the native Rust engine is the authority, the classical engines are demoted conformance oracles, gated fragment-by-fragment by the divergence ledger. Needs Docker and/or a JVM, so it lives here — never in CI or `make check`. |
| [`common-logic-fol/`](./common-logic-fol/) | The CLIF export of the `logic:` foundation is FOL-consistent, and the native-derived ancestor entailment over an ingested CL KB is confirmed by an external first-order reasoner (E prover), via a semantics-preserving CLIF→TPTP-FOF translation — the empirical native ⊇ oracle cross-check. |

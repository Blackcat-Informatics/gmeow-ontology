...
//! Shared build-script traversal for exact local implementation closures.
//!
//! Cargo.lock authenticates registry and Git dependencies. Local `path` dependencies
//! carry no content checksum there, so producer fingerprints derive and hash the full
//! non-dev path-dependency closure from the live manifests instead of hashing unrelated
//! workspace packages or maintaining a hand-written crate list.

use std::collections::BTreeSet;
use std::path::{Component, Path, PathBuf};

const CRATE_INPUT_SUBPATHS: [&str; 6] = [
    "src",
    "assets",
    "templates",
    "fixtures",
...
const EPHEMERAL_DIRECTORY_NAMES: [&str; 3] = [".cache", "node_modules", "target"];

/// Complete transitive closure of non-dev local path dependencies.
...
/// Every library/build-script implementation input for one crate in the closure.

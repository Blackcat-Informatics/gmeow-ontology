...
pub fn index_turtle(turtle: &[u8]) -> Result<TripleIndex> {
    ...
    if !gts.ingestion.declarations_omitted.is_empty() {
        return Err(Diag::of_kind(GraphRead {
            detail: format!(
                "Turtle index lost graph declarations: {:?}",
                gts.ingestion.declarations_omitted
            ),
        }));
    }
    let graph = purrdf::gts::reader::read(&gts.bytes, false, None);
...
pub fn turtle_to_gts(turtle: &[u8]) -> Result<gmeow_gts_profile::GmeowGtsEmission> {
    ...
    let _ingestion = builder.add_view(&dataset).map_err(|err| {
        Diag::of_kind(GraphRead {

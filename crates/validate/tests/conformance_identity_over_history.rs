...
/// Repo-relative path to the compiled statement OWL (annotation reifiers).
const STATEMENT_OWL_REL: &str = "generated/statements/gmeow-statements.owl.ttl";
...
fn ai_author_is_software_agent_with_statement_metadata() {
    ...
    let statements = GraphStore::parse_ttl_file(&repo_root().join(STATEMENT_OWL_REL));

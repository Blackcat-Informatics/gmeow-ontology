...
//! query via [`gmeow_sparql_algebra::pattern_to_select_query`]) to an injected
...
//!
//! [`RemoteQuerySource`] is the dependency-inversion seam: the native HTTP
//! transport ([`crate::remote_http::HttpRemoteQuerySource`], native-only) and the
//! in-memory dog-fooding source ([`LocalRemoteQuerySource`]) are interchangeable
//! implementations. This keeps the wasm build clean (no HTTP client in the core
//! query path) and makes `SERVICE` deterministically testable offline.
//!
//! # Hard-fail vs SILENT
//!
...
use gmeow_sparql_algebra::{GraphPattern, NamedNodePattern, Variable};
...
pub(crate) fn eval_service(
    ...
    let query_text = gmeow_sparql_algebra::pattern_to_select_query(inner);

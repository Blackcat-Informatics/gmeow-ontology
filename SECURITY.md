...
GMEOW (the ontology and its publishing toolchain) is maintained by Blackcat Informatics® Inc. We take security reports seriously and ask that vulnerabilities be reported privately so they can be investigated and fixed responsibly.
...
The most relevant security surface for this project is the toolchain — for
example, untrusted ontology input handled by the build pipeline, the pinned
Docker images, or the CI/release workflows — rather than the vocabulary itself.

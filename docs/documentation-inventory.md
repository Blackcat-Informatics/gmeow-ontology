...
| `docs/rust-gts-integration.md` | Rust/GTS integration and version policy | Project toolchain/process content; required gmeow-gts dep | `keep-root` | New #649 doc. Why GTS is required, the nightly/MSRV float policy, the AGPL/Apache-MIT license boundary, and the `validate --gts` path. Keep as project-level toolchain reference. |

...
use purrdf::{
    DatasetView, GraphMatch, RdfDataset, RdfLiteral, SerializeGraph, TermRef, TermValue,
...
fn dataset_from_files(paths: &[PathBuf]) -> gmeow_errors::Result<Arc<RdfDataset>> {
    let mut parsed: Vec<Arc<RdfDataset>> = paths
        .iter()
        .map(|p| {
...
fn residue_gate(executor: &put_executor::LiftedDataset) -> GateResult {
...
/// Run a native draft through the selected retag, MAXIMAL(G), and five-gate chain.
/// The complete transform bundle is still built and validated; graph consumers share
/// its native result without rendering and reparsing a private text copy.
fn draft_gates(
    root: &Path,
    source_store: &Arc<RdfDataset>,
    ctx: &AcceptanceContext,
    executor: &put_executor::LiftedDataset,
) -> gmeow_errors::Result<(Vec<GateResult>, usize)> {
    let draft_store = retag_dataset_to_internal(&executor.dataset, &ctx.inverse_tag_map)?;
    let transformed = ctx.transform_program.execute_default_graph(&draft_store)?;
    let output_store = &transformed.dataset;
    let gates = vec![
        gate_pure_gmeow(&draft_store)?,
        gate_round_trip(source_store, output_store, &ctx.tag_map)?,
        gate_size_invariant(source_store, output_store)?,
        gate_external_validator(root, output_store, &ctx.tag_map)?,
        gate_coverage(
            source_store,
            output_store,
            executor.lifted,
...
/// language tag maps, prepared transform, and once-derived gate-verified put program. Building
...
struct AcceptanceContext {
    ...
    inverse_tag_map: BTreeMap<String, String>,
    put_program: put_executor::PutLegProgram,
    transform_program: transform::TransformProgram,
}
...
impl AcceptanceContext {
    fn load(root: &Path) -> gmeow_errors::Result<Self> {
        ...
        let inverse_tag_map = invert_tag_map(&tag_map).into_iter().collect();
        ...
        let transform_program = transform::TransformProgram::prepare(
            &ontology_nt,
            &load_cells(root)?,
            &denied_cells(root)?,
...
fn run_acceptance_with(
    ...
    let executor = put_executor::execute_put_legs_default_graph(&source_store, &ctx.put_program)?;
    if executor.dataset.quad_count() == 0 {
        return Err(gmeow_errors::Diag::of_kind(crate::error::Scoreboard {
            message: format!(
    ...
    let (mut gates, output_triples) = draft_gates(root, &source_store, ctx, &executor)?;
...
fn retag_dataset_to_internal(
    dataset: &Arc<RdfDataset>,
    inverse_tag_map: &BTreeMap<String, String>,
) -> gmeow_errors::Result<Arc<RdfDataset>> {
    let mut quads = purrdf::flat_rdf_quads_from_dataset(dataset);
    crate::projections::retag_quads(&mut quads, inverse_tag_map);
    flat_dataset_from_quads(&quads)
        .map_err(|message| Diag::of_kind(crate::error::Scoreboard { message }))
}
...
#[path = "scoreboards.tests.rs"]
#[cfg(test)]
mod tests;

#[cfg(test)]
#[path = "scoreboards_test_support.rs"]
mod test_support;
#[cfg(test)]
use test_support::dataset_from_nt;

...
//! - Interning is keyed on exact native `TermValue` identity. Rendering is lazy
//!   output work; it cannot alias distinct datatypes, scopes or quoted terms.
//!   Datatype value equality is a separate operation of the native reasoner.
...
pub(crate) fn skolemize(term: &TermValue) -> std::borrow::Cow<'_, TermValue> {
    match term {

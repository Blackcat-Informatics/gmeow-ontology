...
use crate::projections::get_leg::{
    Atom, MappingPattern, PROFILES, ProfileBinding, ProjectionCell, curie, local,
};
...
/// per-correspondence loss ledger. The admitted analysis supplies mapping cells;
/// `onto` supplies the ontology character and language-tag map.
pub fn lower_edoal(
    onto: &DslView,
    lookup: &CorrespondenceAnalysis,
) -> gmeow_errors::Result<EdoalLowering> {
    let cells = lookup.projection_cells();
...
fn build_tag_map(onto: &DslView) -> BTreeMap<String, String> {
    ...
    for (subject, object) in onto.quads_with_predicate(LANG_CARRIER_TAG) {
        let (Some(internal), Some(subj_iri)) =
            (crate::ingest::literal_lexical(&object), subject.as_iri())
        else {
            continue;
        };
...
fn emit_edoal_nt(
    ...
    for cell in cells {
        for b in &cell.bindings {
            ...
            // complete source binding identity — the single source of truth — not re-derived
            // inline. A miss is a HARD FAIL: every authored binding is transpiled.
            let typed = lookup.binding(cell, b)?;
            ...
            // One preservation row per semantic binding that drops
            ...
            if b.confidence.is_some() {
                residue.push("get-leg: confidence is projected as binary64; original RDF datatype and lexical identity are not carried by EDOAL".to_owned());
            }
            let key = crate::projections::get_leg::binding_key(cell, b);
...
fn expr_mints_iri(expr: &crate::projections::get_leg::Expr) -> bool {
    ...
    matches!(expr, Expr::ConstTerm(crate::ingest::DslTerm::Iri(_)))
        || matches!(expr, Expr::Op { op, .. } if op == GM_OP_IRI)
}
...
fn make_cell(
    ...
    let node = Nt::stable_bnode(&format!(
        "cell-{}-{key}",
        crate::projections::get_leg::binding_key(cell, b)
    ));
    ...
    if let Some(conf) = &b.confidence {
        nt.add_typed_literal(
            ...
            &format_double(conf.projection_f64()),
            XSD_DOUBLE,
        );
    }
    node

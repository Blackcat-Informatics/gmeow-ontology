...
# `gmeow-rdf-capi` — libpurrdf

The **purrdf semantic C-ABI** (purrdf parcel P8, issue #842): a stable,
SemVer-disciplined `extern "C"` surface over the native `gmeow-rdf` RDF-1.2
...
`gmeow-gts` Rust crate, so a language shim links **`libpurrdf` alone** and still
...
AGPL-3.0-only (the semantic layer). The permissive `gmeow-gts` I/O core it

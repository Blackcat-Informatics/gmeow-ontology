...
fn a_binary_target_is_carried_as_declared_base64() {
    ...
    let receipt = tool["gts_ingestion_receipt"]["cbor"]
        .as_str()
        .expect("GTS output carries its receipt");
    assert_eq!(tool["gts_ingestion_receipt"]["encoding"], json!("base64"));
    let receipt = crate::base64_decode("ingestion receipt", receipt).expect("receipt base64");
    let native = gmeow_gts_profile::read_ingestion_receipt(&direct.bytes, &receipt)
        .expect("receipt authenticates the exact output");
    assert_eq!(Some(native.ingestion), direct.gts_ingestion);
    assert!(native.source_receipts.is_empty());
    assert_eq!(
        tool["bytes"],

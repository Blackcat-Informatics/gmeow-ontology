...
`gmeow:vantage` is the model `gmeow:SoftwareAgent`, `gmeow:wasGeneratedBy` its
`gmeow:ModelInvocation` (new), with confidence / `accordingTo` / the clocks as
RDF 1.2 statement annotations (the canonical form — see `dsl/statements/ai.ttl`).

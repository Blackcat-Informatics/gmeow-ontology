...
pub fn crosscheck_queries() -> i32 {
    let root = project_root();
    let dataset = match merged_query_dataset(&root) {
        Ok(d) => d,
        Err(code) => return code,
    };
    ...
    println!(
        "{} queries answer on the native purrdf engine ({} skipped, no divergence)",
        outcome.checked,
        outcome.skipped.len()
    );
    0
...
/// The `engine-cross-check` feedback surface: prove every committed query answers
/// on the shippable native engine, folding each divergence as one error finding.
///
/// With rdflib retired the two-engine agreement is a single-engine soundness proof
...
pub fn crosscheck_queries_report(root: &Path) -> Result<Report, String> {
    ...
    for diverged in &outcome.diverged {
        report.add_finding(
            Finding::new(
                Severity::Error,
                "engine-cross-check.diverge",
                diverged.clone(),
            )
            .with_tool("engine-cross-check"),
        );
    }
    Ok(report)
}

/// The flat dataset the committed queries execute over: the snapshot's base graph.
fn merged_query_dataset(root: &Path) -> Result<Arc<purrdf::RdfDataset>, i32> {
    let bytes = snapshot_bytes(root)?;
    let base = gmeow_pipeline::projections::gts_base_graph(&bytes)
        .map_err(|e| fail(format!("cannot read base graph: {e}")))?;
    purrdf::flat_dataset_from_quads(&base).map_err(|e| fail(format!("cannot build dataset: {e}")))
}

/// The committed query files the crosscheck proves answer on the native engine.
fn committed_query_files(root: &Path) -> Vec<PathBuf> {
...
/// The tally of one committed-query crosscheck run.
...
/// Execute every committed query over `dataset` on the native purrdf engine. A
/// file that does not parse as ONE executable query is a multi-query demonstration
/// document (SKIPPED, exactly as the two-engine crosscheck did); only a query that
/// parses but then errors at evaluation is a real divergence from the corpus.
fn run_query_crosscheck(
    ...
    for path in queries {
        ...
        match engine.query(
            ...
            Err(e) => outcome.diverged.push(format!("{}: {e}", path.display())),
        }
    }
    outcome
}

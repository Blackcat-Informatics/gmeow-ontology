...
//! JSON-LD `@graph` projector for GMEOW instance data (#700).
//!
...
mod tests {
    ...
    fn test_project_graph_envelope_and_context() {
        let store = load(&format!(
            r#"{PREFIXES_TTL}
            gmeow:alice a gmeow:Person ;
                gmeow:name "Alice" .
        "#
        ));
        let doc = project_graph(&store);
        // @context carries the prefix map (gmeow:).
        assert_eq!(
            doc["@context"]["gmeow"],
            json!("https://blackcatinformatics.ca/gmeow/")
        );
        let graph = doc["@graph"].as_array().expect("@graph array");
        assert_eq!(graph.len(), 1);
        let node = &graph[0];
        assert_eq!(node["@id"], json!("gmeow:alice"));
        assert_eq!(node["@type"], json!("gmeow:Person"));
        assert_eq!(node["gmeow:name"], json!("Alice"));
    }
    ...
    fn test_object_property_is_node_ref() {
        let store = load(&format!(
            r#"{PREFIXES_TTL}
            gmeow:org gmeow:member gmeow:alice .
        "#
        ));
        let doc = project_graph(&store);
        ...
        let org = graph
            .iter()
            .find(|n| n["@id"] == json!("gmeow:org"))
            .expect("org node");
        assert_eq!(org["gmeow:member"], json!({ "@id": "gmeow:alice" }));
    }
    ...
    fn test_typed_and_numeric_and_lang_literals() {
        let store = load(&format!(
            r#"{PREFIXES_TTL}
            gmeow:e gmeow:count 3 ;
                gmeow:flag true ;
                gmeow:at "2026-06-23T00:00:00Z"^^xsd:dateTime ;
                gmeow:label "bonjour"@fr .
        "#
        ));
        let doc = project_graph(&store);
        let node = &doc["@graph"].as_array().unwrap()[0];
        // integer → bare scalar
        assert_eq!(node["gmeow:count"], json!(3));
        // boolean → bare scalar
        assert_eq!(node["gmeow:flag"], json!(true));
    ...
    fn test_multi_valued_predicate_is_array_and_sorted() {
        let store = load(&format!(
            r#"{PREFIXES_TTL}
            gmeow:x gmeow:tag "b", "a", "c" .
        "#
        ));
        let doc = project_graph(&store);
        let node = &doc["@graph"].as_array().unwrap()[0];
        let tags = node["gmeow:tag"].as_array().expect("array");
    ...
    fn test_determinism_byte_stable() {
        let ttl = format!(
            r#"{PREFIXES_TTL}
            gmeow:alice a gmeow:Person ; gmeow:name "Alice" ; gmeow:age 30 .
            gmeow:bob a gmeow:Person ; gmeow:name "Bob" .
        "#
        );

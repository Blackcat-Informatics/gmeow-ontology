...
| Family | Modules | Role |
| --- | --- | --- |
| Engine bridges | `nemo_engine`, `scryer_engine`, `rule_ir`, `sparql_path_lower`, `materialize`, `dispatch` | Drive native rule and query engines from the shared runtime IR. |
...
- Compiler-only data structures belong in `gmeow-logic-compile`; this crate is
  allowed to depend on Nemo/PyO3 and is native-only.

...
"""``gmeow_rdf.compat.rdflib`` — the purrdf P0 rdflib drop-in surface.
...
migration is a pure import-prefix swap:
    ...
    →
    from gmeow_rdf.compat.rdflib import Graph, URIRef, Literal, RDF

Submodules mirror RDFLib's layout (``.term``, ``.namespace``, ``.graph``,
``.collection``, ``.compare``, ``.util``, ``.query``) so ``from rdflib.namespace
import OWL`` → ``from gmeow_rdf.compat.rdflib.namespace import OWL`` and so on.
"""

...
| [`INHABITED-IDENTITY.md`](INHABITED-IDENTITY.md) | de-conflation charter | the `SoftwareAgent` six-category de-conflation via the five-facet template; `DigitalSubject` as an anti-rigid role-mixin with a tenure; identity-continuity |
| [`INHABITED-TOPOLOGY.md`](INHABITED-TOPOLOGY.md) | formal spec | the `InhabitationTenure` + `InhabitationConfiguration` situations, `Inhabitant` / `InhabitedSystem` role-mixins, the `Embodiment` carrier + assignment, `inhabitationLocusKind`, co-tenancy + `ControlAssessment`, the migration transition, the deferred `Holon` |
...
| [`INHABITED-RUNTIME-STACK.md`](INHABITED-RUNTIME-STACK.md) | formal spec (model-serving) | the thin `extensions/model-serving` slice (model / deployment / execution / session) and the `agent-runtime` profile that selects it; the agentic deferral consumed |
...
| [`INHABITED-LOGIC.md`](INHABITED-LOGIC.md) | ceiling charter | the maximal `gmeow:logic` capabilities the slice pioneers — possible-worlds/counterfactual identity, paraconsistent continuity, concurrent-transaction co-tenancy, reflective self-continuity, scoped open/closed world, belief revision; the slice as a logic conformance corpus |

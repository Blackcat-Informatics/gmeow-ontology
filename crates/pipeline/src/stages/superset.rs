...
pub(crate) mod source_contracts;
...
impl RdfFanoutClasses {
    ...
    pub(crate) fn from_source(source: &RdfDataset) -> Result<Self, gmeow_errors::Diag> {
        Ok(Self::from_rules(read_fanout_rules(source)?))
    }

    /// Build the native index from already-read typed rules, including authenticated
    /// producer observations. This does not read or lower an RDF source again.
    fn from_rules(rules: Vec<FanoutRule>) -> Self {
        let rules = rules
            .into_iter()
...
fn walk(dir: &Path, root: &Path, out: &mut Vec<String>) -> Result<(), gmeow_errors::Diag> {
    ...
    for entry in entries {
        ...
        // `.cache/gmeow-sync/actions/` (gitignored shared action cache). The gate

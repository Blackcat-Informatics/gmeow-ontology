...
//! AGPL IR engine (`gmeow-rdf-core`) and the permissive GTS container
//! (`gmeow-gts`) depend ON — so a pure parse→serialize path that touches only these
//! traits stays pure-permissive (`MIT OR Apache-2.0`), owned by neither side. Its
//! value types ([`EventTerm`], [`EventQuad`], …) are therefore **self-contained**:
...
//! event stream folds *into* something. Its dual lives in `gmeow-rdf-core` as

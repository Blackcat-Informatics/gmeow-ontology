...
const NON_CONFORMANT: &[&str] = &[
    ...
    "slices/grounding/math/examples/reference-ast-act.ttl", // math:normalizationStrength → math:structuralNormalization (a shared math:NormalizationStrength individual typed in module.ttl) is untyped standalone, so the exact-one qualified-cardinality restriction on math:NormalizationDeclaration's math:normalizationStrength (sh:class-scoped, QualifiedMinCountConstraintComponent) finds no conforming value in isolation — illustrative, conformant in the merged bundle (make validate passes)

...
/// Producer-owned fixture for exercising the consumer verification verb over the
/// real bundled `dcat.rq` projection without rebuilding that manifest in a test.
pub mod consumer_verification_fixture {
    use super::{BTreeMap, Diag, DistributionEntry};

    /// Authenticated artifact emitted by the explicit bundle fixture producer.
    pub const ARTIFACT: &str = "docs-distribution-verification-manifest-v1.nt";

    /// Tiny deterministic documentation tree whose digest the manifest carries.
    #[must_use]
    pub fn site_tree() -> BTreeMap<String, Vec<u8>> {
        BTreeMap::from([
            ("about.html".to_string(), b"<html>about</html>".to_vec()),
            ("index.html".to_string(), b"<html>hello</html>".to_vec()),
        ])
    }

    /// Build the authenticated manifest during the explicit producer phase.
    ///
    /// # Errors
    /// Propagates deterministic tree packaging or bundled-query projection failures.
    pub fn produce(gts_bytes: &[u8]) -> Result<Vec<u8>, Diag> {
        let digest = super::distribution_blake3(&site_tree())?;
        ...
        super::build_docs_distribution_manifest(&entries, &[], gts_bytes).map(String::into_bytes)
    }
}
...
#[path = "docs_distribution.tests.rs"]
#[cfg(test)]
mod tests;

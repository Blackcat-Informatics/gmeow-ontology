...
//! Native `gmeow:sliceDependsOn` reconciliation patcher (#820 G8).
//!
...
//!   `gmeow:sliceDependsOn` objects (RDF-aware), the edit reuses the author's
...
/// Compute the RDF-aware `gmeow:sliceDependsOn` reconciliation patches for every
...
fn apply_proposal(original: &str, proposal: &DepProposal) -> Result<String, SliceError> {
    ...
    let gmeow_ns = extract_gmeow_prefix(original);
    let patched = surgical_edit(original, &gmeow_ns, &desired, !existing.is_empty())?;
...
/// canonical namespace if no explicit `@prefix gmeow:` is present).
fn extract_gmeow_prefix(text: &str) -> String {
    for line in text.lines() {
        let trimmed = line.trim_start();
        if let Some(rest) = trimmed.strip_prefix("@prefix") {
            let rest = rest.trim_start();
            if let Some(after) = rest.strip_prefix("gmeow:") {
...
/// form (matching every authored manifest, whose `gmeow:slices/<name>` locals are
/// awkward as prefixed names because of the embedded `/`).
fn render_object(iri: &str, _gmeow_ns: &str) -> String {
    format!("<{iri}>")
}

/// Apply the targeted textual edit by **rewriting the whole `gmeow:sliceDependsOn`
...
fn surgical_edit(
    ...
    match find_depends_on_block(original) {
        Some(block) => rewrite_block(original, gmeow_ns, desired, &block),
        None => {
            if !had_existing_block && !desired.is_empty() {
                insert_new_block(original, gmeow_ns, desired)
            } else {
                ...
                Err(SliceError::InvalidManifest(
                    "fix-deps: could not locate the gmeow:sliceDependsOn predicate \
...
/// The byte span of a `gmeow:sliceDependsOn` predicate block: from the start of
...
/// Locate the `gmeow:sliceDependsOn` predicate block in the Turtle text. Scans
...
fn find_depends_on_block(text: &str) -> Option<DependsBlock> {
    let needle = "gmeow:sliceDependsOn";
...
fn rewrite_block(
    ...
    let objects: Vec<String> = desired.iter().map(|o| render_object(o, gmeow_ns)).collect();
    let rendered = if objects.len() == 1 {
        format!("gmeow:sliceDependsOn {}{}", objects[0], block.terminator)
    } else {
        ...
        let body = objects
            ...
            .join(block.newline);
        format!(
            "gmeow:sliceDependsOn{nl}{body}{nl}{indent}{term}",
            nl = block.newline,
            indent = block.indent,
...
/// Insert a brand-new `gmeow:sliceDependsOn` predicate block after the
/// `a gmeow:Slice` declaration when the manifest declared no dependencies yet.
fn insert_new_block(
    original: &str,
    gmeow_ns: &str,
    desired: &BTreeSet<String>,
) -> Result<String, SliceError> {
    // Find the `a gmeow:Slice` line to anchor after, and its indentation.
    let anchor_rel = original
        .find("a gmeow:Slice")
        .or_else(|| original.find("a\tgmeow:Slice"))
        .ok_or_else(|| {
            SliceError::InvalidManifest(
                "fix-deps: manifest has no `a gmeow:Slice` line to anchor a new \
    ...
    let block = if objects.len() == 1 {
        format!("{indent}gmeow:sliceDependsOn {} ;{newline}", objects[0])
    } else {
        ...
        let body = objects
            ...
            .collect::<Vec<_>>()
            .join(newline);
        format!("{indent}gmeow:sliceDependsOn{newline}{body} ;{newline}")
    };
...
<https://blackcatinformatics.ca/gmeow/slices/sliceB> a gmeow:Slice ;
    gmeow:sliceTier gmeow:tierCore ;
    gmeow:sliceDependsOn <https://blackcatinformatics.ca/gmeow/slices/sliceA> .
";
        ...
        // carry the terminal `.`.
        assert!(patched.contains("gmeow:sliceTier gmeow:tierCore ."));
    }
...
<https://blackcatinformatics.ca/gmeow/slices/sliceB> a gmeow:Slice ;
    gmeow:sliceTier gmeow:tierCore ;
    rdfs:label \"sliceB\"@x-gmeow-english .
";
        let p = proposal("sliceB", &["sliceA"], &[]);
        let patched = apply_proposal(manifest, &p).expect("patch must succeed");
        let store = parse_turtle(patched.as_bytes(), "test").expect("must re-parse");
...
<https://blackcatinformatics.ca/gmeow/slices/sliceB> a gmeow:Slice ;
    gmeow:sliceDependsOn <https://blackcatinformatics.ca/gmeow/slices/stale1> ;
    gmeow:sliceTier gmeow:tierCore ;
    rdfs:label \"sliceB\"@x-gmeow-english .
";
...
";
        ...
        assert!(patched.contains("rdfs:label \"agentic\"@x-gmeow-english ."));
    }
}

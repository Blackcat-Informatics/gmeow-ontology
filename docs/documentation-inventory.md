...
| `docs/GTS-SPEC.md` | Full GTS transport specification | Large normative spec; GTS slice references it | `move-to-slice-design` | Likely target: `slices/core/gts/design/GTS-SPEC.md`. Keep as authoritative transport design once generator input tracking is fixed. |
...
| `docs/engine-crosscheck.md` | pyoxigraph trust note | Historical ticket note | `update-or-archive` | Likely implementation history. Keep only if still useful for test/toolchain rationale. |
...
| `docs/gts-reference.md` | Python GTS reference implementation note | GTS implementation detail | `move-to-slice-design` | Likely target: `slices/core/gts/design/REFERENCE-IMPLEMENTATION.md`, or archive if superseded by current Python/Rust/Go docs. |
...
| `docs/lpg-mapping.md` | RDF-to-LPG mapping specification | Projection/tooling design | `move-to-slice-design` | Likely cross-cutting projection design. Until a projection slice/area exists, consider `slices/core/logic/design/` or a future generated interop design area. |
...
| `docs/reasoning.md` | OWL/SHACL reasoning doctrine | Logic/toolchain doctrine; stale paths | `move-to-slice-design`, `promote-to-ontology` | Likely target: `slices/core/logic/design/REASONING.md`. Extract concise reasoner-vs-validator explanations for docs generation. |

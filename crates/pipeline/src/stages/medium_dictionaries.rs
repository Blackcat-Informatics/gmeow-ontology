...
use crate::medium::envelope::{
    DigestStratum, FrameFacts, MediumEnvelope, PrehashedFrameFacts, seal, seal_prehashed,
};
...
///
/// `snapshot_content_digest` is `snapshot_content_id()` over the ENVELOPE-FREE
/// snapshot payload; `snapshot_strata_digest` is the identity of that same
/// payload's canonical quad set, the region
/// `gmeow:stratumPayloadExcludingMediumEnvelope` names. The whole-corpus byte
/// documents have already left scope before this function runs: an envelope needs
/// their identities, not second resident copies of their bytes. Adding the sealed
/// envelopes cannot change either value — the first is taken before they exist, the
/// second over a region that excludes them — so the emission reaches its fixed point
/// in exactly two passes rather than iterating.
///
/// # Errors

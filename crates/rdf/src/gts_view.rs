...
use gmeow_gts::model::{BlobEntry, Graph, Quad, Term, TermKind, Triple3};
...
mod tests {
    ...
    fn language_boundary_reads_tag_map_across_all_scopes() {
        ...
        writer.add_terms(&[
            ...
            lit("x-gmeow-english", Some("en"), None),
            lit("en", Some("en"), None),
            iri(&(NAMESPACE.to_string() + "French")),
            lit("x-gmeow-french", Some("en"), None),
            lit("fr", Some("en"), None),
            iri(&(NAMESPACE.to_string() + "Thing")),
            lit("Hello", Some("x-gmeow-english"), None),
            lit("Bonjour", Some("x-gmeow-french"), None),
        ...
        let view = GtsFoldView::new(gmeow_gts::reader::read(&writer.to_bytes(), true, None));
        assert_eq!(
            view.tag_map().get("x-gmeow-english"),
            Some(&"en".to_string())
        );
        assert_eq!(
            view.tag_map().get("x-gmeow-french"),
            Some(&"fr".to_string())
        );
        assert_eq!(

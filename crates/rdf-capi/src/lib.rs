...
//! # libpurrdf — the GMEOW purrdf semantic RDF-1.2 C-ABI (purrdf P8, #842)
//!
//! A stable, SemVer-disciplined `extern "C"` surface over the native
//! `gmeow-rdf` semantic stack. It is the rich companion to the permissive
...
//! reuses the permissive `gmeow-gts` crate, so no second `.so` is needed.
//!

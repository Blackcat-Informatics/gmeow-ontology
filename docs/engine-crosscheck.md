...
- **Jena** emits the canonical RDF 1.2 lead artifact and runs OWL reasoning
  (OWL 2 DL semantics via the in-process oracle cross-check). gmeow_rdf does **not** reason.
...
- decimal tokens inside string literals (e.g. a `STR(?decimal)`-built
  ...
  keeps trailing zeros in rdflib but not in gmeow_rdf.
...
gmeow_rdf (`gmeow_tools.sparql._load_base`). An rdflib→N-Triples→gmeow_rdf

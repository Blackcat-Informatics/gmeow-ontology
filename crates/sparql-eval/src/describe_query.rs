...
//! **Symmetric Concise Bounded Description** ([`gmeow_rdf_core::describe`]) — the same
//! CBD the docs multi-format export uses — so `DESCRIBE`, the `gmeow` CLI, and the
...
use gmeow_rdf_core::describe::Describer;
use gmeow_rdf_core::{RdfDataset, TermValue};
use gmeow_sparql_algebra::{GraphPattern, NamedNodePattern};
...
mod tests {
    ...
    use gmeow_sparql_algebra::{NamedNode, TermPattern, TriplePattern, Variable};

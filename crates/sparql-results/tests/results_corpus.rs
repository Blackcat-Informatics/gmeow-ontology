...
//! and the populated-`gmeow`-provenance behaviour across every format — including
...
// ---------------------------------------------------------------------------
// 4. POPULATED PROVENANCE — JSON/XML carry the gmeow extension; CSV/TSV trim it
...
fn select_books_csv_drops_provenance_and_stays_pure() {
    ...
    assert!(
        !body.contains("gmeow"),
        "CSV must stay pure W3C (no gmeow leak): {body}"
    );
}

...
- **The floors are a raise-only ratchet — now a hard gate.** The committed floors are the
  ...
  **ontology-resident** in the slice-quality-rubric slice's `module.ttl`: the per-slice tier
  ...
  `generated/governance/slice-quality-axis-floors.tsv` (materialized locally by `make check`)
...
- **The contention rule: yield to the issue lanes.** The lane is a background citizen. Two
  ...
  git-ignored local product regenerated by `make check` — land bundle-touching PRs one at a
...
- Work in a worktree (`.worktrees/<slug>/`), never the top-level checkout; build the
  native extensions before regenerating; regenerate with `make check`, never the bare
...
- [ ] `make check` landed with the change; drifted goldens re-blessed deliberately;

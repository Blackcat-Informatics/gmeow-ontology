...
//! there is **no** `SliceId` or any GMEOW-specific concept in this module. The
//! slice layer (`gmeow-slice`) is responsible for interpreting unit kinds.
//!
//! ## Core types
//!
...
/// `gmeow-slice` layer interprets `Slice`-kind units by wrapping `UnitId`.
///
...
pub enum OriginKind {
    ...
    Source,
    /// The root/top-level ontology file (e.g. `ontology/gmeow.ttl`).
    RootOntology,
...
mod tests {
    ...
    fn artifact_interner_path_resolves() {
        let mut i = ArtifactInterner::new();
        let id = i.intern("ontology/gmeow.ttl");
        assert_eq!(i.path(id), "ontology/gmeow.ttl");
    }
    ...
    fn gate_fails_when_occurrence_missing_for_quad() {
        ...
        let artifact = prov.register_artifact("ontology/gmeow.ttl");
        let q0 = qh(0);

...
//! parse + validate behaviour (#848). It carries the GMEOW mapping artifacts
...
//! deliverable).
//!
//! ## The SSSOM TSV format (as GMEOW writes it)
//!
//! A GMEOW SSSOM file is a YAML-ish `#`-prefixed metadata header followed by a
...
//! gmeow:Foo  skos:closeMatch  sosa:Observation  semapv:ManualMappingCuration  0.7  …
...
//! set of indented `#   prefix: uri` lines. GMEOW *owns* this format, so the
...
/// because GMEOW never emits a pipe-bearing entity slot. Exposed so a consumer can
...
///
/// The named scalars cover everything GMEOW writes; any other `# key: value`
/// scalar is preserved verbatim in [`extra`](SssomMeta::extra) so nothing is lost
/// (maximal information flow). The `comment` value is kept *as authored* — GMEOW
...
///
/// The named columns cover GMEOW's [`SSSOM_ORDER`]; any other column is preserved
/// verbatim in [`extras`](SssomMapping::extras) so a non-GMEOW SSSOM file still
...
/// Parse a GMEOW SSSOM TSV document into an owned [`SssomMappingSet`].
///
...
fn parse_header(lines: &[&str]) -> Result<SssomMeta, RdfDiagnostic> {
    ...
    for (offset, raw) in lines.iter().enumerate() {
        ...
        // principle, though GMEOW always writes the trailer last).
        ...
        // scalar (or end of header). GMEOW indents curie entries with three
        ...
        match key {
            "curie_map" => {
                ...
                // is a malformed flow sequence GMEOW never emits — reject it the
...
/// verbatim (no YAML unquoting — GMEOW owns the format and the comment value is
...
/// an absolute IRI (`http://`, `https://`, …). GMEOW writes bare absolute URIs for
...
/// Serialize a mapping set back to canonical GMEOW SSSOM TSV.
///
...
gmeow:Foo\tskos:closeMatch\tsosa:Observation\tObservation\tsemapv:ManualMappingCuration\t0.7\ta note
";
...
";
    ...
    fn parse_ignores_trailer_comments() {
        let doc = format!("{CLEAN}# # REFUSED gmeow:Bar — out of scope\n# # provenance note\n");
        let set = parse_tsv(&doc).expect("parse");
...
";
    ...
    fn validate_flags_confidence_too_high() {
        let set = parse_tsv(&negative(
            "gmeow:A\tskos:closeMatch\tsosa:Observation\tsemapv:ManualMappingCuration\t1.5\tx\n",
        ))
        .expect("parse");
        let diags = validate(&set);
    ...
    fn validate_flags_confidence_negative() {
        let set = parse_tsv(&negative(
            "gmeow:A\tskos:closeMatch\tsosa:Observation\tsemapv:ManualMappingCuration\t-0.5\tx\n",
        ))
        .expect("parse");
        let diags = validate(&set);
        let js: Vec<_> = diags.iter().filter(|d| d.check == "JsonSchema").collect();

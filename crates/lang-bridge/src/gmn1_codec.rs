...
impl GmnGlyphRegistry {
    ...
    pub fn from_dataset(ds: &RdfDataset) -> Result<Self, GlyphRegistryError> {
        ...
        Self::from_current_codebook(ds, &codebook)
    }

    fn from_current_codebook(
        ds: &RdfDataset,
        codebook: &CurrentCodebook,
    ) -> Result<Self, GlyphRegistryError> {
...
/// Decode an authored GMN glyph's canonical uppercase `U+XXXX` sequence.
/// Invalid tokens, non-scalar values and empty sequences are rejected.
pub fn decode_codepoint_sequence(value: &str) -> Result<String, GlyphRegistryError> {
...
/// Compile the selected dictionary and its codebook identity from one native
/// source. The dictionary and glyph table share the same resolved membership.
pub fn compile_gmn_codebook(
    ds: &RdfDataset,
) -> Result<(GmnDictionary, CurrentCodebook), DictionaryError> {
    let codebook = resolve_current_codebook(ds).map_err(|error| DictionaryError(error.0))?;
    let dictionary = GmnDictionary::from_current_codebook(ds, &codebook)?;
    Ok((dictionary, codebook))
}
...
impl GmnDictionary {
    ...
    pub fn from_dataset(ds: &RdfDataset) -> Result<Self, DictionaryError> {
        compile_gmn_codebook(ds).map(|(dictionary, _)| dictionary)
    }
    ...
    fn from_current_codebook(
        ...
        let glyphs = GmnGlyphRegistry::from_current_codebook(ds, codebook)
            .map_err(|error| DictionaryError(error.0))?;
        ...
        Ok(Self {
            version: codebook.dictionary_version.clone(),
            term_to_alias,
            alias_to_term,
            glyphs,
            acceptance,
    ...
    /// Borrow the selected dictionary's exact alias for an IRI.
    #[must_use]
    pub fn alias_for(&self, iri: &str) -> Option<&str> {
        self.term_to_alias.get(iri).map(String::as_str)
    }

    /// Borrow the selected dictionary's exact reverse binding for an alias.
    #[must_use]
    pub fn term_for(&self, alias: &str) -> Option<&str> {
    ...
    #[must_use]
    pub fn alias_entries(&self) -> &BTreeMap<String, String> {
        &self.term_to_alias
    }
...
/// the producer's construct-coverage observation runs over the selected grounding slices.
#[derive(
...
impl ConstructCoverageTally {
    /// Combine already observed source-local counts without reclassifying models.
    pub fn merge(&mut self, other: &Self) {
        for (category, count) in &other.counts {
            *self.counts.entry(*category).or_insert(0) += count;
        }
        self.uncovered.extend(other.uncovered.iter().cloned());
    }

    /// Fold every quad of `model` into this tally, via [`classify_model`].
    pub fn absorb(&mut self, model: &Gmn0Model, dict: &GmnDictionary) {
        self.absorb_classifications(classify_model(model, dict));
    }

    /// Fold existing codec classifications without dispatching over the model again.
    pub fn absorb_classifications(
        &mut self,
        classifications: impl IntoIterator<Item = QuadCoverage>,
    ) {
        for coverage in classifications {
            match coverage {
                QuadCoverage::Covered {
                    subject,
                    predicate,
...
/// One transient native round trip shared by the selected semantic checks. The
/// canonical strings belong to the whole model, never isolated claim fragments.
pub(crate) struct RoundTrip {
    pub(crate) document: Gmn1Document,
    pub(crate) reconstructed: Gmn0Model,
    pub(crate) original_canonical: String,
    pub(crate) reconstructed_canonical: String,
}

impl RoundTrip {
    pub(crate) fn evaluate(model: &Gmn0Model, dict: &GmnDictionary) -> Result<Self, Gmn1Error> {
        let document = gmn1_write(model, dict)?;
        let reconstructed = gmn1_read(&document, dict)?;
        let original_canonical = model.canonical_nquads();
        let reconstructed_canonical = reconstructed.canonical_nquads();
        Ok(Self {
            document,
            reconstructed,
            original_canonical,
    ...
    pub(crate) fn check_canonical(&self) -> Result<(), Gmn1Error> {
        if self.original_canonical == self.reconstructed_canonical {
            Ok(())
        } else {
            Err(non_decodable(format!(
                "round-trip canonical mismatch:\n--- original ---\n{}\n--- reconstructed ---\n{}",
                self.original_canonical, self.reconstructed_canonical
            )))
        }
    }
}
...
pub fn round_trip_check(model: &Gmn0Model, dict: &GmnDictionary) -> Result<(), Gmn1Error> {
    RoundTrip::evaluate(model, dict)?.check_canonical()
}

#[path = "gmn1_codec.tests.rs"]
#[cfg(test)]
mod tests;

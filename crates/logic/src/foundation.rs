...
impl ModalFact for FoundationQuad {
    fn graph(&self) -> &str {
        &self.graph
    }
...
fn evaluate_modal_formulas(quads: &[FoundationQuad]) -> gmeow_errors::Result<Vec<FoundationQuad>> {
    modal::evaluate(quads).map(|verdicts| {
        verdicts
            .into_iter()
            .map(|verdict| FoundationQuad {
                ...
                object: n3(&verdict.object),
                rule_iri: verdict.rule_iri,
                source_quad_ids: verdict.source_quad_ids,
                derivation_id: verdict.derivation_id,
            })
            .collect()
    })
}

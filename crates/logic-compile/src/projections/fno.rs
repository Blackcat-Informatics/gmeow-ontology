...
fn extract_functions(dsl: &DslView) -> Result<Vec<ProjectionFunction>, String> {
    let mut out = Vec::new();
...
fn extract_cells(dsl: &DslView) -> Result<Vec<ProjectionCell>, String> {
    let mut out = Vec::new();
...
fn parse_pattern(dsl: &DslView, pattern: &DslTerm) -> Result<ParsedPattern, String> {
...
fn flatten_item(dsl: &DslView, node: &DslTerm, out: &mut Vec<Atom>) -> Result<(), String> {
...
fn parse_atom(dsl: &DslView, node: &DslTerm) -> Result<Atom, String> {
    let predicate = dsl
...
fn parse_bind(dsl: &DslView, node: &DslTerm) -> Result<Bind, String> {
    let Some(var) = dsl.object_literal_of_term(node, GM_BIND_VAR) else {
        return Err("bind missing bindVar".to_owned());
    };
    let mut refs = BTreeSet::new();
    let Some(expr_node) = dsl.first_object_of(node, GM_BIND_EXPR) else {
        return Err("bind missing bindExpr".to_owned());
    };

...
use gmeow_conformance::external::ontouml::OntoumlError;
...
fn ontouml_divergence_cases_are_honest_gaps_never_a_reproduced_verdict() {
    ...
    for case_dir in subdirs(&root) {
        ...
        let observation = super::common::supplemental()
            .ontouml
            .get(&super::common::input_key(&model_path))
            .expect("selected OntoUML source")
            .as_ref()
            .expect("producer observed valid source bytes");
        match observation {

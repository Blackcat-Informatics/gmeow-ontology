...
fn concept_doi(root: &Path) -> Result<String, PipelineError> {
    let path = root.join("metadata").join("gmeow-self.ttl");
    let bytes = std::fs::read(&path)?;
    let ds = parse_dataset(&bytes, "text/turtle", None)
        .map_err(|e| PipelineError::Parse(format!("syntax error in self-desc: {e}")))?;
    if let Some(pred) = ds.term_id_by_value(&TermValue::iri(DCTERMS_IDENTIFIER)) {
        for q in ds.quads_for_pattern(None, Some(pred), None, GraphMatch::Default) {
            if let TermRef::Literal { lexical, .. } = ds.resolve(q.o) {
                if lexical.starts_with("10.") && lexical.contains('/') {
    ...
    Err(PipelineError::Parse(
        "no concept DOI (dcterms:identifier 10.…/…) in metadata/gmeow-self.ttl".to_string(),
    ))
}

/// Render the Apache content-negotiation include.
pub fn render_apache(root: &Path) -> Result<String, PipelineError> {

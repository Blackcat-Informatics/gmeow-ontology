...
use gmeow_sparql_results::ParsedSolutions;
...
fn compare_eval(case: &SparqlTestCase, result: &SparqlResult) -> Result<(), String> {
    match (&case.expected, result) {
        ...
        (ExpectedResult::Graph(path), SparqlResult::Graph(actual)) => {
            ...
            let expected = gmeow_rdf::parse_dataset(&expected_bytes, media, None)
                .map_err(|e| format!("parse expected graph {}: {e}", path.display()))?;
            let actual_canon = gmeow_rdf_core::canonicalize(actual).nquads;
            let expected_canon = gmeow_rdf_core::canonicalize(&expected).nquads;
...
fn read_solutions(path: &Path, json: bool) -> Result<ParsedSolutions, String> {
    ...
    if json {
        gmeow_sparql_results::from_json(&bytes)
    } else {
        gmeow_sparql_results::from_xml(&bytes)
    }
...
fn read_boolean(path: &Path, json: bool) -> Result<bool, String> {
    let bytes = std::fs::read(path).map_err(|e| format!("read {}: {e}", path.display()))?;
    if json {
        gmeow_sparql_results::from_json_boolean(&bytes)
    } else {
        gmeow_sparql_results::from_xml_boolean(&bytes)
    }

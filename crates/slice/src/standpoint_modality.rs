...
//!
//! The closed 5-value `gmeow:StandpointModality` vocabulary fuses four separable
...
/// field carries the qualitative `gmeow:CredenceLevel` band (the projection key),
/// not the numeric `gmeow:claimCredence` decimal.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct StandpointAxes {
    /// `gmeow:Polarity` value local name (`polarityAffirm` / `polarityDeny` / `polaritySuspend`).
    pub polarity: String,
    /// `gmeow:ModalForce` value local name.
    pub modal_force: String,
    /// `gmeow:CredenceLevel` qualitative band local name (the round-trip key).
    pub credence: String,
    /// `gmeow:AssertoricForce` value local name.
    pub assertoric_force: String,
    /// `gmeow:TruthDirectedness` value local name.
    pub truth_directedness: String,
    /// `gmeow:SupportStatus` value local name.
...
impl StandpointAxes {
    ...
    /// cross-check, where the `gmeow:decomposesToAxis` bundle is unordered. Local
...
pub(crate) struct Decomposition {
    /// The legacy `gmeow:StandpointModality` value local name.
...
/// `gmeow:StandpointModality` values — mirrored byte-for-byte by the
/// `gmeow:decomposesToAxis` bundles in `slices/core/standpoint/module.ttl`
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            ...
            "[UNKNOWN_LEGACY_MODALITY] {:?} is not one of the five legacy \
             gmeow:StandpointModality values (unequivocal, probable, conceivable, \
...
/// Expand a legacy `gmeow:StandpointModality` value (by local name) to its
...
/// Project a six-axis tuple DOWN to the one legacy `gmeow:StandpointModality`
...
pub fn project_down(axes: &StandpointAxes) -> ProjectionVerdict {
    ...
    ProjectionVerdict::Unsupported(format!(
        "[UNSUPPORTED_MODALITY_PROJECTION] six-axis tuple {} has no legacy \
         gmeow:StandpointModality equivalent; it is never silently approximated \
...
mod tests {
    ...
    /// The GMEOW vocabulary namespace — used only to build full IRIs when the
    ...
    fn rust_table_matches_committed_turtle_bundle() {
        ...
        for d in DECOMPOSITIONS {
            ...
            assert_eq!(
                committed,
                d.axes().value_set(),
                "gmeow:decomposesToAxis bundle for gmeow:{} in module.ttl must equal \

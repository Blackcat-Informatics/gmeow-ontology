...
- Avoid forcing a single role (it is non-functional — a channel may be both personal and work, and competing classifications coexist) and avoid encoding the provider organization here (use [`gmeow:contactPointProvider`](gmeow-contactPointProvider.md)).

### How to use

- Point the contact point at one or more [`gmeow:ContactPointType`](../classes/gmeow-ContactPointType.md) individuals; the projection layer reads them to select or label channels per consumer.

### Examples

- ex:lillithEmail [`gmeow:contactPointType`](gmeow-contactPointType.md) [`gmeow:contactPointTypeWork`](../individuals/gmeow-contactPointTypeWork.md).

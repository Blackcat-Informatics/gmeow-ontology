...
//! The flat-export read-side views: CSVW/Markdown/JSONL/llms.txt + the
//! dataset/semantic-web tiers (N-Quads, TriG, statements JSONL, SKOS, OBO Graphs,
//! ShEx) under git-ignored `dist/`.
//!
//! These are pure functions of a folded [`RdfDataset`], so they live here rather
//! than in `gmeow-pipeline`: the same renderers back the `stage-export-export`
//! stage, the `gmeow` consumer CLI's `describe`/`lookup` surface, and the MCP tool
//! surface, none of which should have to pull in the build executor. The stage
//! wrapper that reads THIS run's carrier and declares the DAG edges is
//! `gmeow_pipeline::stages::export`.
//!
//! The native flat-export leaf: reads ONLY the materialized GTS snapshot (the narrow
...
use crate::lpg_prefixes::PREFIXES_BY_LEN;
...
impl<'a> FoldView<'a> {
    ...
    pub fn with_requested(dataset: &'a RdfDataset, requested: Vec<String>) -> Self {
    ...
    pub fn lex(&self, tid: usize) -> &'a str {
        match self.tref(tid) {
            TermRef::Iri(s) => s,
    ...
    pub fn annotations(&self) -> Vec<(usize, usize, usize)> {
        self.dataset
            .annotations()
    ...
    pub fn reifiers(&self) -> Vec<(usize, (usize, usize, usize))> {
        self.dataset
            .reifiers()
    ...
    pub fn subjects_by_type(&self, class_iri: &str, scope: &str) -> Vec<usize> {
    ...
    pub fn objects(&self, s_tid: usize, p_iri: &str, scope: &str) -> Vec<usize> {
    ...
    pub fn predicate_objects(&self, s_tid: usize, scope: &str) -> Vec<(usize, usize)> {
    ...
    pub fn has(&self, s_tid: usize, p_iri: &str, o_tid: usize, scope: &str) -> bool {
        let Some(p_tid) = self.tid_of_iri(p_iri) else {
            return false;
        };
        self.spo
    ...
    pub fn public_text_with_fallback(&self, s_tid: usize, p_iri: &str) -> (String, bool) {
...
pub struct Term {
    pub category: &'static str, // "class" | "property" | "individual"
    pub iri: String,
    pub curie: String,
    pub label: String,
    ...
    pub definition_fallback: bool,
    pub parents: Vec<String>,
    pub prop_kind: &'static str,
    pub domain: String,
    pub range: String,
    pub functional: bool,
    ...
    /// Mirrors `gmeow_docs_model::model::logic_stereotypes` so the shared term card
...
/// `gmeow_docs_model::model::logic_stereotypes`.
...
/// read in `gmeow_docs_model::model::extract_terms`.
...
/// Build the neutral, shared [`gmeow_docs_model::card::Card`] from a folded [`Term`].
///
...
/// `gmeow_docs_model::card::render_card_body` renderer, so the MCP card and the site
...
pub fn term_to_card(t: &Term, modeled_defs: &BTreeSet<String>) -> gmeow_docs_model::card::Card {
...
pub fn fold_meta(view: &FoldView) -> Result<(String, String), gmeow_errors::Diag> {
...
/// A hard failure inside the flat-export renderers (a CSVW/SKOS/OBO-Graphs
/// projection that will not build, a term the projection cannot encode). This is a
/// leaf crate with no stage vocabulary, so it raises the crate's own
/// [`Export`](crate::error::Export) kind; the pipeline stage that drives these
/// renderers lifts the resulting `Diag` unchanged.
fn err(message: impl Into<String>) -> gmeow_errors::Diag {
    gmeow_errors::Diag::of_kind(crate::error::Export {
...
// format via `gmeow_docs_model::llms`: an `# H1` + `> {GMEOW_SUMMARY}` blockquote +
...
fn llms_sections(
...
) -> Vec<gmeow_docs_model::llms::LlmsSection> {
    let bullet = |t: &Term| gmeow_docs_model::llms::LlmsBullet {
        text: t.curie.clone(),
        url: doc_urls.and_then(|m| m.get(&t.iri).cloned()),
        signature: llms_signature(t),
        note: gmeow_docs_model::llms::cap_note(&llms_note(t)),
    };
    ...
    .map(|(heading, bullets)| gmeow_docs_model::llms::LlmsSection { heading, bullets })
    .collect()
}
...
fn write_llms_txt(
    ...
    title: &str,
    version: &str,
    primer: &gmeow_docs_model::gmn1_primer::Gmn1Primer,
) -> Vec<u8> {
    ...
    sections.push(gmeow_docs_model::llms::standing_reference_section());
    ...
    gmeow_docs_model::llms::render_index(title, &prose, &sections).into_bytes()
}
...
/// shared `gmeow_docs_model::card` renderer. Emitted in a deterministic total order and
/// bounded by [`gmeow_docs_model::llms::LLMS_FULL_TOKEN_BUDGET`] (the elided remainder is
...
pub fn consumer_llms_full(
    ...
    let mut out = gmeow_docs_model::llms::llms_header(title, &prose);
    out.push_str("## Terms\n\n");
    ...
    let budget = gmeow_docs_model::llms::LLMS_FULL_TOKEN_BUDGET;
    let mut used = gmeow_docs_model::llms::estimate_tokens(&out);
    ...
    for t in &ordered {
        ...
        let cost = gmeow_docs_model::llms::estimate_tokens(&block);
    ...
    out.push('\n');
    out.push_str(&gmeow_docs_model::llms::render_section(
        &gmeow_docs_model::llms::standing_reference_section(),
    ));
    ...
    out.push('\n');
    out.push_str(&gmeow_docs_model::llms::render_section(&primer.section()));
    out
}
...
// standard llmstxt.org format via the shared `gmeow_docs_model::llms` skeleton (`# H1`
...
// through the single `gmeow_docs_model::card` renderer, so the MCP card is the genuine
// twin of the docs-site `card.md`.
//
// Consumed unconditionally by the native MCP server surface (no longer
...
mod consumer {
    ...
    /// MCP-surface twin of [`gmeow_docs_model::describe::Resolution`]. A bare local name
    ...
    impl<T> ConsumerResolution<T> {
        /// The resolved payload, or `None` for `Ambiguous`/`NotFound` — for callers
        /// that only exercise the unique-resolution path and treat both failure modes
        /// alike. A caller that must DISTINGUISH them (the CLI and the MCP surface,
        /// which report an ambiguity's candidate list) matches on the variants.
    ...
    /// boundary instead.
    pub fn resolve_term_iri<'a>(terms: &'a [Term], query: &str) -> ConsumerResolution<&'a str> {
    ...
    pub fn lookup_envelope(terms: &[Term], query: &str) -> String {
    ...
    /// rendered through the shared `gmeow_docs_model::llms` emitter so it matches the
    ...
    pub fn consumer_llms_txt(
        ...
        sections.push(gmeow_docs_model::llms::standing_reference_section());
        gmeow_docs_model::llms::render_index(title, &prose, &sections)
    }

    /// `doc_card`: resolve `query` to a term and build its `# {curie}{signature}`
    /// title and the COMPACT shared [`gmeow_docs_model::card::Card`] — the live MCP twin
    /// of the docs-site `terms/{slug}/card.md`, through the ONE shared
    /// `gmeow_docs_model::card` builder + renderer (§19 one-path). `None` when the query
    ...
    /// populates the rich panels for [`gmeow_docs_model::card::CardDetail::Full`] from the
    ...
    pub fn doc_card_build(
        terms: &[Term],
        query: &str,
        modeled_defs: &BTreeSet<String>,
    ) -> ConsumerResolution<(String, gmeow_docs_model::card::Card)> {
        match resolve_term(terms, query) {
    ...
    pub fn doc_url_map(view: &FoldView) -> std::collections::HashMap<String, String> {
        let documents = format!("{NAMESPACE}documents");
    ...
    pub fn okf_index_envelope(terms: &[Term]) -> String {
...
pub fn render_all_with_languages(
    dataset: &RdfDataset,
    languages: &[String],
    modeled_defs: &BTreeSet<String>,
) -> Result<BTreeMap<String, Vec<u8>>, gmeow_errors::Diag> {
    ...
    let primer = gmeow_docs_model::gmn1_primer::build_primer(dataset)
...
/// Read a materialized fold from `generated/dist/gmeow.gts` under `root` as a native
/// `RdfDataset`.
///
/// The repo-rooted read side: a caller that already holds a checkout (the leaf unit
/// tests here, and `gmeow-pipeline`'s okf leaf tests) folds the committed file
/// directly. A pipeline stage instead reads THIS run's snapshot carrier
/// (`gmeow_pipeline::stages::export::read_fold_upstream`), because GTS is exit-only
/// and the carrier is the single internal transport.
pub fn read_fold(root: &std::path::Path) -> Result<std::sync::Arc<RdfDataset>, gmeow_errors::Diag> {

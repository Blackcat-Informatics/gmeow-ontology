...
mod reasoned_gates;
pub use reasoned_gates::REASONED_GATES_MEMBER;
...
/// tar of native reasoning reports, the DL/EL ledger and prepared verification laws.
...
pub struct Bundle {
    view: GtsFoldView,
    prepared_reasoned_gates: Mutex<Option<Arc<[u8]>>>,
}

impl Bundle {
    /// Read one required bounded native member from this retained snapshot.
    ///
    /// # Errors
    /// Rejects missing or ambiguous members, corrupt archives and oversized payloads.
    pub fn required_archive_member(
        &self,
        representation: &str,
        path: &str,
        max_bytes: usize,
    ...
    /// Authenticate and decode the unique required representation without
    /// materializing any of its archive members.
    pub fn required_blob(
        &self,
        representation: &str,
    ) -> gmeow_errors::Result<std::borrow::Cow<'_, [u8]>> {
        gmeow_gts_profile::archive::required_blob(self.view.graph(), representation)
    }
    ...
    /// ([`REP_REASONING`]): entailment explanations, the DL/EL cross-check ledger
    /// and the prepared verification laws over the bundle's reasoned closure.

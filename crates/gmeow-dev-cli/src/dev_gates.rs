...
/// Authenticate the standalone fuzz workspace's selected PurRDF substrate.
pub fn fuzz_substrate_check() -> i32 {
    match gmeow_validate::substrate::verify_fuzz_substrate(&project_root()) {
        Ok(()) => {
            println!("fuzz and production PurRDF resolutions agree");
            0
        }
...
#[path = "dev_gates.vendored_corpus_license_tests.rs"]
#[cfg(test)]

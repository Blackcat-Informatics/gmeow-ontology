...
fn parse_binding(view: &DslView, node: &DslTerm) -> Result<ProfileBinding, String> {
    ...
    if emit_sssom {
        if sssom_predicate.is_none() {
            return Err(
                "profile binding with gmeow:emitSssom true missing gmeow:sssomPredicate".to_owned(),
            );
        }
        if sssom_file.is_none() {
            return Err(
                "profile binding with gmeow:emitSssom true missing gmeow:sssomFile".to_owned(),
            );
        }
    }

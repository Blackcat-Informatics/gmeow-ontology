...
//! by no `gmeow:profileDescriptor`) currently emits at `sh:Warning`; a later task
//! migrates it to `sh:Violation`. This test is the executable witness for WHY that
//! migration bites: it drives a synthetic `ValidationResult` at the POST-migration
//! severity (`ShaclSeverity::Violation`) through the REAL lowering
//! `gmeow_validate::findings::diag_from_shacl` — the same production surface that
//! interns SHACL results into canonical [`Diag`]s — and shows the interned [`Grade`]
//! is the gate-fatal up-set corner `(Error, DataShapeViolation, Binding)`, so
...
//! inside `diag_from_shacl`. This is deterministic and passes NOW — it exercises the
//! morphism the Warning→Violation flip relies on, independent of the live generated
//! shape's current severity.

...
fn shape_value(ds: &RdfDataset, id: TermId, ctx: &str) -> gmeow_errors::Result<ShapeValue> {
    match ds.resolve(id) {
        ...
        TermRef::Literal {
            ...
            let datatype = obj_iri(ds, datatype, ctx)?;
            Ok(ShapeValue::Literal(purrdf::RdfLiteral {
                lexical_form: lexical.to_owned(),
                datatype: (language.is_none() && datatype != XSD_STRING).then_some(datatype),

...
//!
//! Serialises computed dependency edges into the `gmeow:graph/slice-analysis`
...
pub const ANALYSIS_GRAPH_IRI: &str = "https://blackcatinformatics.ca/gmeow/graph/slice-analysis";

const GMEOW_NS: &str = "https://blackcatinformatics.ca/gmeow/";

/// `gmeow:computedSliceDependency` — a blank node asserting a computed edge.
pub const COMPUTED_SLICE_DEPENDENCY: &str =
    "https://blackcatinformatics.ca/gmeow/computedSliceDependency";

/// `gmeow:dependencyStatus` — status literal ("matched"/"undeclared"/"stale"/"forbidden").
pub const DEPENDENCY_STATUS: &str = "https://blackcatinformatics.ca/gmeow/dependencyStatus";

/// `gmeow:dependencyEvidence` — human-readable evidence summary literal.
pub const DEPENDENCY_EVIDENCE: &str = "https://blackcatinformatics.ca/gmeow/dependencyEvidence";

/// `gmeow:computedProfileMembership` — profile membership assertion.
pub const COMPUTED_PROFILE_MEMBERSHIP: &str =
    "https://blackcatinformatics.ca/gmeow/computedProfileMembership";

/// `gmeow:termCoverage` — integer count of terms owned by a slice.
pub const TERM_COVERAGE: &str = "https://blackcatinformatics.ca/gmeow/termCoverage";
...
pub struct AnalysisGraph {
    ...
    /// Every triple is implicitly in `gmeow:graph/slice-analysis`.
...
/// Emit the computed dependency edges as the `gmeow:graph/slice-analysis`
/// named graph.
///
/// # Arguments
///
...
/// - `term_count_of` — maps a slice IRI to its term count (for `gmeow:termCoverage`).
///
/// # Errors
///
...
pub fn emit_analysis_graph(
    ...
    writeln!(
        body,
        "    <{GMEOW_NS}toolchainCompiler> {:?}^^xsd:string ;",
        toolchain.compiler_version
    )
    .unwrap();
    writeln!(
        body,
        "    <{GMEOW_NS}toolchainProfile> {:?}^^xsd:string .",
    ...
    for (i, edge) in edges.iter().enumerate() {
        ...
        writeln!(
            body,
            "    <{GMEOW_NS}dependencyFromSlice> <{}> ;",
            edge.from_slice
        )
        .unwrap();
        writeln!(
            body,
            "    <{GMEOW_NS}dependencyToSlice> <{}> ;",
            edge.to_slice
        )
        .unwrap();
        writeln!(
            body,
            "    <{GMEOW_NS}dependencyEdgeKind> {kind_str:?}^^xsd:string ;"
        )
        .unwrap();
        writeln!(
            body,
...
mod tests {
    ...
    fn tc() -> ToolchainContext {
        ToolchainContext::new("gmeow-logic v1", "el")
    }
    ...
    fn no_authored_overwrite() {
        ...
        let authored = "@prefix gmeow: <https://blackcatinformatics.ca/gmeow/> .\n\
                        gmeow:sliceA gmeow:sliceDependsOn gmeow:sliceB .\n";
    ...
    fn status_coverage_all_variants() {
        let edges = vec![
            ...
            make_edge(
                "https://blackcatinformatics.ca/gmeow/sliceD",
                "https://blackcatinformatics.ca/gmeow/sliceB",
                EdgeKind::Ontology,
    ...
    fn forbidden_status_emitted_for_tier_violations() {
        ...
        let edges = vec![make_edge(
            "https://blackcatinformatics.ca/gmeow/coreSlice",
            "https://blackcatinformatics.ca/gmeow/extSlice",
            EdgeKind::Ontology,
            ReconciliationStatus::Matched,
        )];
    ...
    fn self_attestation_guard_fires() {
        let poisoned = format!(
            "@prefix gmeow: <https://blackcatinformatics.ca/gmeow/> .\n\
             <{ANALYSIS_GRAPH_IRI}> a gmeow:SliceAnalysisGraph .\n"
        );

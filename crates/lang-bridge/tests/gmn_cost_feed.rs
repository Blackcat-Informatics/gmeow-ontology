...
/// `gmeow:gmnDictV2` through `GmnDictionary::from_dataset` (the same "compiled carrier"
...
fn codec_emits_the_authored_qualifier_marker_aliases_verbatim() {
    ...
    let ds = load_lang_module();
    let dict = GmnDictionary::from_dataset(&ds).expect("dict-v2 loads from the carrier");

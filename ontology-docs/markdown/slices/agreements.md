...
| [`gmeow:Contract`](../reference/classes/gmeow-Contract.md) | Contract | A legally enforceable agreement — an Agreement whose obligations are backed by a legal order, distinguished from a bare understanding only by that enforceabili... |
...
| [`gmeow:hasParty`](../reference/properties/gmeow-hasParty.md) | has party | Relates an agreement to an agent that is bound by it. NOT functional: an agreement binds two or more parties, and each is attached by a separate [`gmeow:hasParty`](../reference/properties/gmeow-hasParty.md)... |

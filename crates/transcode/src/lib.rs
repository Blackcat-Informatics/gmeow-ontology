...
/// own `purrdf.schema.json` header; the consumer's schema is gmeow's, so the YAML-LD
/// serializer is always handed this rather than `None`. Carried here verbatim from
/// the former `stages::yaml_ld` wrapper so this crate needs no pipeline dependency.
const GMEOW_BUNDLED_SCHEMA: &str = "gmeow.schema.json";
...
pub struct TranscodeOutput {
    ...
    pub bytes: Vec<u8>,
    /// Complete native ingestion evidence for a selected GTS target, absent otherwise.
    pub gts_ingestion: Option<purrdf::gts_compose::IngestReport>,
...
pub fn transcode(
    ...
    if to == Codec::Gts {
        ...
        let emission = gmeow_gts_profile::view_to_gmeow_gts(&dataset).map_err(|e| {
            gmeow_errors::Diag::of_kind(crate::CodecError {
                message: e.to_string(),
            })
        })?;
        ...
        return Ok(TranscodeOutput {
            ...
            gts_ingestion: Some(emission.ingestion),
            realized,
        });
    }

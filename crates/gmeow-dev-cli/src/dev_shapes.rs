...
fn constrains_to_literals(c: &ConstraintComponent) -> bool {
    is_facet(c)
        || matches!(
            c,
            ConstraintComponent::HasValue(ShapeValue::Literal(_))
        ...
        || matches!(c, ConstraintComponent::In(vals) if vals
            .iter()
            .any(|v| matches!(v, ShapeValue::Literal(_))))
}
...
fn reasoner_safe_emit(
    ...
    for pc in &ir.properties {
        ...
        // Fixed values preserve the complete native term in the hasValue antecedent.

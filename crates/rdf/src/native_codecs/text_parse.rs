...
//!
//! The gmeow-gts N-Quads/Turtle IRIREF readers took the raw bytes between `<` and
...
///
/// Line-oriented like the gmeow-gts parser: blank lines and `#`-comment lines are
...
impl<'a> TokenCursor<'a> {
    ...
    /// later by [`validate_statement`], exactly as the gmeow-gts parser does.
    ...
    /// admit). The gmeow-gts N-Quads parser requires the parenthesized form.
...
impl<'a> DocParser<'a> {
    ...
    fn term(&mut self, graph: Option<&Node>) -> Result<Node, RdfDiagnostic> {
        match self.peek() {
            ...
            // lexical form (kept verbatim, e.g. `-200.0`), matching gmeow-gts.
    ...
    fn literal(&mut self) -> Result<Node, RdfDiagnostic> {
        ...
        match self.peek() {
            Some(Token::LangTag(_)) => {
                ...
                // gmeow-gts's Turtle parser keeps the raw `@lang` text (including any
...
impl Interner {
    ...
    fn atom(&mut self, node: &Node) -> usize {
        ...
        // as gmeow-gts does, so the term table matches.

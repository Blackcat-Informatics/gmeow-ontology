...
mod tests {
    ...
    // above) was five hardcoded suffix strings plus two hand-written graphs — example-
    // based testing wearing the word "property". This module replaces it with a
    // GENERATOR (`gen_expr`) driven through `proptest`, exercised through the REAL
    ...
    //
    // ## On the module's name/path
    //
    // This crate's own plan named the check `physical::interning`. That literal path
    // is reachable ONLY from a `mod interning` declared directly under
    // `crates/logic/src/physical/mod.rs`; this hardening pass's editable surface is
    // restricted to `physical/lower.rs`'s own `mod tests`, so the nearest reachable
    // path from here is `physical::lower::tests::interning` — every test below is
    // still reachable by substring as `cargo nextest run -p gmeow-logic interning`
    // (or, for the exact sub-path, `cargo nextest run -p gmeow-logic
    // physical::lower::tests::interning`). Reaching the LITERAL `physical::interning`
    // path would require a one-line `mod` declaration in `physical/mod.rs`, a file
    // outside this pass's granted editable surface; that is reported here rather than
    ...
    mod interning {
            ...
            match expr {
                ...
                GenExpr::App(tag, args) => {
                    ...
                    let children: Vec<String> = args
                        .iter()
                        .map(|arg| {
                            render_node(arg, ttl, node_counter, bind_counter, env, free_decls, mint)
                        })
                        .collect();
                    let slot_iris: Vec<String> =
                        children.iter().map(|_| fresh(node_counter)).collect();
                    ttl.push_str(&format!(
        ...
        fn lower_rendered(rendered: &Rendered) -> String {
            ...
            let mut dag = TermDag::new();
            let node =
                lower_math_expression(&mut dag, &graph, &rendered.root).unwrap_or_else(|e| {
                    panic!(

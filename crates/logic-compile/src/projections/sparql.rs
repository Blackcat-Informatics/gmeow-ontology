...
fn templates_of(cell: &ProjectionCell, b: &ProfileBinding) -> Result<Vec<String>, String> {
    ...
    // Rewrite the value var to the retagged `?_final_*` IFF `language_retag` actually
    // emitted the BIND that defines it. Using the SAME decision (the bind_expr being
    // `Some`) is the single source of truth: it avoids rewriting the CONSTRUCT var to a
    // never-bound `?_final_*` (a dropped triple) when the retag preconditions failed.
    if let Some(val) = &p.value {
        if language_retag(p).1.is_some() {

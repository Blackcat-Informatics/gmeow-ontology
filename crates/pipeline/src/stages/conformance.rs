...
pub mod advice_wing;
pub(crate) mod cl_ingest;
pub mod contextual_results;
mod diagnostics;
pub(crate) mod documentation_graph;
pub(crate) mod epistemics;
pub(crate) mod execution;
mod expression_substrate;
pub mod flagship_unwired;
pub(crate) mod glossary;
mod gmn_consume;
mod gmn_dictionary;
pub(crate) mod gmn_grounding;
mod gmn_migration;
pub(crate) mod gmn_signatures;
pub(crate) mod gmn_vectors;
mod grounding_catalogs;
pub(crate) mod gufo_superset;
pub mod heavy;
pub(crate) mod inference_validation;
mod language_catalog;
mod logic_module_contracts;
pub(crate) mod math_lowering;
mod medium_axis;
pub(crate) mod native_cases;
pub(crate) mod native_scene;
mod operator_rules;
mod operator_scenes;
pub mod production_shapes;
pub(crate) mod registry_surface;
mod source_admission;
mod source_glyph_legend;
mod source_lint;
mod supplemental;
mod verify_gates;
pub(crate) mod wasm_gmn;
mod wellfounded_plan;
...
/// no `f64`).
///
/// `lane` is the `corpus.json` lane (`"a"`/`"b"`/`"divergence"`/`"native-profiled"`). The
...
/// counted as failures. A `native-profiled` corpus grades its semantic-operation members like Lane A.
/// Source-admission observations are counted independently and never compared as semantic verdicts, so
...
/// special-cases `"divergence"`, so `"native-profiled"` already falls into the
...
pub fn build_conformance_divergence(
    root: &Path,
    admissions: &native_cases::Admissions,
) -> Result<Vec<u8>, gmeow_errors::Diag> {
    full_conformance_nq(root, &grade_external_corpora(root, admissions)?)
}
...
pub fn grade_external_corpora(
    root: &Path,
    admissions: &native_cases::Admissions,
) -> Result<GradedCorpora, gmeow_errors::Diag> {
    let external = root.join(EXTERNAL_ROOT);
    let mut by_corpus: BTreeMap<String, Vec<ExternalComparison>> = BTreeMap::new();
    let (semantic, source_admissions) = grade_external_cases(root, &external, admissions)?;
    for graded in semantic {
        by_corpus
            .entry(graded.corpus)
            .or_default()
    ...
    Ok(GradedCorpora {
        comparisons: by_corpus,
        source_admissions,
    })
}

/// Complete selected-operation grading, kept native until its terminal projections.
pub struct GradedCorpora {
    comparisons: BTreeMap<String, Vec<ExternalComparison>>,
    source_admissions: Vec<source_admission::GradedAdmission>,
}
...
/// ([`divergence_nq_from_corpora`]), distinct native source-admission observations,
/// and the reified `gmeow:CapabilityGap` individuals ([`capability_gaps_nq`]). This is the SINGLE shared producer both
...
fn full_conformance_nq(root: &Path, graded: &GradedCorpora) -> Result<Vec<u8>, gmeow_errors::Diag> {
    let mut out = divergence_nq_from_corpora(&graded.comparisons);
    push_nq_block(
        &source_admission::emit(&graded.source_admissions)?,
        &mut out,
    );
...
/// Cases here count semantic comparisons only; source-admission records remain
/// distinct in graph/conformance and the exact authenticated operation inventory.
pub(crate) fn agreement_tallies_json(
    root: &Path,
    graded: &GradedCorpora,
) -> Result<Vec<u8>, gmeow_errors::Diag> {
    let by_corpus = &graded.comparisons;
    let external = root.join(EXTERNAL_ROOT);
...
fn grade_external_cases(
    root: &Path,
    external: &Path,
    admissions: &native_cases::Admissions,
) -> Result<(Vec<GradedCase>, Vec<source_admission::GradedAdmission>), gmeow_errors::Diag> {
    let mut graded: Vec<GradedCase> = Vec::new();
    let mut observed = Vec::new();
    let mut admission_inventory = std::collections::BTreeSet::new();
    if !external.is_dir() {
    ...
    for corpus_dir in sorted_dirs(external)? {
        ...
        for case_dir in sorted_dirs(&corpus_dir)? {
            ...
            let profile: serde_json::Value = serde_json::from_slice(
                &std::fs::read(&profile_path).map_err(|error| stage_err(&error.to_string()))?,
            )
            .map_err(|error| stage_err(&error.to_string()))?;
            let typed = gmeow_conformance::profile::parse_profile(&case, &profile)?;
            if typed.verdict_mode == gmeow_conformance::profile::VerdictMode::ClassSourceAdmission {
                let key = case_dir
                    .join("input.nq")
                    .strip_prefix(root)
                    .map_err(|error| stage_err(&error.to_string()))?
                    .to_string_lossy()
                    .into_owned();
                let observation = admissions
                    .get(&key)
                    .ok_or_else(|| {
                        stage_err(&format!(
                            "missing producer source-admission observation for {key}"
                        ))
                    })?
                    .as_ref()
                    .map_err(|error| stage_err(&error.to_string()))?;
                    ...
                    .and_then(serde_json::Value::as_str)
                {
                    if expected != observation.admission.observation_status() {
                        return Err(stage_err(
                            "source-admission observation violates its declared expected status",
                        ));
                    }
                }
                observed.push(source_admission::grade(
                    &corpus,
                    &case,
                    &case_dir,
                    published,
                ...
                admission_inventory.insert(key);
                continue;
            }
    ...
    if admission_inventory != admissions.keys().cloned().collect() {
        return Err(stage_err(
            "source-admission grading must consume its exact producer-selected inventory",
        ));
    }
    Ok((graded, observed))
}
...
/// Preserve an independently observed rejection as typed artifact data. Live
/// diagnostics cross this boundary once; cached records are never rehydrated as
/// operational errors or treated as execution certificates.
...
/// Raw external-case inputs whose edits invalidate the producer verdict.
fn external_case_input_files(root: &Path) -> Result<Vec<PathBuf>, gmeow_errors::Diag> {
...
/// [`crate::stages::carrier`] folds into `gmeow.gts`. It consumes the upstream
/// native source catalog for the shared shipped rule library; case execution is
/// producer-owned and its authenticated observations feed the golden consumer.
...
impl Stage for ConformanceStage {
    ...
    fn consumes(&self) -> &[String] {
        ...
        DEPS.get_or_init(|| vec![crate::stages::parse_sources::STAGE_ID.to_owned()])
    }
    ...
    fn impl_version(&self) -> &str {
        "conformance.v15-typed-source-admission-observations"
    }
    fn input_files(&self, root: &Path) -> Result<Vec<PathBuf>, gmeow_errors::Diag> {
        let mut files = external_case_input_files(root)?;
        files.extend(execution::input_files(root)?);
        files.extend(supplemental::input_files(root)?);
        files.extend(diagnostics::input_files(root));
        files.extend(glossary::input_files(root));
        files.extend(cl_ingest::input_files(root));
        files.extend(gmn_vectors::input_files(root)?);
        files.extend(gmn_grounding::input_files(root)?);
        files.extend(operator_scenes::input_files(root)?);
        files.extend(gmn_signatures::input_files(root));
        files.extend(native_scene::input_files(root));
        files.extend(verify_gates::input_files(root));
        files.extend(expression_substrate::input_files(root));
        files.extend(logic_module_contracts::input_files(root));
        files.extend(gufo_superset::input_files(root));
        files.extend(math_lowering::input_files(root)?);
        files.extend(wasm_gmn::input_files(root));
        files.extend(wellfounded_plan::input_files(root));
        files.extend(grounding_catalogs::input_files(root));
        files.extend(action_policy::input_files(root));
        files.extend(medium_axis::input_files(root));
        files.extend(source_lint::input_files(root));
        files.extend(crate::stages::superset::source_contracts::input_files(root));
        files.push(root.join(crate::stages::yaml_ld::source_observation::SOURCE));
        files.sort();
        files.dedup();
        Ok(files)
    }
    fn run(&self, input: StageInput<'_>) -> Result<StageOutput, gmeow_errors::Diag> {
        // Complete native observations before the single selected-operation grading
        // pass publishes semantic comparisons and source-admission evidence.
        let mut artifacts: BTreeMap<String, Vec<u8>> = BTreeMap::new();
        glossary::record(input.root, &mut artifacts)?;
        cl_ingest::record(input.root, &mut artifacts)?;
        epistemics::record(
            input.root,
            crate::stages::parse_sources::catalog(&input)?,
            &mut artifacts,
        )?;
        // Launch case execution before consumers that need the standalone logic
        // module. Execution overlaps that one shared compilation with its native
        // observation set; the consumers below then borrow the completed value.
        let admissions = execution::record(
            input.root,
            crate::stages::parse_sources::catalog(&input)?,
            &mut artifacts,
        )?;
        let prepared_operators = operator_rules::record(
            crate::stages::parse_sources::catalog(&input)?,
            &mut artifacts,
        )?;
        registry_surface::record(
            crate::stages::parse_sources::catalog(&input)?
                .document(crate::stages::compile_logic::SOURCE_PATH)?,
            &mut artifacts,
        )?;
        let graded = grade_external_corpora(input.root, &admissions)?;
        let nq = full_conformance_nq(input.root, &graded)?;
        let tallies = agreement_tallies_json(input.root, &graded)?;
        ...
        supplemental::record(input.root, &mut artifacts)?;
        diagnostics::record(
            input.root,
            crate::stages::parse_sources::catalog(&input)?,
            &mut artifacts,
        )?;
        language_catalog::record(
            crate::stages::parse_sources::catalog(&input)?,
            &mut artifacts,
        )?;
        documentation_graph::record(
            crate::stages::parse_sources::catalog(&input)?,
            &mut artifacts,
        )?;
        source_glyph_legend::record(
            crate::stages::parse_sources::catalog(&input)?,
            &mut artifacts,
        )?;
        crate::medium::source_observation::record(
            crate::stages::parse_sources::catalog(&input)?,
            &mut artifacts,
        )?;
        native_scene::record(input.root, &mut artifacts)?;
        verify_gates::record(
            input.root,
            crate::stages::parse_sources::catalog(&input)?,
            &mut artifacts,
        )?;
        expression_substrate::record(
            input.root,
            crate::stages::parse_sources::catalog(&input)?,
            &mut artifacts,
        )?;
        logic_module_contracts::record(
            input.root,
            crate::stages::parse_sources::catalog(&input)?,
            &mut artifacts,
        )?;
        gufo_superset::record(
            input.root,
            crate::stages::parse_sources::catalog(&input)?,
            &mut artifacts,
        )?;
        let lint_math = math_lowering::record(input.root, &mut artifacts)?;
        source_lint::record(input.root, lint_math, &mut artifacts)?;
        wasm_gmn::record(
            input.root,
            crate::stages::parse_sources::catalog(&input)?,
            &mut artifacts,
        )?;
        wellfounded_plan::record(
            input.root,
            crate::stages::parse_sources::catalog(&input)?,
            &mut artifacts,
        )?;
        grounding_catalogs::record(
            input.root,
            crate::stages::parse_sources::catalog(&input)?,
            &mut artifacts,
        )?;
        action_policy::record(input.root, &mut artifacts)?;
        medium_axis::record(
            input.root,
            crate::stages::parse_sources::catalog(&input)?,
            &mut artifacts,
        )?;
        crate::stages::superset::source_contracts::record(
            crate::stages::parse_sources::catalog(&input)?,
            &mut artifacts,
        )?;
        crate::stages::yaml_ld::source_observation::record(input.root, &mut artifacts)?;
        let gmn_started = std::time::Instant::now();
        gmn_dictionary::record(
            crate::stages::parse_sources::catalog(&input)?,
            &mut artifacts,
        )?;
        gmn_signatures::record(
            input.root,
            crate::stages::parse_sources::catalog(&input)?,
            &mut artifacts,
        )?;
        let grounding_operators = gmn_grounding::record(
            input.root,
            crate::stages::parse_sources::catalog(&input)?,
            &prepared_operators,
            &mut artifacts,
        )?;
        operator_scenes::record(
            input.root,
            &prepared_operators,
            grounding_operators,
            &mut artifacts,
        )?;
        let gmn_timing = crate::node::StageRunTiming {
            phase: "gmn-grounding-observations".to_owned(),
            elapsed_ms: gmn_started.elapsed().as_millis(),
            metadata: Some(format!(
                "observation_bytes={}",
                artifacts[gmn_grounding::CHANNEL].len() + artifacts[gmn_dictionary::CHANNEL].len()
            )),
        };
        let mut output = StageOutput::new(StageProduct::from_artifacts_over(
            self.id(),
            dataset,
            artifacts,
        ));
        output.timings.push(gmn_timing);
        Ok(output)
    }
}
...
#[cfg(test)]
#[path = "conformance_test_support.rs"]
mod test_support;
#[cfg(test)]
pub(crate) use test_support::diagnostic_observations;

...
/// Parse the small, repo-owned named-ternary Datalog fixture language used only by
/// the deterministic benchmark corpus. Production reasoning accepts typed rules.
pub(crate) fn parse_benchmark_rules(source: &str) -> gmeow_errors::Result<Vec<EvalRule>> {
    ...
    for line in source.lines() {
        ...
        let (head_text, body_text) = text
            .split_once(":-")
            .ok_or_else(|| ir_err(format!("benchmark rule lacks ':-': {text}")))?;
        let mut head = parse_benchmark_atom(head_text.trim(), false)?;
        head.negated = false;
        let mut body = Vec::new();
        for atom in split_benchmark_atoms(body_text) {
            let negated = atom.trim_start().starts_with('~');
            body.push(parse_benchmark_atom(atom.trim(), negated)?);
        }
        rules.push(EvalRule {
            head,
            body,
    ...
    if !statement.trim().is_empty() {
        return Err(ir_err(format!("unterminated benchmark rule: {statement}")));
    }
    Ok(rules)
}

fn split_benchmark_atoms(body: &str) -> Vec<&str> {
    let mut out = Vec::new();
    let mut depth = 0u32;
...
fn parse_benchmark_atom(text: &str, negated: bool) -> gmeow_errors::Result<EvalAtom> {
    let text = text.trim().strip_prefix('~').unwrap_or(text.trim());
    let open = text
        .find('(')
        .ok_or_else(|| ir_err(format!("invalid benchmark atom: {text}")))?;
    let close = text
        .rfind(')')
        .ok_or_else(|| ir_err(format!("invalid benchmark atom: {text}")))?;
    let predicate = strip_angle(text[..open].trim()).to_owned();
    ...
    Ok(EvalAtom {
        subject: parse_benchmark_term(args[0])?,
        predicate,
        object: parse_benchmark_term(args[1])?,
        negated,
    })
}

fn parse_benchmark_term(text: &str) -> gmeow_errors::Result<EvalTerm> {
    if text.starts_with('?') {
        return Ok(EvalTerm::Var(text.to_owned()));
    }
    if text.starts_with('!') {
        return Err(ir_err(format!(
            "existential variable {text} is not valid in a Datalog benchmark rule"
        )));
    }
    if let Some(iri) = text
        .strip_prefix('<')
        .and_then(|value| value.strip_suffix('>'))
    {
    ...
    if text.starts_with('"') {
        return Ok(EvalTerm::ConstLit(crate::term_codec::decode_term(text)?));
    }
    Err(ir_err(format!("unsupported benchmark term: {text}")))
}

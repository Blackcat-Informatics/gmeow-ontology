...
//! make this test depend on an unrelated `make regenerate` pass): it carries
...
//! `generated/dist/gmeow.gts` via `make regenerate`, which this test must not
//! depend on).
...
/// test must stay independent of `make regenerate`).

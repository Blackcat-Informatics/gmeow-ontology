...
//! setup `S` vs the per-twin whole-graph SHACL scan, using `gmeow-cost-measure`'s
//! `CountingAllocator` (total bytes + alloc count + peak-live, each deterministic as
//! a per-region delta). The decision this feeds (issue's Option 1 amortization vs
//! off-gate): a disk cache can only amortize `S`, never `V_ontology_only`, so if the
//! scan dominates, amortization is futile and off-gate is correct.
//!

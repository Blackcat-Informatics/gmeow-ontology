...
//! ~0.05 s. They live in their own binary, apart from the sibling
//! `conformance_ai_claims` (which carries the single `with_ontology()` whole-graph
//! SHACL case that rides the H8 budget cliff and is therefore carved out of the
//! per-commit profile in `.config/nextest.toml`). Splitting them keeps these cheap

...
| `cc2-capability-present` | the five frontier labels, the ordered walk of the method's `rdf:List` into its steps, the task's expansion and reachability closure, the approval obstruction on the one gated step, and the whole PIN — its type, its frozen sequence, its digest, its authority and the episode's selection are all derived from a roster candidate plus the `logic:AuthorizationProof` that licenses it. Policy-maximal automation is a consequence of the axis tuples here, not a claim about them; and the pin is a consequence of the authorization, not a record somebody typed. |

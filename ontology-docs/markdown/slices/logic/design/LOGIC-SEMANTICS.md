...
> Status: formal-semantics specification for [`logic:`](https://blackcatinformatics.ca/logic/). This is the **semantics** member of the
...
[`logic:`](https://blackcatinformatics.ca/logic/) is one coherent model in which the following coexist rather than compete.
...
[`logic:`](https://blackcatinformatics.ca/logic/) resolves to `https://blackcatinformatics.ca/logic/`, declared once in the unified prefix
...
every metadata triple hung off the reifier (`dsl/statements/vocabulary.ttl:45-54`). [`logic:`](https://blackcatinformatics.ca/logic/) lifts
that same form from statements-about-facts to axioms-and-rules-about-anything.

### RDF 1.2 triple terms, precisely

[`logic:`](https://blackcatinformatics.ca/logic/) targets **normative RDF 1.2**, and uses the term *RDF 1.2 triple terms* consistently —
...
> [`logic:`](https://blackcatinformatics.ca/logic/) MUST restrict canonical RDF output to normative **RDF 1.2 Full**. Any symmetric or
...
> world-indexed truth are all mediated by explicit [`logic:`](https://blackcatinformatics.ca/logic/) predicates.
...
"Sound" is ambiguous until you answer "sound relative to which semantics?" Because [`logic:`](https://blackcatinformatics.ca/logic/) admits
...
|---|---|---|
| [`logic:PositiveHornProfile`](https://blackcatinformatics.ca/logic/PositiveHornProfile) | monotonic Horn rules | least-model; always terminating for the function-free fragment |
| [`logic:StratifiedNAFProfile`](https://blackcatinformatics.ca/logic/StratifiedNAFProfile) | negation-as-failure with stratification | unique perfect model; PTIME data complexity |
| [`logic:WellFoundedProfile`](https://blackcatinformatics.ca/logic/WellFoundedProfile) | well-founded semantics | three-valued; total, polynomial |
| [`logic:StableModelProfile`](https://blackcatinformatics.ca/logic/StableModelProfile) | answer-set / stable-model semantics | possibly multiple models; NP-hard |
| [`logic:ProceduralPrologProfile`](https://blackcatinformatics.ca/logic/ProceduralPrologProfile) | SLD, cut, builtins | operational, **not purely declarative** |
| [`logic:ProbabilisticProfile`](https://blackcatinformatics.ca/logic/ProbabilisticProfile) | probabilistic inference over [`logic:probability`](https://blackcatinformatics.ca/logic/probability) | requires an independence or dependency model |
...
> The canonical rule semantics is **cut-free**. [`logic:cut`](https://blackcatinformatics.ca/logic/cut) is permitted **only** in the
...
arbitrary confidence metadata as if it were a probability model. [`logic:`](https://blackcatinformatics.ca/logic/) keeps four distinct
predicates:

| Predicate | Meaning |
|---|---|
| [`logic:probability`](https://blackcatinformatics.ca/logic/probability) | probabilistic fact/event semantics; requires an independence or dependency model |
| [`logic:confidence`](https://blackcatinformatics.ca/logic/confidence) | epistemic confidence assigned by a source or process |
| [`logic:weight`](https://blackcatinformatics.ca/logic/weight) | a solver/ranking weight, not necessarily probabilistic |
| [`logic:evidenceStrength`](https://blackcatinformatics.ca/logic/evidenceStrength) | evidential support, provenance-derived (reuses the evidence-slice warrant axes) |

The governing rules:

> Probabilistic inference is available **only** in [`logic:ProbabilisticProfile`](https://blackcatinformatics.ca/logic/ProbabilisticProfile). A [`logic:confidence`](https://blackcatinformatics.ca/logic/confidence)
> annotation MUST NOT be interpreted as a probability unless an explicit mapping to
> [`logic:probability`](https://blackcatinformatics.ca/logic/probability) is declared.
...
Probabilistic inference under [`logic:ProbabilisticProfile`](https://blackcatinformatics.ca/logic/ProbabilisticProfile) computes **exact marginals by weighted
...
- under **[`logic:FullIndependence`](https://blackcatinformatics.ca/logic/FullIndependence)**: `P(θ) = ∏_{f true in θ} p_f · ∏_{f false in θ} (1 − p_f)`;
- under a **[`logic:DependencyModel`](https://blackcatinformatics.ca/logic/DependencyModel)**: a declared explicit joint table over a correlated fact set
 replaces the product for those facts (each [`logic:JointOutcome`](https://blackcatinformatics.ca/logic/JointOutcome) carries its [`logic:jointProbability`](https://blackcatinformatics.ca/logic/jointProbability),
...
> A declared [`logic:probabilityModel`](https://blackcatinformatics.ca/logic/probabilityModel) is **required**. A [`logic:ProbabilisticProfile`](https://blackcatinformatics.ca/logic/ProbabilisticProfile) query with
> probabilistic facts but **no** declared model returns status `unknown` — it never silently assumes
> independence. A [`logic:confidence`](https://blackcatinformatics.ca/logic/confidence) (or [`logic:weight`](https://blackcatinformatics.ca/logic/weight), or [`logic:evidenceStrength`](https://blackcatinformatics.ca/logic/evidenceStrength)) annotation is
...
the surface for [`logic:counterfactualOf`](https://blackcatinformatics.ca/logic/counterfactualOf)): `:- probability_model(full_independence | dependency).`,
...
**Turing-completeness is a design goal, not a side effect.** [`logic:`](https://blackcatinformatics.ca/logic/) is meant to compute, not
...
[`logic:`](https://blackcatinformatics.ca/logic/) manages it the way the project manages every hard constraint: by making termination a
...
the budget and the fragment. Because [`logic:`](https://blackcatinformatics.ca/logic/) is open-world, paraconsistent, and provenance-carrying,
...
face of "decidable but intractable." [`logic:`](https://blackcatinformatics.ca/logic/) does not hide that cost behind a silent timeout; it
makes the decidability class, the fragment certification, and the budget boundary explicit,
machine-readable, and tested.

## The [`logic:`](https://blackcatinformatics.ca/logic/) Foundation (UFO⁺)
...
it embodies the very restraint [`logic:`](https://blackcatinformatics.ca/logic/) rejects. The foundational theory — **UFO⁺** — is authored
canonically in the [`logic:`](https://blackcatinformatics.ca/logic/) namespace; [gUFO](../../../external/ontologies.md#target-gufo) becomes a generated down-projection (the rollout and the
gUFO/BFO/DOLCE distinction are in
[LOGIC-MIGRATION.md](LOGIC-MIGRATION.md#foundation-projection-and-discipline)).

The foundational categories are [`logic:`](https://blackcatinformatics.ca/logic/) terms in the one namespace: [`logic:Kind`](https://blackcatinformatics.ca/logic/Kind), [`logic:SubKind`](https://blackcatinformatics.ca/logic/SubKind),
[`logic:Phase`](https://blackcatinformatics.ca/logic/Phase), [`logic:Role`](https://blackcatinformatics.ca/logic/Role), [`logic:Category`](https://blackcatinformatics.ca/logic/Category), [`logic:Mixin`](https://blackcatinformatics.ca/logic/Mixin), [`logic:RoleMixin`](https://blackcatinformatics.ca/logic/RoleMixin), [`logic:PhaseMixin`](https://blackcatinformatics.ca/logic/PhaseMixin),
[`logic:Relator`](https://blackcatinformatics.ca/logic/Relator), [`logic:Event`](https://blackcatinformatics.ca/logic/Event), [`logic:Situation`](https://blackcatinformatics.ca/logic/Situation), and relations such as [`logic:rigidlyAppliesTo`](https://blackcatinformatics.ca/logic/rigidlyAppliesTo),
[`logic:suppliesIdentity`](https://blackcatinformatics.ca/logic/suppliesIdentity), and [`logic:mediates`](https://blackcatinformatics.ca/logic/mediates). They obey the same naming discipline as the rest of
...
`relator_mediation` (**RelComp**, `:218`). In [`logic:`](https://blackcatinformatics.ca/logic/) these become **actual axioms**. The discipline
...
grouping, not modal or second-order semantics; so [`logic:rigidlyAppliesTo`](https://blackcatinformatics.ca/logic/rigidlyAppliesTo) and
[`logic:suppliesIdentity`](https://blackcatinformatics.ca/logic/suppliesIdentity) are each given an explicit interpretation and **lowered** (compiled) onto
...
- **Rigidity.** [`logic:rigidlyAppliesTo(T)`](https://blackcatinformatics.ca/logic/rigidlyAppliesTo(T)) lowers to the integrity constraint *for every individual
...
over types" becomes first-order quantification over those individuals: [`logic:suppliesIdentity(K, x)`](https://blackcatinformatics.ca/logic/suppliesIdentity(K, x))
...
`gmeow_logic.foundation(input_nq, anti_rigidity_policy)`): it derives [`logic:violation`](https://blackcatinformatics.ca/logic/violation) facts for
[`logic:StereotypeCardinality`](https://blackcatinformatics.ca/logic/StereotypeCardinality), [`logic:MixIden`](https://blackcatinformatics.ca/logic/MixIden), [`logic:FreeRole`](https://blackcatinformatics.ca/logic/FreeRole), [`logic:MixRig`](https://blackcatinformatics.ca/logic/MixRig), and [`logic:RelComp`](https://blackcatinformatics.ca/logic/RelComp)
under [`logic:StratifiedNAFProfile`](https://blackcatinformatics.ca/logic/StratifiedNAFProfile)-certified stratified NAF, with absence expressed via
...
emitting [`logic:rigidityViolation`](https://blackcatinformatics.ca/logic/rigidityViolation) quads in the failing world. The runner
...
[`logic:dischargeObligation`](https://blackcatinformatics.ca/logic/dischargeObligation)), `"schema-only"` (emits nothing at the instance level), or
`"witness-required"` (emits [`logic:witnessRequiredViolation`](https://blackcatinformatics.ca/logic/witnessRequiredViolation) absent a materialized counter-world). The
policy governs **only** the obligation/witness facet; [`logic:violation`](https://blackcatinformatics.ca/logic/violation) ([`Task`](../../../reference/classes/gmeow-Task.md) 2) and
[`logic:rigidityViolation`](https://blackcatinformatics.ca/logic/rigidityViolation) ([`Task`](../../../reference/classes/gmeow-Task.md) 3) are computed by separate passes and are identical across all three
...
A *world* in [`logic:`](https://blackcatinformatics.ca/logic/) is a **reified, named context**, the kind of object the ontology already
...
□/◇. [`logic:`](https://blackcatinformatics.ca/logic/) promotes this to a [`logic:World`](https://blackcatinformatics.ca/logic/World) with a typed [`logic:accessibleFrom`](https://blackcatinformatics.ca/logic/accessibleFrom) generalizing
...
**derived [`logic:`](https://blackcatinformatics.ca/logic/) frame** whose contents are *computed by a revision rule*, not asserted; the
...
| Counterfactual (hypothesized) | [`logic:`](https://blackcatinformatics.ca/logic/) (new) | constructed by revision; closeness poset | `counterfactualOf` (new) |
...
[`logic:`](https://blackcatinformatics.ca/logic/) does not defuse this by computing the permutations. It takes the design's own clause
...
- **Ties are broken by declared type-level priority** — an entrenchment ordering for which [`logic:`](https://blackcatinformatics.ca/logic/)

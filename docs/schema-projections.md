...
| **Pydantic** | `gmeow_models/` — a FLAT single-module package (`_base.py`, `models.py`, `__init__.py`, `py.typed`, `__about__.py`) | A user-approved capability regression from the former slice-routed multi-module package: no per-slice modules, no per-class docs-digest linkage, no generated `README.md`. Recorded in `.deficiencies` (2026-07-17 entry) with an upstream `purrdf` feature request as the forward path. `gmeow_models/__about__.py` (the wheel version, from the ontology's `owl:versionInfo`) remains gmeow-orchestrated, stamped alongside purrdf's artifacts. |

"""Phase-0 benchmark for the purrdf test-speed work (#242).
...
def main() -> None:
    ...
    _time("(b) merged_store (purrdf, cached)", lambda: sparql.merged_store())
    _time("(c) store_with (fresh purrdf store)", lambda: sparql.store_with())

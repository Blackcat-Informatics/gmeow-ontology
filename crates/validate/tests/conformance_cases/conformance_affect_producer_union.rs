...
//! maint-heavy cost-class contract group.
//!
...
//! `conformance_{finance,agentic,ai_claims,music_analysis,math_producers}` groups
...
//! amortise the ~2% setup, never the per-twin scan. This exhaustive group is
//! therefore dispatched only by the consolidated runner's `maint-heavy` libtest,
//! exactly like its cost-class

...
# Worked example (risk design, the named P15 consumer's flagship case): the
...
| [`gmeow:Mitigation`](../reference/classes/gmeow-Mitigation.md) | Mitigation | A reified countermeasure binding: a measure — a [`gmeow:Norm`](../reference/classes/gmeow-Norm.md) or a Procedure (procedures extension) — set against a causal link or hazard it counters. The measure... |
...
| [`gmeow:cascadeSeverity`](../reference/properties/gmeow-cascadeSeverity.md) | cascade severity | The cascade's graded severity — according to whoever grades it (statement-layer indexed). NOT functional design review: grading is evaluative and source-variab... |
...
| [`gmeow:hazardSeverity`](../reference/properties/gmeow-hazardSeverity.md) | hazard severity | The hazard's graded severity, when graded standalone (a hazard feeding a cascade usually inherits attention from the cascade's grade). NOT functional (design r... |
...
| [`gmeow:linkMechanism`](../reference/properties/gmeow-linkMechanism.md) | link mechanism | How the causation works — prose. NOT functional and range-open (localizable prose, the localizable-prose convention lesson): one mechanism account per language... |
...
| [`gmeow:mitigationStatus`](../reference/properties/gmeow-mitigationStatus.md) | mitigation status | The mitigation's lifecycle status. NOT functional design review: lifecycle is mutable and source-variable — status-over-time rides validFrom/validUntil on the... |
...
| [`gmeow:typePrevents`](../reference/properties/gmeow-typePrevents.md) | type prevents | Flat type-level prevention: events of the subject kind block events of the object kind. NOT transitive. Grounds [sumo:prevents](https://www.ontologyportal.org/SUMO.owl#prevents) at alignment time; the D3FEND cou... |
...
### [`gmeow:Hazard`](../reference/classes/gmeow-Hazard.md) · [`gmeow:hazardBearer`](../reference/properties/gmeow-hazardBearer.md) · [`gmeow:manifestedAsType`](../reference/properties/gmeow-manifestedAsType.md) · [`gmeow:hazardSeverity`](../reference/properties/gmeow-hazardSeverity.md)

A bearer's disposition toward harm — the first [`gufo:Disposition`](../external/terms.md#gufo-disposition) use in GMEOW, and
the reason this slice needs no counterfactual machinery: a hazard that never
manifests is still fully real. [`hazardBearer`](../reference/properties/gmeow-hazardBearer.md) (functional, mandatory) names the one
entity it inheres in; [`manifestedAsType`](../reference/properties/gmeow-manifestedAsType.md) (non-functional, at least one) the feared
[`EventType`](../reference/classes/gmeow-EventType.md)(s), never an [`Event`](../reference/classes/gmeow-Event.md) — the type level is where counterfactuals live
without machinery. [`hazardSeverity`](../reference/properties/gmeow-hazardSeverity.md) grades a standalone hazard (optional).

### [`gmeow:typeCauses`](../reference/properties/gmeow-typeCauses.md) · [`gmeow:typeEnables`](../reference/properties/gmeow-typeEnables.md) · [`gmeow:typePrevents`](../reference/properties/gmeow-typePrevents.md) · [`gmeow:typeMitigates`](../reference/properties/gmeow-typeMitigates.md)
...
(P12). Each is statement-layer indexed; [`typeCauses`](../reference/properties/gmeow-typeCauses.md) [`pairsWith`](../reference/properties/gmeow-pairsWith.md) [`CausalLink`](../reference/classes/gmeow-CausalLink.md), the
reified form to promote to when modality, mechanism, or strength rides on the claim.

### [`gmeow:CausalLink`](../reference/classes/gmeow-CausalLink.md) · [`gmeow:linkAntecedent`](../reference/properties/gmeow-linkAntecedent.md) · [`gmeow:linkConsequent`](../reference/properties/gmeow-linkConsequent.md) · [`gmeow:linkMechanism`](../reference/properties/gmeow-linkMechanism.md) · [`gmeow:linkStrength`](../reference/properties/gmeow-linkStrength.md)

The reified type-level causal claim — antecedent kind × consequent kind × modality,
with optional mechanism prose and solver-input strength. [`linkAntecedent`](../reference/properties/gmeow-linkAntecedent.md) and
[`linkConsequent`](../reference/properties/gmeow-linkConsequent.md) (both functional, mandatory, and distinct — nothing type-causes
itself) carry the kinds; [`linkMechanism`](../reference/properties/gmeow-linkMechanism.md) the localizable how-prose; [`linkStrength`](../reference/properties/gmeow-linkStrength.md) a
solver-INPUT weight, never an output written back as assertion (P12).

### [`gmeow:CausalModality`](../reference/classes/gmeow-CausalModality.md) · [`gmeow:causalModality`](../reference/properties/gmeow-causalModality.md)

The force of a causal link — an OPEN vocabulary seeded with necessitates, promotes,
enables, prevents. Distinct from doxastic [`standpointModality`](../reference/properties/gmeow-standpointModality.md) and ontic
[`hasDeterminacy`](../reference/properties/gmeow-hasDeterminacy.md): causal modality is what the link CLAIMS about the world's
mechanics. [`causalModality`](../reference/properties/gmeow-causalModality.md) is functional and mandatory: if you reified, the modality
is your reason.

### [`gmeow:Cascade`](../reference/classes/gmeow-Cascade.md) · [`gmeow:cascadeFirstLink`](../reference/properties/gmeow-cascadeFirstLink.md) · [`gmeow:linkNext`](../reference/properties/gmeow-linkNext.md) · [`gmeow:cascadeSeverity`](../reference/properties/gmeow-cascadeSeverity.md)

A NAMED chain of causal links — a [`SocialObject`](../reference/classes/gmeow-SocialObject.md) failure narrative that exists
independent of anything occurring. Entered at [`cascadeFirstLink`](../reference/properties/gmeow-cascadeFirstLink.md) (functional,
mandatory), walked by [`linkNext`](../reference/properties/gmeow-linkNext.md) (non-functional → branching failure trees,
SHACL-acyclic, not transitive), graded by mandatory [`cascadeSeverity`](../reference/properties/gmeow-cascadeSeverity.md) — an ungraded
cascade is just a story.

### [`gmeow:SeverityLevel`](../reference/classes/gmeow-SeverityLevel.md) · [`gmeow:moreSevereThan`](../reference/properties/gmeow-moreSevereThan.md)

An OPEN, ordered severity vocabulary (catastrophic ≻ severe ≻ moderate ≻ minor; the
kernel [`GranularityLevel`](../reference/classes/gmeow-GranularityLevel.md) pattern, fourth use) anchored terminologically to ISO 31000 /
IEC 60812 in docs, never axioms. [`moreSevereThan`](../reference/properties/gmeow-moreSevereThan.md) is transitive ON LEVELS ONLY —
cascades themselves are never ordered transitively.

### [`gmeow:RiskFactor`](../reference/classes/gmeow-RiskFactor.md) · [`gmeow:Mitigation`](../reference/classes/gmeow-Mitigation.md) · [`gmeow:mitigationMeasure`](../reference/properties/gmeow-mitigationMeasure.md) · [`gmeow:mitigationCounters`](../reference/properties/gmeow-mitigationCounters.md)

[`RiskFactor`](../reference/classes/gmeow-RiskFactor.md) is the named umbrella over [`CausalLink`](../reference/classes/gmeow-CausalLink.md) + [`Hazard`](../reference/classes/gmeow-Hazard.md) (a generator-visible range,
never instantiated directly). [`Mitigation`](../reference/classes/gmeow-Mitigation.md) is a reified countermeasure binding: a
measure set against a [`RiskFactor`](../reference/classes/gmeow-RiskFactor.md) it counters. [`mitigationMeasure`](../reference/properties/gmeow-mitigationMeasure.md) has an intentionally
OPEN range (the [`tenurePosition`](../reference/properties/gmeow-tenurePosition.md) precedent) so a [`gmeow:Norm`](../reference/classes/gmeow-Norm.md) or a [`Procedure`](../reference/classes/gmeow-Procedure.md) plugs in
without extension→extension coupling (P16); [`mitigationCounters`](../reference/properties/gmeow-mitigationCounters.md) ranges over [`RiskFactor`](../reference/classes/gmeow-RiskFactor.md)
(a bowtie barrier or a source control).

### [`gmeow:MitigationStatus`](../reference/classes/gmeow-MitigationStatus.md) · [`gmeow:mitigationStatus`](../reference/properties/gmeow-mitigationStatus.md)
...
deleted, P10). [`mitigationStatus`](../reference/properties/gmeow-mitigationStatus.md) is non-functional: status-over-time rides
[`validFrom`](../reference/properties/gmeow-validFrom.md)/[`validUntil`](../reference/properties/gmeow-validUntil.md) on the statement, single-valuedness per base graph is SHACL's
job.

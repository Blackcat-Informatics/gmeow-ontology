...
//!
//! This module is the canonicalization authority for the gmeow-rdf family. It
...
//!
//! RDFC-1.0 is specified over triples/quads. gmeow's IR additionally carries a
...
//! `urn:gmeow:rdfc:` sentinel IRIs that no real dataset IRI can occupy:
//!
//! - reifier `(r, t)` → `r <urn:gmeow:rdfc:reifies> t .` (`t` is the triple term)
//! - annotation `(r, p, o)` → `r p o <urn:gmeow:rdfc:annotation> .`
//!
...
const SENTINEL_REIFIES: &str = "urn:gmeow:rdfc:reifies";
/// Sentinel graph for an annotation row in the canonical form (§ overlay).
const SENTINEL_ANNOTATION_GRAPH: &str = "urn:gmeow:rdfc:annotation";
...
pub struct Canonicalized {
    ...
    /// reserved `urn:gmeow:rdfc:` sentinels).
    pub nquads: String,
...
enum Component {
    ...
    /// A reifier binding `r <urn:gmeow:rdfc:reifies> t`.
...
mod tests {
    ...
    fn reifier_count_shows_in_canon() {
        ...
        assert!(
            one.contains("<urn:gmeow:rdfc:reifies> <<( "),
    ...
    fn annotation_shows_in_canon() {
        ...
        assert!(
            with.contains("<urn:gmeow:rdfc:annotation> ."),

...
`make check` (the local native reasoning lane) and CI such that a regression below its floor
fails the build.
...
(`lint validate … coverage acceptance …`), so the local native reasoning lane blocks on every
floor too.

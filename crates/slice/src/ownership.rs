...
impl EdgeKind {
    /// Whether this edge kind reconciles against `gmeow:sliceDependsOn`
...
/// How a computed edge reconciles with the authored `gmeow:sliceDependsOn`.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum ReconciliationStatus {
    /// A semantic edge that *is* declared in `gmeow:sliceDependsOn`.
    Matched,
    /// A semantic edge that is *not* declared in `gmeow:sliceDependsOn`.
    Undeclared,
...
pub enum OwnershipDiagnostic {
    ...
    /// A semantic edge has no authored `gmeow:sliceDependsOn` declaration.
    UndeclaredDependency {
        from_slice: SliceIri,
        to_slice: SliceIri,
        edge_kind: EdgeKind,
    },
    /// A `gmeow:sliceDependsOn` declaration has no semantic evidence.
    StaleDependency {
        from_slice: SliceIri,
        to_slice: SliceIri,
    },
...
impl<'a> OwnershipAnalyzer<'a> {
    ...
    pub fn analyze(&self) -> Result<OwnershipReport, SliceError> {
        ...
        // ── Phase 4: authored declarations (gmeow:sliceDependsOn) ───────────

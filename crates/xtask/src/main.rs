...
/// Drive the deterministic `gmeow:TermEquivalence` align\* → native RDF-1.2 `skos:*Match`
/// migration over an explicit list of `.ttl` files and/or a `--root` directory it walks
/// for `mappings/*.ttl` cell files. Each file is gated by the engine's per-file
/// refuse-to-write self-check (field round-trip + SSSOM byte-identity); a file that fails
/// is ABORTED (never written) and the run exits non-zero. `--dry-run` runs every gate but
/// writes nothing.
fn run_migrate_skos_match(mut args: impl Iterator<Item = String>) -> ExitCode {
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            "--root" => {
                let Some(value) = args.next() else {
                    eprintln!("xtask migrate-skos-match: --root requires a directory");
                    return ExitCode::from(2);
                };
                roots.push(PathBuf::from(value));
            }
            other if other.starts_with('-') => {
                eprintln!("xtask migrate-skos-match: unknown argument {other:?}");
                return ExitCode::from(2);
            }
    ...
    for root in &roots {
        match discover_cell_files(root) {
            Ok(found) => files.extend(found),
            Err(error) => {
                eprintln!("xtask migrate-skos-match: walk {}: {error}", root.display());
                return ExitCode::FAILURE;
            }
        }
    }
    ...
    if files.is_empty() {
        eprintln!(
            "xtask migrate-skos-match: no input files (pass FILE.ttl paths and/or --root DIR)"
        );
        return ExitCode::from(2);
    }
    ...
    for path in &files {
        let source = match std::fs::read_to_string(path) {
            Ok(text) => text,
            Err(error) => {
                eprintln!("ABORT {}: read: {error}", path.display());
                aborted += 1;
                continue;
            }
        };
        match gmeow_logic_compile::migrate_skos_match::migrate_turtle_source(&source) {
            Ok(result) if result.cells_migrated == 0 => {
                unchanged += 1;
                println!("SKIP  {} (no align* cells)", path.display());
            }
            Ok(result) => {
                if dry_run {
                    println!(
                        "OK    {} ({} cells; dry-run, not written)",
                        path.display(),
                        result.cells_migrated
                    );
                } else if let Err(error) = std::fs::write(path, &result.rewritten) {
                    eprintln!("ABORT {}: write: {error}", path.display());
                    aborted += 1;
                    continue;
                } else {
                    println!("WRITE {} ({} cells)", path.display(), result.cells_migrated);
                }
                migrated += 1;
            }
            Err(error) => {
                eprintln!("ABORT {}: {}", path.display(), error.message());
                aborted += 1;
            }
        }
    }
    ...
    println!(
        "migrate-skos-match: {migrated} migrated, {unchanged} unchanged, {aborted} aborted{}",
...
/// Recursively collect `*.ttl` files under `<root>/**/mappings/` that carry a
/// `gmeow:TermEquivalence` cell, excluding `declined-bridges.ttl` (declined records, kept
/// as-is) and anything under a `tests/` path (hand-migrated to keep intent visible).
fn discover_cell_files(root: &Path) -> std::io::Result<Vec<PathBuf>> {
    let mut out = Vec::new();
    ...
    while let Some(dir) = stack.pop() {
        for entry in std::fs::read_dir(&dir)? {
            ...
            let is_declined =
                path.file_name().and_then(|n| n.to_str()) == Some("declined-bridges.ttl");
            if !in_mappings || is_test || is_declined {
                continue;
            }
            if std::fs::read_to_string(&path)?.contains("gmeow:TermEquivalence") {
                out.push(path);
            }
        }
    }

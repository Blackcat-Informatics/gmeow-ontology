...
equality through the in-repo ``purrdf`` kernel instead (issue #630): each graph
...
def _canonical_quads(nt: str) -> list[str]:
    ...
    for quad in purrdf.parse(nt.encode("utf-8"), format=purrdf.RdfFormat.N_TRIPLES):
        dataset.add(
            purrdf.Quad(quad.subject, quad.predicate, quad.object, quad.graph_name)
        )
...
def graphs_isomorphic(a: Graph, b: Graph) -> bool:
    ...
    both graphs are RDFC-1.0 canonicalized via ``purrdf`` and their canonical

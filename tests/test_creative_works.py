...
def _graph() -> Graph:
    ...
    Returns:
        g (rdflib.Graph): The merged RDF graph with imports excluded (include_imports=False).
    """
...
def test_wemi_tiers_are_kinds() -> None:
    """
    Assert that each WEMI tier class (Work, Expression, Manifestation, Item) is declared as a gufo:Kind in the merged graph.
    
    The test loads the merged graph and verifies that each listed class has an `rdf:type` of `gufo:Kind`.
    """
    graph = _graph()
...
def test_wemi_tiers_subclass_information_object() -> None:
    ...
    Asserts that GMEOW.Work, GMEOW.Expression, GMEOW.Manifestation, and GMEOW.Item have GMEOW.InformationObject in their rdfs:subClassOf closure.
    """
    graph = _graph()
    for cls in (GMEOW.Work, GMEOW.Expression, GMEOW.Manifestation, GMEOW.Item):
...
def test_wemi_tiers_subclass_creative_work() -> None:
    """
    Verify that each WEMI tier class is declared as a direct subclass of GMEOW.CreativeWork.
    
    Checks that GMEOW.Work, GMEOW.Expression, GMEOW.Manifestation, and GMEOW.Item each have an (rdfs:subClassOf, GMEOW.CreativeWork) triple in the merged graph; the test fails if any are missing.
    """
    graph = _graph()
    for cls in (GMEOW.Work, GMEOW.Expression, GMEOW.Manifestation, GMEOW.Item):
...
def test_media_etc_subclasses_manifestation() -> None:
    """
    Assert that specific media-related classes are direct subclasses of GMEOW.Manifestation.
    
    Checks that GMEOW.MediaObject, GMEOW.WebPage, GMEOW.BookRelease, and GMEOW.SerialInstallment each have an explicit rdfs:subClassOf relation to GMEOW.Manifestation.
    """
    graph = _graph()
    for cls in (
        GMEOW.MediaObject,
...
def test_spine_relations_exist() -> None:
    """
    Asserts that each spine relation is declared as an `owl:ObjectProperty` in the merged graph.
    
    Checks that the expected WEMI spine properties (e.g., `realizes`, `realizedThrough`, `embodies`, `embodiedIn`, `exemplifies`, `exemplifiedBy`, `hasCarrier`) are present with `rdf:type owl:ObjectProperty`.
    """
    graph = _graph()
    for prop in (
        GMEOW.realizes,
...
def test_realizes_inverse() -> None:
    """
    Asserts that the merged graph declares GMEOW.realizedThrough as the owl:inverseOf of GMEOW.realizes.
    
    Raises:
        AssertionError: If the inverseOf triple (GMEOW.realizedThrough, OWL.inverseOf, GMEOW.realizes) is not present in the graph.
    """
    graph = _graph()
...
def test_exemplifies_inverse() -> None:
    """
    Assert that the `exemplifiedBy` property is declared as the OWL inverse of `exemplifies` in the merged graph.
    
    This test loads the merged WEMI graph and checks for the triple `(GMEOW.exemplifiedBy, owl:inverseOf, GMEOW.exemplifies)`.
    """
    graph = _graph()
...
def test_contribution_is_relatore_kind() -> None:
    """
    Check that GMEOW.Contribution is declared as a gufo Kind and as a subclass of gufo:Relator.
    
    Asserts the graph contains (GMEOW.Contribution, rdf:type, GUFO.Kind) and (GMEOW.Contribution, rdfs:subClassOf, GUFO.Relator).
    """
    graph = _graph()
...
def test_contribution_properties_exist() -> None:
    """
    Verify that contribution-related properties are declared as functional object properties.
    
    Asserts that GMEOW.contributor, GMEOW.contributionTarget, and GMEOW.contributionRole each have both rdf:type owl:ObjectProperty and rdf:type owl:FunctionalProperty.
    """
    graph = _graph()
    for prop in (
        GMEOW.contributor,
...
def test_has_contributor_exists() -> None:
    """
    Verify that GMEOW.hasContributor is declared as an OWL ObjectProperty in the merged graph.
    """
    graph = _graph()
...
def test_has_author_is_subproperty_of_has_contributor() -> None:
    """
    Verify that GMEOW.hasAuthor is declared as an rdfs:subPropertyOf GMEOW.hasContributor in the merged graph.
    
    This test loads the merged graph and checks for the (hasAuthor, rdfs:subPropertyOf, hasContributor) triple.
    """
    graph = _graph()
...
def test_flat_shortcuts_are_subproperties() -> None:
    """
    Verify that contributor shortcut properties (hasTranslator, hasIllustrator, hasNarrator, hasEditor) are declared as OWL object properties and are subproperties of `GMEOW.hasContributor`.
    """
    graph = _graph()
    for prop in (
        GMEOW.hasTranslator,
...
def test_creative_work_type_value_vocab() -> None:
    """
    Assert that CreativeWorkType is a subclass of gufo:QualityValue and that the common work-type vocabulary members are modeled as individuals of GMEOW.CreativeWorkType.
    
    Verifies that GMEOW.CreativeWorkType rdfs:subClassOf GUFO.QualityValue and that each expected work-type resource (e.g., workTypeLiterary, workTypeWritten, workTypeNarrative, workTypeMusical, workTypeComposedMusical, workTypeVisual, workTypePhotographic, workTypeAudiovisual, workTypeFilm, workTypeChoreographic, workTypeCartographic, workTypeSoftware, workTypeDataset) has rdf:type GMEOW.CreativeWorkType.
    """
    graph = _graph()
...
def test_contribution_role_value_vocab() -> None:
    """
    Verify the contribution-role vocabulary is modeled as a value class and that its members are individuals.
    
    Asserts that `GMEOW.ContributionRole` is a subclass of `gufo:QualityValue` and that each expected contribution role (e.g., `roleAuthor`, `roleEditor`, `roleTranslator`, etc.) is typed as an instance of `GMEOW.ContributionRole`.
    """
    graph = _graph()
...
def test_manifestation_format_value_vocab() -> None:
    """
    Verify that GMEOW.ManifestationFormat is a subclass of GUFO.QualityValue and that each expected manifestation format is an individual of GMEOW.ManifestationFormat.
    
    Asserts the subclass relationship and that the listed format resources (hardcover, paperback, EPUB, PDF, audiobook, vinyl, digital file, web page) have rdf:type GMEOW.ManifestationFormat.
    """
    graph = _graph()
...
def test_carrier_medium_value_vocab() -> None:
    """
    Verify that GMEOW.CarrierMedium is declared as a subclass of gufo:QualityValue and that specific carrier medium resources are modeled as individuals of GMEOW.CarrierMedium.
    
    Checks that the following resources are typed as instances of GMEOW.CarrierMedium: GMEOW.mediumPrint, GMEOW.mediumEInkFile, GMEOW.mediumOpticalDisc, and GMEOW.mediumServerObject.
    """
    graph = _graph()
...
def test_creation_event_types_exist() -> None:
    """
    Verify that the expected creation event type individuals are present in the merged graph.
    
    Checks that the individuals GMEOW.eventTypeWorkConception, GMEOW.eventTypeExpressionCreation,
...
def _add_work(g: Graph, work: URIRef) -> None:
    ...
    Adds two triples for the given resource: types it as `GMEOW.Work` and assigns the rdfs:label "Test Work".
...
def _add_expression(g: Graph, expr: URIRef, work: URIRef) -> None:
    """
    Add an Expression node to the graph and attach a populated reference frame required for SHACL validation.
    ...
    Description:
        Creates triples typing `expr` as a `GMEOW.Expression`, gives it a label, links it to `work` via `GMEOW.realizes`, and associates it with `EX.englishFrame`. Also populates `EX.englishFrame` and related nodes with the properties and types expected by the SHACL shape (realm, axis, dimension count, frame kind, host requirement, and determinacy).
    """
...
def _add_item(g: Graph, item: URIRef, manif: URIRef) -> None:
    """
    Populate the graph with a minimal Item individual linked to a Manifestation for SHACL tests.
...
def test_item_without_manifestation_fails_shacl() -> None:
    ...
    Asserts that SHACL validation returns a failing result and that at least one validation error message contains the substring "Item must exemplify".
    """
    g = Graph()
...
def test_contribution_missing_role_fails_shacl() -> None:
    ...
    Creates a minimal graph with a Contribution that has a contributor and a contributionTarget but no contributionRole, runs SHACL, and asserts validation fails and at least one error message contains "Contribution must specify exactly one role".
    """
    g = Graph()
    g.add((EX.contribution, RDF.type, GMEOW.Contribution))
    g.add((EX.contribution, GMEOW.contributor, EX.alice))

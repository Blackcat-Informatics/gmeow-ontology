...
- **Source:** [`slices/core/logic/design/LOGIC-MIGRATION.md`](https://github.com/Blackcat-Informatics/gmeow-ontology/blob/main/slices/core/logic/design/LOGIC-MIGRATION.md)
...
> Status: rollout specification for [`logic:`](https://blackcatinformatics.ca/logic/). This is the **migration** member of the
...
|---|---|---|
| **v0: IR + projection ledger** | Parse [`logic:`](https://blackcatinformatics.ca/logic/) into a typed IR; emit OWL/SHACL/Datalog/N3/gUFO projections + the projection report | Native solving |
...
| **v6: probabilistic/weighted layer** | Explicit [`logic:probability`](https://blackcatinformatics.ca/logic/probability)/[`logic:weight`](https://blackcatinformatics.ca/logic/weight) semantics with tests | Treating [`logic:confidence`](https://blackcatinformatics.ca/logic/confidence) as probability |

The executable-spec authority depends on the rung's kind, and the Rust core must pass the identical
corpus either way ([Principle 7](https://github.com/Blackcat-Informatics/gmeow-ontology/blob/main/CONSTITUTION.md#principle-7)):
...
The Python oracle is permanent either way ([Principle 7](https://github.com/Blackcat-Informatics/gmeow-ontology/blob/main/CONSTITUTION.md#principle-7)).
...
**v6 (probabilistic/weighted layer).** Exact marginal inference by weighted model counting
under [`logic:ProbabilisticProfile`](https://blackcatinformatics.ca/logic/ProbabilisticProfile), gated by an **explicitly declared** [`logic:ProbabilityModel`](https://blackcatinformatics.ca/logic/ProbabilityModel)
([`logic:FullIndependence`](https://blackcatinformatics.ca/logic/FullIndependence) or a [`logic:DependencyModel`](https://blackcatinformatics.ca/logic/DependencyModel) carrying an explicit joint). Probabilistic
facts ([`logic:probability`](https://blackcatinformatics.ca/logic/probability)) enter inference only through the declared model; a [`logic:confidence`](https://blackcatinformatics.ca/logic/confidence)
...
[`logic:`](https://blackcatinformatics.ca/logic/) does not require rewriting the existing model on day one. Two adapter phases run in parallel,
each letting legacy source coexist with canonical [`logic:`](https://blackcatinformatics.ca/logic/) source while the source-of-truth boundary
moves:

- **The [`owl:*`](http://www.w3.org/2002/07/owl#*) adapter.** Existing [`owl:*`](http://www.w3.org/2002/07/owl#*) axioms are normalized into the same typed IR as [`logic:`](https://blackcatinformatics.ca/logic/)
 axioms, so the OWL projection of [`logic:`](https://blackcatinformatics.ca/logic/) and the hand-authored OWL agree by round-trip isomorphism
 (the gate already proven for the statement layer, `statement_compile.py:123-130,194-210`).
- **The [`gufo:`](http://purl.org/nemo/gufo#) adapter.** Existing [`gufo:`](http://purl.org/nemo/gufo#) stereotype authoring is accepted exactly parallel to the
 [`owl:*`](http://www.w3.org/2002/07/owl#*) adapter: a slice may declare a [`logic:`](https://blackcatinformatics.ca/logic/) foundational role, and the [gUFO](../../../external/ontologies.md#target-gufo) projection generator
 emits the [`gufo:`](http://purl.org/nemo/gufo#) stereotypes that `reasoning_lint.py` validates. Nothing in the current model
 breaks.

The adapters are temporary by design — they exist so the migration is incremental, not so the legacy
forms remain canonical. A construct authored in both [`logic:`](https://blackcatinformatics.ca/logic/) and a legacy form must normalize
identically or the build fails.

## Foundation projection and discipline

UFO⁺ is authored canonically in [`logic:`](https://blackcatinformatics.ca/logic/); the upper ontologies are generated, and they are **not all
the same kind of projection**:

- **[gUFO](../../../external/ontologies.md#target-gufo)** is the **primary generated down-projection** of UFO⁺ — the OWL realization of the same UFO
...
- **BFO, DOLCE, and [SUMO](../../../external/ontologies.md#target-sumo)** are generated **alignment/bridge views, not truth-preserving projections**,
...
**Implemented in v3.** The lint-to-axiom move for the OntoUML disciplines is now
partially complete:

- **Five in-world disciplines lowered, evaluated natively (done; oracle retired in).** The
 native Rust evaluator `crates/logic/src/foundation.rs` (exposed as `gmeow_logic.foundation`) derives
 ?C [`logic:violation`](https://blackcatinformatics.ca/logic/violation) <label> facts reproducing, class-for-class, the offending sets `reasoning_lint.py`
 produces — five [`logic:violation`](https://blackcatinformatics.ca/logic/violation) labels from four lint checks: `exactly_one_stereotype`
 ([`logic:StereotypeCardinality`](https://blackcatinformatics.ca/logic/StereotypeCardinality)), `identity_overlap` ([`logic:MixIden`](https://blackcatinformatics.ca/logic/MixIden)), `anti_rigidity_discipline`
 ([`logic:FreeRole`](https://blackcatinformatics.ca/logic/FreeRole), [`logic:MixRig`](https://blackcatinformatics.ca/logic/MixRig)), and `relator_mediation` ([`logic:RelComp`](https://blackcatinformatics.ca/logic/RelComp)). The lowering
 certifies under [`logic:StratifiedNAFProfile`](https://blackcatinformatics.ca/logic/StratifiedNAFProfile). Correctness is proven end-to-end by the foundation
 ...
 **`reasoning_lint.py` is now the regression specification for these five disciplines**, not the
 enforcement mechanism; enforcement is the native lowering materialized over [`logic:`](https://blackcatinformatics.ca/logic/) facts. The
 Python oracle (`logic_foundation.py`) that originally hosted the lowering has been deleted (
...
- **Positive cross-world rigidity decided over the materialized world set (done).** This is a
 ...
 finite materialized world set — emitting [`logic:rigidityViolation`](https://blackcatinformatics.ca/logic/rigidityViolation) quads in the world where rigidity
...
- **Witness-world construction deferred to.** Anti-rigidity formally requires a world of
 existence where the instance lacks the type. Construction of that counter-world belongs to Stratum C
 and is tracked in. The `"anti_rigidity_policy"` `profile.json` field (see
...
- **IR normalization** for existing OWL and [gUFO](../../../external/ontologies.md#target-gufo) source (adapter phase);
- **equivalence** for matching [`logic:`](https://blackcatinformatics.ca/logic/), [`owl:*`](http://www.w3.org/2002/07/owl#*), and [`gufo:`](http://purl.org/nemo/gufo#) source forms — the round-trip
...
- **oracle/engine agreement** — the Python oracle and the Rust core must pass one shared,
 language-neutral conformance corpus identically (the conformance gate; [Principle 7](https://github.com/Blackcat-Informatics/gmeow-ontology/blob/main/CONSTITUTION.md#principle-7));
...
- **foundation conformance**: the [gUFO](../../../external/ontologies.md#target-gufo) downcast passes `reasoning_lint.py` unchanged and every existing
 [`gufo:`](http://purl.org/nemo/gufo#)-dependent slice stays valid.
...
1. **HermiT/ELK move from authority to secondary validators.** They already sit outside the fast gate
 ...
 With **v5 landed ** the native [`logic:`](https://blackcatinformatics.ca/logic/) corpus now spans Strata A/B/C (worlds A/B/C, backward
 ...
 (a dual-run analogous to 's SHACL cross-check) proving `make reason --mode native` ≡ ELK on the
 EL fragment before authority moves. Tracked as its own follow-up epic; the SHACL counterpart is
...
3. **[`owl:*`](http://www.w3.org/2002/07/owl#*) and [`gufo:`](http://purl.org/nemo/gufo#) adapter source is retired per slice**, only after that slice's [`logic:`](https://blackcatinformatics.ca/logic/) form

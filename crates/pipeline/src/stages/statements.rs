...
fn extract_cells(store: &Store) -> Result<Vec<StatementCell>, PipelineError> {
    ...
    for quad in store.quads_for_pattern(
        ...
        cells.extend(parse_cell(store, cell)?);
    }
    Ok(cells)
}
...
/// cloned per reifier.
fn parse_cell(store: &Store, cell: &NamedNode) -> Result<Vec<StatementCell>, PipelineError> {
    ...
    let mut reifiers = all_named(store, cell, &format!("{GMEOW}reifier"))?;
...
fn all_named(
    ...
    subject: &NamedNode,
    predicate: &str,
) -> Result<Vec<NamedNode>, PipelineError> {
    let p = nn(predicate)?;
    let mut out: Vec<NamedNode> = Vec::new();
    for quad in store.quads_for_pattern(Some(subject.into()), Some(p.as_ref()), None, None) {
        let quad = quad.map_err(|e| PipelineError::Parse(e.to_string()))?;
        match quad.object {

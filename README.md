...
| `make rdf12` | RDF 1.2 / RDF\* view via Apache Jena (**required** — fails if absent) |
| `make quality` | OOPS! pitfall scan (network, best-effort) |
| `make release` | Reasoned closure (HermiT) + build + metadata + RDF 1.2 / RDF\* view |
...
queries/rdf12-project.rq  RDF 1.2 / rdf-star projection query
...
projection* — applies to the alignment layer itself. Every mapping is authored
...
### RDF 1.2 / RDF\* — first-class

GMEOW is **RDF 1.2 / RDF\*-first**: statement-level metadata — provenance, confidence,
temporal scope — is native triple-term content, modelled directly rather than bolted on.
Because RDF 1.2 is still finalizing at the W3C and most current tooling (including the OWL 2
DL reasoners GMEOW gates on) does not yet consume it, GMEOW *also* emits a **plain-RDF
compatibility encoding** — OWL 2 axiom annotations (`owl:Axiom` +
`owl:annotatedSource/Property/Target`) — for those not-yet-updated tools. That plain-RDF form
is the **downgrade for legacy tooling**, the same lossy-compatibility-as-projection principle
GMEOW applies to schema.org / vCard / FOAF — not a competing source of truth. The RDF 1.2
Turtle serialization (`make rdf12`, via Apache Jena) is gated only on that toolchain maturing.

...
The headline arithmetic: of the roughly twenty-five distinct concepts across the three sources, most
are REUSE, EXTEND, or BRIDGE. What is genuinely **MINT** is the inhabitation topology and continuity in
core (the tenure/configuration situations, the role-mixins, the assessments, the claim form, the
embodiment carrier and assignment, the transition value) plus the model-serving identities in a thin
extension — the authoritative inventory is the [Net new vocabulary](#net-new-vocabulary) table. The
count is deliberately *higher* than a naïve "collapse it to five," because terms whose identity
criteria differ (subject ⟂ model ⟂ deployment ⟂ runtime ⟂ session) are kept distinct rather than
overloaded onto one node.
...
| **Role** | the interface specification between an Actor and the holon it inhabits; permitted actions, info access, presentation norms | REUSE | `logic:Role` / `gmeow:Role` (organization slice — relationally-acquired function-in-context). Info-access gating is carried by the `InhabitationTenure` that names the role-filler; presentation norms route to `gmeow:Persona` / `gmeow:Register`. |
...
| **scene graph** | the runtime view mapping Avatars + Roles, not underlying Actors | BRIDGE (solver-side) | Not asserted data. The scene graph is a **computed projection** (Principle 12) over the embodiment and role fillers of the currently-active `InhabitationTenure`s — a view, never triples. |
...
| **HostSystem / RuntimeEnvironment** | the environment capable of being inhabited | REUSE | `gmeow:PhysicalObject` / `gmeow:SoftwareAgent` located via places, composed via `gmeow:partOf`. **No new class** — "hosted on" is containment. (A thin `gmeow:RuntimeEnvironment` subkind may be minted in `extensions/model-serving` only if it earns its keep — see [`INHABITED-RUNTIME-STACK.md`](INHABITED-RUNTIME-STACK.md).) |

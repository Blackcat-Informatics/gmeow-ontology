...
use crate::rule_ir::{
    DerivedRow, EvalRule, Fact, FactKey, FactStore, echo_asserted, least_model_of_reduct,
    world_edb_facts,
};
use crate::{
...
fn model_key_vec(m: &StableModel) -> Vec<FactKey> {
...
fn cautious_ground_slice(
    ...
    let cautious_keys: BTreeSet<FactKey> = match models.first() {
        None => BTreeSet::new(),
        Some(first) => {

...
- **Source:** [`slices/core/logic/design/LOGIC-RUNTIME.md`](https://github.com/Blackcat-Informatics/gmeow-ontology/blob/main/slices/core/logic/design/LOGIC-RUNTIME.md)
...
> Status: runtime specification for [`logic:`](https://blackcatinformatics.ca/logic/). This is the **runtime** member of the
...
- **Closest RDF-native logic kin:** N3 Logic with EYE/cwm — RDF rules, quoted graphs, builtins, both
 chaining directions. [`logic:`](https://blackcatinformatics.ca/logic/) is its maximal successor.
- **Backward chaining (chosen): embed Scryer Prolog.** [`logic:`](https://blackcatinformatics.ca/logic/) embeds **Scryer Prolog** (Rust) for
 ...
 optimization, not a prerequisite. Cut is confined to [`logic:ProceduralPrologProfile`](https://blackcatinformatics.ca/logic/ProceduralPrologProfile).
- **Materialization / existential rules: Nemo.** **Nemo** (knowsys; Rust) provides the
 ...
 because a canonical engine cannot rest on proprietary availability ([Principle 5](https://github.com/Blackcat-Informatics/gmeow-ontology/blob/main/CONSTITUTION.md#principle-5)).
...
- **[`Contradiction`](../../../reference/classes/gmeow-Contradiction.md) witnesses:** minimal conflict-set / unsat-core plus truth-maintenance (ATMS/JTMS);
...
- **The Rust core is the production engine**, in three layers: **oxigraph** (storage, SPARQL, RDF 1.2
 ...
 Rust** (the [`logic:`](https://blackcatinformatics.ca/logic/) IR compiler and [`owl:*`](http://www.w3.org/2002/07/owl#*)/[`gufo:`](http://purl.org/nemo/gufo#) adapters, the modal/foundation lowering,
...
- **PyO3 and wasm are the bindings.** PyO3 makes the Rust core a drop-in behind the `gmeow logic`
 Python API, cross-checked against the oracle like the rdflib≡pyoxigraph gate ([Principle 7](https://github.com/Blackcat-Informatics/gmeow-ontology/blob/main/CONSTITUTION.md#principle-7)). A wasm
...
> Ownership boundary. Nemo provides the existential-rule/chase *substrate* used by [`logic:`](https://blackcatinformatics.ca/logic/) world
> construction — it is not, by itself, a world-construction engine. [`logic:`](https://blackcatinformatics.ca/logic/) defines the
...
> [`logic:`](https://blackcatinformatics.ca/logic/)-level design choice layered on Nemo, not a property Nemo ships.
...
The logic compiler mirrors that — compile [`logic:`](https://blackcatinformatics.ca/logic/) (and adapter-phase [`owl:*`](http://www.w3.org/2002/07/owl#*) / [`gufo:`](http://purl.org/nemo/gufo#)) into one
typed IR, then emit the canonical RDF 1.2 artifact and every projection from it. Passing HermiT and
ELK over the OWL artifacts proves only the projected subset, never full [`logic:`](https://blackcatinformatics.ca/logic/) consistency. The
...
A logic this expressive is humane only if it can say *why* in words — and [`logic:`](https://blackcatinformatics.ca/logic/) can, because GMEOW
already mandates the raw material. Every term has an [`rdfs:label`](http://www.w3.org/2000/01/rdf-schema#label), a [`skos:definition`](http://www.w3.org/2004/02/skos/core#definition),
[`skos:scopeNote`](http://www.w3.org/2004/02/skos/core#scopeNote)/[`skos:example`](http://www.w3.org/2004/02/skos/core#example) where the documentation doctrine applies, and the markdown guide

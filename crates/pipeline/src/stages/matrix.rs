...
pub fn render_matrix(root: &Path) -> Result<String, PipelineError> {
    ...
    let footer = format!(
        "\n**{total} slices** · documentation: {} full guides, {stubs} stubs remaining (#325); advisory coverage: {adv_done}/{adv_total} core class/property terms (#471).\n",
        total - stubs
    );
    Ok(format!("{HEADER}{}\n{footer}", lines.join("\n")))
}
...
fn term_counts(path: &Path) -> Result<(usize, usize, usize, usize, usize), PipelineError> {
    ...
    let classes = subjects_of(&format!("{OWL}Class"));
    let obj = subjects_of(&format!("{OWL}ObjectProperty"));
    let dt = subjects_of(&format!("{OWL}DatatypeProperty"));
    let ann = subjects_of(&format!("{OWL}AnnotationProperty"));
    let inds = subjects_of(&format!("{OWL}NamedIndividual"));

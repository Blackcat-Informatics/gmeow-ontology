...
//! The Symmetric CBD `describe` extraction (in `gmeow-rdf-core`) must produce a
//! structurally valid subgraph that every `native_codecs` serializer here in
//! `gmeow-rdf` can emit — the docs multi-format export depends on exactly that hand-off.

use gmeow_rdf::describe::describe;
use gmeow_rdf::native_codecs::jsonld::serialize_dataset_to_jsonld;

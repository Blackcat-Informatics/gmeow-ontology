...
//! computed by the producer-prepared shipped `logic:Rule` set from the entry's lifecycle-axis witnesses, so a
...
//!
//! Tiny synthetic provenance controls remain here. Authored example evaluations and
//! mutations are producer work; commands::operator_corpus_tests renders their authenticated
//! native records and preserves the agreement/disagreement assertions.
...
// Authored source provenance cases live in commands::operator_corpus_tests;
// these integration controls retain only independently constructed small inputs.

...
use gmeow_rdf_events::{RdfEventSink, RdfEventSource};
...
/// the `gmeow-gts` text codecs. Holds no state and references no oxigraph Store.
...
mod tests {
    ...
    fn quoted_triple_term_round_trips() {
        ...
        let ds = b.freeze().expect("freeze");
        // FINDING (#909): gmeow-gts 0.9.5's `to_turtle`/`to_ntriples`/`to_trig`/
        ...
        // term N" (reproducible directly against gmeow-gts's own `from_nquads` output).
        ...
        // path are correct (the GTS graph is byte-identical to gmeow-gts's native
        // `from_nquads` representation); this is a gmeow-gts serializer gap.
        assert_round_trips(&ds, "application/n-quads");
    }

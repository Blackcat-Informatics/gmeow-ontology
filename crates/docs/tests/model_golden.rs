...
use gmeow_docs::{
    DocConcern, DocCompetency, DocExample, DocExternalTerm, DocLearningPath, DocLinkage,
...
struct ModelSummary {
    ...
    // ── New (#1020) structural collections: counts + one sample each ─────────
    /// Number of SHACL node shapes reverse-mapped to documented terms.
    shape_count: usize,
    /// Number of competency questions reverse-mapped to documented terms.
    competency_count: usize,
    /// ONE fully serialized shape (first by target term then shape IRI).

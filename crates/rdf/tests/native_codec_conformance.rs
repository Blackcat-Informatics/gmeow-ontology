...
//! `gmeow-rdf` text codecs (#909 acceptance: "W3C syntax test suites round-trip").
//!
//! This harness is deliberately **oxigraph-free**: it builds under
//! `--no-default-features --features gts`, so a green run proves the native Turtle /
...
/// Allowlist of W3C tests the native `gmeow-gts` codecs do not handle the same way the
...
///
/// As of gmeow-gts 0.9.7 (#909) the native codecs pass the **entire** W3C RDF 1.2 syntax
...
///   validator accepts GMEOW's long private-use subtags (`x-gmeow-norwegiannynorsk`)

...
//! emitters: the projection parser and closed-algebra renderer live in this crate.
...
/// The 45 projection profiles, in stable compiler emission order.
const PROFILES: &[&str] = &[
    "schema-org",
    "vcard",
    "foaf",
...
pub(crate) fn render_expr(expr: &Expr) -> String {
    match expr {
        ...
        Expr::Op { op, args } => {
            ...
            // Unknown operators produce a sentinel that cannot match committed
            // bytes; the parity gate catches any such source bug.
...
/// The language-retag block. Returns `(retag_lines, bind_expr)` when both are set.
...
mod tests {
    ...
    #[test]
    fn emit_sparql_produces_all_profiles() {
        // This legacy oxigraph emitter is SUPERSEDED by the oxigraph-free
        // `gmeow_logic_compile::projections::sparql` lowering (#1089), which the
        // pipeline now drives; the committed `*.rq` are generated by that lowering's
        // deterministic cell order. Byte-parity against the committed corpus is now
        // proven by `crates/pipeline/tests/edoal_sparql_correspondence_parity.rs`. This
        // smoke test only asserts the legacy emitter still produces every profile.
        let sets = emit_sparql_sets(&repo_root()).expect("emit sparql");
        assert_eq!(sets.len(), 45, "expected 45 SPARQL files");
        let _ = first_diff("a", "a");
    }

    #[test]
    fn committed_queries_contain_every_rendered_guarded_branch() {
        let root = repo_root();
        let dsl_store = collect_dsl_store(&root).expect("collect DSL");
        let onto_store = collect_ontology_store(&root).expect("collect ontology");
        let vocab = suppression_vocab(&onto_store).expect("suppression vocab");
        let cells = projections(&dsl_store).expect("projection cells");
        let queries = root.join("generated").join("queries");
        let mut checked = 0usize;
        let mut missing: Vec<String> = Vec::new();

        for cell in &cells {
            let guards = injected_guards(&cell.pattern, &vocab);
            if guards.is_empty() {
                continue;
            }
            for binding in &cell.bindings {
                let query_path = queries.join(format!("{}.rq", binding.profile));
                let query = std::fs::read_to_string(&query_path)
                    .unwrap_or_else(|e| panic!("read {}: {e}", query_path.display()));
                let rendered = branch_of(cell, binding, &vocab).expect("render guarded branch");
                checked += 1;
                if !query.contains(&rendered) {
                    missing.push(format!("{} {}", binding.profile, cell.iri));
                }
            }
        }
        ...
        assert!(
            checked > 0,
            "expected at least one guarded projection branch"
        );
        assert!(
            missing.is_empty(),
            "{} guarded branch(es) missing from committed queries:\n{}",
            missing.len(),
            missing.join("\n")
        );
    }

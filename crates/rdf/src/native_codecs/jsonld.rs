...
//!
//! Serializes the frozen [`RdfDataset`] to the GMEOW JSON-LD-star lead artifact and a
...
//! and never touches the external `gmeow-gts` codecs. GTS is exit-only.
//!
...
/// `schemas-archive/gmeow.schema.json`, so a bare member name resolves inside the
/// bundle.
...
/// `@annotation` idiom produced by the GMEOW JSON-LD-star emitter and reconstructs RDF
...
pub fn parse_jsonld(json_bytes: &[u8]) -> Result<Arc<RdfDataset>, RdfDiagnostic> {
    ...
    let emit_node = |node: &Value, graph_iri: Option<&str>| -> Result<(), RdfDiagnostic> {
        ...
        if let Some(types) = node.get("@type") {
            ...
            // (standard JSON-LD) or a `{"@id": …}` node reference (the GMEOW emitter
...
fn validated_iri_term(iri: &str) -> Result<RdfTerm, RdfDiagnostic> {
    gmeow_sparql_algebra::NamedNode::new(iri.to_string()).map_err(|e| decode(e.to_string()))?;
...
/// native GMEOW statement-metadata cell:
///
/// ```turtle
...
pub fn jsonld_to_gmeow_statement_metadata_nquads(
    json_bytes: &[u8],
) -> Result<String, RdfDiagnostic> {
...
pub fn yamlld_to_gmeow_statement_metadata_nquads(
    yaml_bytes: &[u8],
) -> Result<String, RdfDiagnostic> {
    jsonld_to_gmeow_statement_metadata_nquads(yamlld_to_jsonld(yaml_bytes)?.as_bytes())
}

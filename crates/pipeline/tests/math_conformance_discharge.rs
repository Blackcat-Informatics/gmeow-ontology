...
fn fixture_closure(
    ...
    let derived: Vec<purrdf::RdfQuad> = result
        ...
        .map(|ax| {
            purrdf::RdfQuad::new(
                purrdf::RdfTerm::iri(bare_iri(&ax.subject)),
                bare_iri(&ax.predicate),
...
/// `crates/logic/src/verify.rs` applies before building its derived-edge quads.
fn bare_iri(term: &str) -> &str {
    term.strip_prefix('<')

...
const INCREMENTAL_SOLVER_VERSION: &str = "gmeow-native-dbsp-v2-native-bindings";
...
pub(crate) struct IncrementalDelta {
    /// Signed closure rows in native [`FactKey`] order.
...
impl IncrementalSession {
    ...
    /// derived fact is charged once, in native `FactKey` order at its first inner
    ...
    /// native [`FactKey`] order. The settle circuit itself is untouched.
    ///
    /// # Errors
    ///
...
pub(crate) enum UnsupportedFragmentReason {
    ...
    Builtins,
    /// A complete-group reduce operator, requiring aggregate-aware maintenance.
    Aggregation,
...
impl FragmentRefusal {
    ...
    fn message(&self) -> String {
        match &self.reason {
            UnsupportedFragmentReason::Aggregation => format!(
                "incremental circuit requires aggregate-aware maintenance for rule <{}>",
                self.rule_iri
            ),
    ...
    for rule in rules {
        if rule.reduction.is_some() {
            return Err(FragmentRefusal {
                rule_iri: rule.rule_iri.clone(),
                reason: UnsupportedFragmentReason::Aggregation,
                offending_var: None,
            });
        }
        ...
        if !rule.builtins.is_empty() || !rule.numeric.is_empty() {
            return Err(FragmentRefusal {
                rule_iri: rule.rule_iri.clone(),

...
[`logic:`](https://blackcatinformatics.ca/logic/) is the canonical reasoning language for GMEOW. It is RDF 1.2-native, not an OWL syntax
...
ontology. OWL deliberately trades expressivity for decidability; [`logic:`](https://blackcatinformatics.ca/logic/) does not.

[`logic:`](https://blackcatinformatics.ca/logic/) is deliberately **Turing-complete** — a computational substrate, not merely a description
language. It is meant to compute, generate, and search, not only to classify, which is the
dividing line between OWL (sub-Turing by design) and the Prolog/N3 lineage [`logic:`](https://blackcatinformatics.ca/logic/) belongs to.
...
[`logic:`](https://blackcatinformatics.ca/logic/) applies that doctrine to logic itself, and to the foundational ontology.
...
[`logic:`](https://blackcatinformatics.ca/logic/) is a deliberate superset. Each predecessor contributes a fragment; each imposes a
restraint we reject; [`logic:`](https://blackcatinformatics.ca/logic/) subsumes the contribution and discards the restraint.

| Predecessor | Contributes | Restraint we reject | How [`logic:`](https://blackcatinformatics.ca/logic/) exceeds it |
|---|---|---|---|
...
[`logic:`](https://blackcatinformatics.ca/logic/) is N3's maximal successor — it keeps the RDF-native, quoted-formula, bidirectional core
...
lessons carry directly into [`logic:`](https://blackcatinformatics.ca/logic/):
...
- **Orthogonal factorization is the master principle.** Ithkuil composes meaning from
 ...
 reachable from a small set of composable primitives. The [`logic:`](https://blackcatinformatics.ca/logic/) foundational categories and
...
- **Evidentiality is obligatory and structural.** Ithkuil's *Validation* category forces every
 ...
 [`logic:`](https://blackcatinformatics.ca/logic/) makes epistemic/standpoint/provenance/confidence scope **obligatory and structural**,
...
- **Essence — existential versus representational.** Ithkuil marks whether a referent is actual or
 hypothetical/ideal/representational. [`logic:`](https://blackcatinformatics.ca/logic/) carries this as a modal/disclosure axis on every
...
| [`logic:`](https://blackcatinformatics.ca/logic/) namespace registration | `config.py` `PREFIXES` (add [`logic:`](https://blackcatinformatics.ca/logic/) beside `gmeow`, `config.py:21-24`) |
...
| Engine pattern (Rust core + Python oracle + corpus gate) | GTS GTS conformance design; `crates/gts/`; pyoxigraph model; [Principle 7](https://github.com/Blackcat-Informatics/gmeow-ontology/blob/main/CONSTITUTION.md#principle-7) |
...
[`logic:`](https://blackcatinformatics.ca/logic/) is the project's own doctrine applied to logic and to its foundation. The CONSTITUTION
...
for facts; [`logic:`](https://blackcatinformatics.ca/logic/) realizes it for axioms, rules, and the upper ontology. OWL, Datalog, SHACL,
...
- [`logic:`](https://blackcatinformatics.ca/logic/) is the canonical, maximally expressive logic — a Turing-complete computational substrate,
...
- the foundational ontology (UFO⁺) is authored in [`logic:`](https://blackcatinformatics.ca/logic/), with its discipline expressed as axioms

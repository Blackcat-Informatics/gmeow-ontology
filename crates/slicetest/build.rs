...
//! Emit the exact implementation identity for declarative slice-spec verdicts.
//!
//! Slice verdicts and pipeline stage products have different implementation closures.
//! Keeping their identities separate means an edit that can affect only one producer
//! invalidates only that producer's action nodes.
...
fn main() {
    ...
    hash.update(b"gmeow:slicetest-build:v1\x1f");
    ...
    println!(
        "cargo:rustc-env=GMEOW_SLICETEST_BUILD_FINGERPRINT={}",
        hex(&hash.finalize())
    );
}

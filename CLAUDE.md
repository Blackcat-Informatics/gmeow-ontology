...
* **Lifecycle throughput benchmark — average six elapsed hours per issue for the complete Stage 1 → Stage 2 → Stage 3 lifecycle.** This is an end-to-end orchestration objective, not a six-hour allowance per stage or a task estimate. Record stage transitions and blocker duration so delays are visible and actionable.
* **`.deficiencies` is the emergency ledger for failed work, never an authorization surface.** Every entry below its marker is 100% unauthorized, 100% a bug, and means its originating issue or PR failed. It records critically undone work misrepresented to pass gates, misrepresented by an agent, or exposed when an agent is found fundamentally defective — literally a cry for help from a failing agent. The normal file contains only its notice and marker. Any entry blocks completion, PR creation, and merge of the work that produced it until the defect is verified and has a durable, visible remediation owner. Remove the emergency entry only after that ownership is confirmed; removal neither resolves the bug nor makes the failed work successful. Never add an entry to make incomplete work appear complete.
...
* `generated/dist/gmeow.gts` is now a git-ignored local product (not tracked, no merge driver); after integrating `main`, run `make check` once to re-materialize it from the merged sources and gate in the same pass. A stale local bundle is **not** silently accepted — the production `stage-superset-gate` projects the emitted bundle and compares its declared inventory with the materialized tree semantically because CBOR cannot use a byte-only comparison. No test is permitted to rerun the producer for this check.

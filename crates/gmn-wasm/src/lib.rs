...
/// ~1.7 MB tiktoken vocabulary deliberately excluded from THIS crate's wasm image (it
/// takes lang-bridge with `default-features = false`, dropping the `glyph-cost` feature),
/// so the browser bundle carries no tokenizer; the cost is pinned here instead. The
/// native-only anti-rot test

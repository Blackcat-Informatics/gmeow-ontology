...
//! [`DslView`] for function signatures and ontology ranges. Mapping patterns and
//! transform bindings are borrowed from the single [`CorrespondenceAnalysis`].
...
use crate::projections::get_leg::{Expr, ProjectionCell as MappingCell};
...
) -> gmeow_errors::Result<FnoLowering> {
    let functions = extract_functions(dsl)?;
    let cells = extract_cells(analysis.projection_cells())?;
...
/// Project the already admitted get-leg model into the FnO signature model.
/// FnO owns no mapping-pattern or expression parser.
fn extract_cells(cells: &[MappingCell]) -> gmeow_errors::Result<Vec<ProjectionCell>> {
    cells
        .iter()
        .map(|cell| {
            let binds = cell
                ...
                .map(|bind| {
                    ...
                    collect_expr_vars(&bind.expr, &mut refs);
                    Bind {
                        var: bind.var.clone(),
                        refs,
                    }
            ...
            Ok(ProjectionCell {
                atoms: cell
                    ...
                    .map(|atom| Atom {
                        ...
                        path_alts: atom.path_alts,
                        object_var: atom.object_var,
                    })
                    .collect(),
                value: cell.pattern.value.clone(),
                binds: order_binds(binds)?,
                transform_bindings: cell
                    .bindings
                    .iter()
                    .filter_map(|binding| {
                        binding
                            .transform
                            .as_ref()
                            .map(|transform| (binding.profile.clone(), transform.clone()))
                    })
                    .collect(),
            })
        })

...
const NON_CONFORMANT: &[&str] = &[
    ...
    "slices/grounding/math/examples/statistics-hypotheses-pvalues.ttl", // math:alternativeSidedness carries the six shared math:Sidedness NamedIndividuals (math:twoSidedAlternative/oneSidedAlternative/greaterAlternative/lessAlternative/exactTail/midPTail) defined in module.ttl — the sh:in value type is satisfied by IRI so SHACL is silent on it, but the closed-world JSON-Schema projection enumerates that sh:in as bare compact-IRI strings while the instance projector carries each value as an {"@id": …} reference, so those nodes have no closed-world schema entry standalone; the SHACL-isolation trigger is its bare gmeow:ReferenceFrame (testFrame), whose full gmeow:FrameProfile (frameKind/frameRealm/hasAxis/dimensionCount/determinacyModel/requiresHost) is carried against the module, exactly like the sibling pvalue fixtures — illustrative, validated unioned with the module by make validate

...
fn hash_seed_determinism_jsonld_star() {
    ...
    let json_a = serialize_graph(synth_to_dataset(&graph_a).as_ref()).expect("serialize graph A");
    let json_b = serialize_graph(synth_to_dataset(&graph_b).as_ref()).expect("serialize graph B");
    assert_eq!(
        json_a, json_b,
...
fn yaml_ld_has_explicit_context_and_no_anchors() {
    let graph = minimal_graph();
    let yaml =
        serialize_graph_yaml(synth_to_dataset(&graph).as_ref(), None).expect("serialize YAML-LD");
    assert!(
...
fn hand_authored_yaml_ld_star_fixture_transpiles_to_gmeow() {
    let bytes = gmeow_action_cache::selection::source_artifacts::load(
        &repo_root(),
        "stage-conformance",
        source_observation::CHANNEL,
    )
    .expect("load exact producer-selected statement-metadata observation");
    let dataset =
        purrdf::restore_pack(&bytes).expect("restore native producer result without RDF parsing");
    assert!(
        !flat_quads(&dataset)
            .iter()
            .any(|q| matches!(q.object, RdfTerm::Triple(_))),
    ...
    let observed_feature = ox_named_node("https://blackcatinformatics.ca/gmeow/observedFeature");
...
fn annotation_fragment_validates_against_json_schema() {
    ...
    let mut schema: Value = serde_json::from_slice(&schema_bytes).expect("schema is valid JSON");
...
fn yaml_ld_star_ingest_rejects_anchors() {
    ...
    let err =
        yaml_ld_star_to_json(anchored.as_bytes()).expect_err("YAML anchors/aliases must hard-fail");
    assert!(

...
`gmeow-rdf-events` is the permissive RDF 1.2 event protocol shared by the AGPL
RDF IR engine and the permissive GTS container. It is intentionally dependency
free and owns its own event term IDs rather than leaking either side's concrete
identity model.

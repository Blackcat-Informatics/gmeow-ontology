...
- Avoid for non-work lineages (use [`gmeow:versionOf`](gmeow-versionOf.md)) and for plain replacement (use [`gmeow:supersedes`](gmeow-supersedes.md)); never merge an edition into its work with [owl:sameAs](http://www.w3.org/2002/07/owl#sameAs), since editions stay first-class works with distinct identifiers and provenance.

### How to use

- Point the edition work at exactly one parent work (the property is functional); keep each edition's own identifiers, dates, and rights local, and use [`gmeow:supersedes`](gmeow-supersedes.md) when one edition replaces another in the lineage.

### Examples

- ex:hamletFolio1623 [`gmeow:editionOf`](gmeow-editionOf.md) ex:hamlet.

...
///
/// `base` is the rule-IRI base (e.g. `https://blackcatinformatics.ca/gmeow/rule/`)
...
/// as `gmeow:inWorld` annotations, not Turtle named graphs).
...
mod tests {
    ...
    fn rule_iri_is_base_plus_encoded_name() {
        assert_eq!(
            ...
            "https://blackcatinformatics.ca/gmeow/rule/el%3AsubClassOf-transitive"
        );
    }
    ...
    fn emit_reifier_blank_subject_is_anonymous_with_annotations() {
        ...
        let out = emit_reifier(
            &reifier,
            &[(
                "https://gmeow.org/ontology#viaRule".to_owned(),
                "<https://gmeow.org/rule/x>".to_owned(),
            )],
        );
    ...
    fn emit_resource_property_list() {
        let out = emit_resource(
            "https://gmeow.org/ontology#dl-el-crosscheck",
            &[
                (
                    "http://www.w3.org/1999/02/22-rdf-syntax-ns#type".to_owned(),
                    "<https://gmeow.org/ontology#CrosscheckLedger>".to_owned(),
                ),
                (
                    "https://gmeow.org/ontology#consistent".to_owned(),
                    "true".to_owned(),
                ),
            ],
        );
        assert!(out.contains("<https://gmeow.org/ontology#dl-el-crosscheck>"));
        assert!(out.contains("#type> <https://gmeow.org/ontology#CrosscheckLedger> ;"));

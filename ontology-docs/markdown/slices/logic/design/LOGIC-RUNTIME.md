...
(*"Rust core + PyO3/wasm bindings, gated by a language-neutral conformance corpus"*; a
...
- **The Rust core is the production engine**, in three layers: **oxigraph** (storage, SPARQL, RDF 1.2
 ...
 Rust** (the `logic:` IR compiler and [`owl:*`](http://www.w3.org/2002/07/owl#*)/[`gufo:`](http://purl.org/nemo/gufo#) adapters, the modal/foundation lowering,
...
The safety net is the corpus gate defines (see [LOGIC-CONFORMANCE.md](LOGIC-CONFORMANCE.md)): one
...
The logic compiler mirrors that — compile `logic:` (and adapter-phase [`owl:*`](http://www.w3.org/2002/07/owl#*) / [`gufo:`](http://purl.org/nemo/gufo#)) into one
...
the scopeNote/example backlog is **fuel for the explanation surface**, not cosmetic debt.

...
pub(crate) fn xsd_value_compare(
    ...
    let left = gmeow_xsd::parse_by_iri(left_lexical, left_datatype)
        .ok()
        .flatten()?;
    let right = gmeow_xsd::parse_by_iri(right_lexical, right_datatype)
        .ok()
        .flatten()?;
    match gmeow_xsd::value_cmp(&left, &right)? {
...
pub(crate) fn xsd_canonical_lexical(lexical: &str, datatype: &str) -> Option<String> {
    gmeow_xsd::parse_by_iri(lexical, datatype)
        .ok()
        .flatten()

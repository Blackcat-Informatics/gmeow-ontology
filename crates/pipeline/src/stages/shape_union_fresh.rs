...
pub fn authored_shape_files(root: &Path) -> Result<Vec<std::path::PathBuf>, gmeow_errors::Diag> {
    let mut files = shape_files(root)
    ...
    let generated_dir = root.join("generated").join("shapes");
    files.retain(|p| !p.starts_with(&generated_dir));
    Ok(files)
}
...
pub fn load_shapes_fresh(
    ...
    let generated_dir = root.join("generated").join("shapes");
    for file in files {
        if file.starts_with(&generated_dir) {
            let rel = file
                .strip_prefix(root)
                .ok()
                .and_then(|p| p.to_str())
            ...
            generated_disk.insert(rel.to_string());
        } else if file.starts_with(root.join("shapes")) {
            base.push(file);
        } else {
            slices.push(file);
        }

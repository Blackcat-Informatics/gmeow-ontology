...
(`wasm-parity`, `acceptance`, `bench-soak`, `conformance-heavy`, and
`medium-consumer-surface`) moved
...
| 13 | Conformance standalone logic lowering | After all 333 consistency actions, five selected class diagnostics, two source admissions, and two determinism actions had published, `stage-conformance` retained a single active worker for the remainder of an interrupted 819.213 s run | The same grounding module was selected under both a canonical HTTPS source IRI and a private URN, creating distinct `SourceCatalog` keys; stage order also placed the serial lowering before the independent native case set | Accepted structurally: every consumer now selects the canonical source identity, and execution joins that one shared lowering with native observations before prepared-operator, diagnostic, and round-trip consumers borrow it. The next O3/full-LTO run records elapsed time, peak RSS, action reuse, and artifact bytes; no speedup claim is inferred from the interrupted trace. |
| 14 | Warm pipeline cache hydration | The prior warm artifact check was 287.173 s; cached reason hydration was 119.698 s and cached snapshot hydration 129.369 s | Large cache products must be deserialized even when computation is skipped | Superseded by the unified sync boundary: full-run cumulative stage snapshots are no longer persisted; the clean whole-run manifest skips the pipeline instead. |
| 15 | Source ingestion | Hypothesis: repeated authored RDF parsing dominated source-load | Phase telemetry measured authored dataset load at 90 ms and base N-Quads at 126 ms | Rejected: an immutable authored-corpus abstraction would add API surface without touching the measured bottleneck. |
| 16 | Self-description carrier construction | 28.602 s baseline, 27.445 s candidate | Real but secondary whole-repo work | No change: output assembly is already a single canonical carrier path; speculative caching or a parallel representation would threaten Principle 4 for less than one tenth of the former slice-quality cost. |

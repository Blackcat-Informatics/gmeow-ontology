...
use crate::McpServer;
use explorer_describe::{attestation_path, verified_describe};

#[gmeow_test_batch_macros::batch_mcp_test]
fn explorer_describe_is_the_same_on_both_routes_and_matches_the_attestation() {
    let server = McpServer::from_snapshot(&super::snapshot())
        .expect("authenticated bundle view; tests never produce it");
    let witness = verified_describe(&server).expect("verified explorer describe witness");
    assert!(
        witness
            .subject

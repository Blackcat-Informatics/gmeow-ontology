...
//! RDF 1.2 statement layer into the frozen [`gmeow_rdf::RdfDataset`] IR via the
//! native codecs ([`gmeow_rdf::parse_dataset`]), pattern queries route through the
//! IR's [`gmeow_rdf::DatasetView::quads_for_pattern`] (an indexed lookup), and the
//! object terms surface as the native [`Object`] value model.
//!
//! Multi-file accumulation (the merged DSL / ontology stores) uses
//! [`gmeow_rdf::RdfDatasetBuilder::push_dataset`], which standardizes blank nodes
//! apart per source dataset (a fresh [`gmeow_rdf::BlankScope`] per file, C0.2) — the
...
///
/// `new` validates the IRI through the native `gmeow-iri` parser (via the
/// `gmeow-sparql-algebra` validator, the same RFC-3987 check oxigraph applied), so
...
impl NamedNode {
    ...
    pub fn new(iri: impl Into<String>) -> Result<Self, SliceError> {
        let iri = iri.into();
        gmeow_sparql_algebra::NamedNode::new(iri.clone())
...
/// linters use. Wraps [`gmeow_rdf::RdfDataset`]; the query helpers resolve a query
/// IRI to a dataset-local term id via [`gmeow_rdf::RdfDataset::term_id_by_value`]
/// and pattern-scan via the indexed [`gmeow_rdf::DatasetView::quads_for_pattern`].
#[derive(Debug)]
pub struct Dataset {
    ds: RdfDataset,
}
...
impl Dataset {
    ...
    /// parse errors. Lenient on GMEOW's `@x-gmeow-*` language tags (the native codecs
    /// are fully lenient).
    ...
    /// `gmeow_rdf::canonical_nquads` over a flat oxigraph quad set: both canonicalize
    ...
    pub fn canonical_nquads_flat(&self) -> Result<String, SliceError> {
        ...
        Ok(gmeow_rdf::canonicalize(&frozen).nquads)
    }
    ...
    /// oxigraph-free twin of `gmeow_rdf::oxigraph::flat_rdf_quads_from_dataset`.
...
/// standardizing each document's blank nodes apart (a fresh [`gmeow_rdf::BlankScope`]

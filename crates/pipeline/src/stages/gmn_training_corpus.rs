...
mod tests {
    ...
    /// `gmnRingRestricted` (flows nowhere but itself) — the injected-leak ring the negative
    /// ring-leak test tags a referenced atom with.
    const RING_RESTRICTED: &str = "https://blackcatinformatics.ca/gmeow/gmnRingRestricted";
    ...
    fn ctx() -> GenContext {
        GenContext::resolve(&repo_root()).expect("resolve signature")
    }

    /// The corpus dataset is byte-deterministic across two runs (no clock, no RNG, no HashMap
    /// iteration order) — mirrors math_producers' determinism discipline.
    #[test]
    fn run_is_deterministic() {
        let stage = GmnTrainingCorpusStage::new();
        let root = repo_root();
        ...
        assert_eq!(
            ...
            "the GMN training corpus must be byte-deterministic"
        );
    }

    /// Every KEPT pair carries ALL FIVE verdicts (each `true`) AND a proof-obligation
    /// derivation IRI — no accepted pair is missing any verdict or its obligation.
    #[test]
    fn every_kept_pair_carries_all_five_verdicts_and_a_proof_obligation() {
        let ctx = ctx();
        let (quads, kept, _rejected) = build_corpus(&ctx);
        assert!(kept > 0, "the corpus must keep at least one pair");
        // Group by kept example subject.
        let example_type = format!("{GMEOW}GmnTrainingExample");
        let subjects: Vec<String> = quads
            .iter()
            .filter(|q| {
                q.predicate == RDF_TYPE
                    && matches!(&q.object, RdfTerm::Iri(o) if *o == example_type)
            })
            ...
            .collect();
        assert_eq!(subjects.len(), kept, "one type triple per kept pair");
        let verdicts = [
            "gmnVerdictParsed",
            "gmnVerdictRoundTrip",
            "gmnVerdictTypechecked",
        ...
        for subject in &subjects {
            for verdict in verdicts {
                let pred = format!("{GMEOW}{verdict}");
                let ok = quads.iter().any(|q| {
                    matches!(&q.subject, RdfTerm::Iri(s) if s == subject)
                        && q.predicate == pred
                        && matches!(&q.object, RdfTerm::Literal(l) if l.lexical_form == "true")
                });
                assert!(ok, "kept pair {subject} missing verdict {verdict}");
            }
            let has_obligation = quads.iter().any(|q| {
                matches!(&q.subject, RdfTerm::Iri(s) if s == subject)
                    && q.predicate == format!("{GMEOW}gmnTrainingProofDerivation")
                    && matches!(&q.object, RdfTerm::Iri(_))
            });
            assert!(
                has_obligation,
                "kept pair {subject} carries no discharged proof obligation"
            );
        }
    }

    /// Exactly one `gmnSchemaVersion` per kept pair (the version-provenance stamp is
    /// single-valued, per Task 4).
    #[test]
    fn every_kept_pair_is_version_tagged() {
        let ctx = ctx();
        let (quads, kept, _rejected) = build_corpus(&ctx);
        assert!(kept > 0);
        let example_type = format!("{GMEOW}GmnTrainingExample");
            ...
            })
        {
            let versions = quads
                ...
                .filter(|q| {
                    matches!(&q.subject, RdfTerm::Iri(s) if *s == subject)
                        && q.predicate == gmeow_lang_bridge::PRED_GMN_SCHEMA_VERSION
                })
                .count();
            assert_eq!(
                versions, 1,
                "kept pair {subject} must carry EXACTLY one gmnSchemaVersion, found {versions}"
            );
        }
    }

    /// The rejection-recording path is LIVE over the shipped corpus: the adversarial probes
    /// (blank nodes with a reserved `__` label) are DROPPED and RECORDED as
    /// `gmeow:GmnRejectedCandidate` individuals carrying a typed `gmnRejectionReason` class and a
    /// `gmnRejectionStage` — never a silent drop, and never mistakenly kept.
    #[test]
    fn adversarial_probes_are_recorded_as_typed_rejections() {
        let ctx = ctx();
        let (quads, _kept, rejected) = build_corpus(&ctx);
        assert!(
            rejected >= adversarial_probes(&ctx).len(),
            "every adversarial probe must be recorded as a rejection, found {rejected}"
        );
        let rejected_type = format!("{GMEOW}GmnRejectedCandidate");
        let rejected_subjects: Vec<String> = quads
            .iter()
            .filter(|q| {
                q.predicate == RDF_TYPE
                    && matches!(&q.object, RdfTerm::Iri(o) if *o == rejected_type)
            })
            .filter_map(|q| match &q.subject {
                RdfTerm::Iri(s) => Some(s.clone()),
                _ => None,
            ...
            .collect();
        assert_eq!(
            rejected_subjects.len(),
            rejected,
            "one type triple per rejection"
        );
        for subject in &rejected_subjects {
            let has_reason = quads.iter().any(|q| {
                matches!(&q.subject, RdfTerm::Iri(s) if s == subject)
                    && q.predicate == format!("{GMEOW}gmnRejectionReason")
                    && matches!(&q.object, RdfTerm::Iri(_))
            });
            let has_stage = quads.iter().any(|q| {
                matches!(&q.subject, RdfTerm::Iri(s) if s == subject)
                    && q.predicate == format!("{GMEOW}gmnRejectionStage")
            });
            assert!(
                has_reason,
                "rejection {subject} carries no typed gmnRejectionReason"
            );
            assert!(
                has_stage,
                "rejection {subject} carries no gmnRejectionStage"
            );
        }
    }

    /// A ring-leaking term is REJECTED at the ring-leak verifier with a typed reason, and is
    /// NOT emitted as a kept pair. Injects an atom tagged at `gmnRingRestricted` (which flows
    /// nowhere) referenced by an admitted `gmnRingCore` subject → a `lang:GmnRingLeak`.
    #[test]
    fn a_ring_leaking_term_is_rejected_with_reason() {
        let ctx = ctx();
        let atoms = base_atoms();
        let a = atoms[0].0.clone();
        let b = atoms[1].0.clone();
        let c = atoms[2].0.clone();
        let op = ctx.operators[0].term_iri.clone();
        // A chained (depth-2) term `{ a op b, b op c }`, so `b` is a CONTENT SUBJECT with its own
        // data claim. Tag a/c core (admitted) and the MIDDLE atom b restricted (excluded, flows
        // nowhere): a's admitted claim references the excluded subject b — a reference ring leak.
        let mut builder = RdfDatasetBuilder::new();
        for (s, o) in [(&a, &b), (&b, &c)] {
            builder.push_owned_quad(&RdfQuad::new(
                RdfTerm::Iri(s.clone()),
        ...
        for (atom, ring) in [(&a, RING_CORE), (&b, RING_RESTRICTED), (&c, RING_CORE)] {
            builder.push_owned_quad(&RdfQuad::new(
                RdfTerm::Iri(atom.clone()),
                PRED_CONTENT_RING.to_owned(),
                RdfTerm::Iri(ring.to_owned()),
            ));
        }
        let leaking = Gmn0Model::from_dataset(&builder.freeze().expect("freeze leaking model"));
        // The consume filter refuses admission into the trusted target — a typed ring leak.
        let err = consume_project(&leaking, &ctx.lattice, RING_TRUSTED, None, &ctx.dict)
            .expect_err("a ring-leaking term must be refused, not admitted");
        assert_eq!(
            err.failure_class(),
            gmeow_lang_bridge::CLASS_RING_LEAK,
            "the rejection reason is the shipped lang:GmnRingLeak class: {err}"
        );

        // The SAME chained term, tagged uniformly core through the stage's own verifier-5 helper,
        // IS admitted — the leak rejection is falsifiable, not a vacuous refusal. It is therefore
        // never emitted as a kept pair only in the leaking tagging.
        let clean = Candidate {
            triples: vec![
                (iri(&a), op.clone(), iri(&b)),
                (iri(&b), op.clone(), iri(&c)),
            ],
        };
        assert!(
            ring_admits(&clean.model(), &ctx, RING_CORE, RING_TRUSTED).is_ok(),
            "the clean (all-core) tagging of the same term is admitted"
        );
    }
}

...
use gmeow_rdf_core::{SparqlEngine, SparqlRequest, SparqlResult, TermValue};
...
pub fn load(manifest_path: &Path) -> Result<Vec<SparqlTestCase>, String> {
    ...
    let dataset = gmeow_rdf::parse_dataset(&bytes, "text/turtle", Some(BASE))
...
fn query_rows(
    dataset: &std::sync::Arc<gmeow_rdf_core::RdfDataset>,
    query: &str,

...
- Avoid for the activity that did the deriving (use [`gmeow:wasGeneratedBy`](gmeow-wasGeneratedBy.md) → [`gmeow:Activity`](../classes/gmeow-Activity.md)) and for mere topical aboutness (use [`gmeow:hasAboutness`](gmeow-hasAboutness.md)); derivation asserts genealogical descent, not subject matter.

### How to use

- Point the derived thing at its source; carry the derivation's [`gmeow:confidence`](gmeow-confidence.md) and lossiness on the statement layer (a summary is a lossy projection of its source), and attach [`gmeow:wasGeneratedBy`](gmeow-wasGeneratedBy.md) to name the generating activity and, through it, the agent.

### Examples

- ex:summary [`gmeow:wasDerivedFrom`](gmeow-wasDerivedFrom.md) ex:report.

...
pub fn project_nemo(
    program: &LogicProgram,
    loss: &mut crate::loss_ledger::LossLedger,
) -> gmeow_errors::Result<ProjectionResult> {
    ...
    for rule in &program.rules {
        ...
        if !unbound.is_empty() {
            ...
            return Err(Diag::of_kind(crate::error::Text {
                detail: format!(
...
pub fn extract_nemo_rules_section(nemo_content: &str) -> gmeow_errors::Result<String> {
    const MARKER: &str = "% === Rules ===";
    match nemo_content.find(MARKER) {
        Some(idx) => Ok(nemo_content[idx + MARKER.len()..].trim().to_owned()),
        None => Err(Diag::of_kind(crate::error::Text {
            detail: "Nemo projection is missing the \"% === Rules ===\" marker; cannot extract \

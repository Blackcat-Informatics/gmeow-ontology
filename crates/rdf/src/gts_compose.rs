...
//!
//! This is the byte-emitting heart of `src/gmeow_tools/gts_producer.py::_Builder`,
//! lifted out of the `python`-gated [`crate::py_gts`] surface so the non-python
//! Rust consumers (`gmeow-pipeline`) can author a full multi-named-graph `dist`
...
//! canonicalization, frame-id chaining, and signing is delegated to `gmeow-gts`.
//!
//! The `python` wrapper ([`crate::py_gts`]) delegates to THIS core; there is one

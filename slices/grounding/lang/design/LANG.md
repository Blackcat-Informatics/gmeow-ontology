...
| [`LANG-PROJECTIONS.md`](LANG-PROJECTIONS.md) | projection contract | the generated lossy lowerings — OntoLex-Lemon, WordNet/ILI, Universal Dependencies/CoNLL-U, ISO 24617 SemAF, AMR, NIF/Web Annotation, TEI, BCP-47/ISO 639/ISO 15924, EBNF/ABNF grammar surfaces — each carrying a preservation judgment in the loss ledger |
...
| [`LANG-REFERENCES.md`](LANG-REFERENCES.md) | references appendix | the classified survey of external standards, ontologies, formalisms, and engines — each tagged subsume/project/link/reference × license × kind; the primary anchors (OntoLex-Lemon, Universal Dependencies, Unicode/CLDR, Wikidata lexemes); and the original surface GMEOW authors where no external ontology exists; staged for the `metadata/references.ttl` ledger |
...
The layers **interlock at declared bridges, not by merger**, and every bridge runs one way.
...
- **`lang:` → `logic:`.** A form *denotes* into a `logic:` term, formula, type, or proof object
  ...
  annotations that `lang:` objects may later analyze, so the slice DAG stays acyclic.
- **`lang:` does not depend on `math:`.** Formal-language-theoretic facts *about* `lang:` objects —
  the language generated by a grammar as a set of strings, automaton equivalence, information
  measures over codes — are `math:` objects that reference `lang:` individuals, in the same way
  `logic:` keeps its quantitative facets abstract and lets `math:` objects satisfy them. The
  resulting grounding order is `logic:` < `lang:` < `math:` in the dependency DAG, and the `math:`
  slice's rendering seam (`math:ExpressionRendering`, `math:rendersAs`) is expected to graft onto
  the general `lang:Rendering` theory by adding `lang` to the mathematics slice's dependency set — a
  queued migration recorded in [`LANG-TRANSLATION.md`](LANG-TRANSLATION.md), never a fork.
...
<https://blackcatinformatics.ca/gmeow/slices/lang>
    a gmeow:Slice ;
    rdfs:label "lang"@x-gmeow-english ;
    dcterms:title "GMEOW Language Grounding Module"@x-gmeow-english ;
    gmeow:sliceTier gmeow:tierCore ;
    gmeow:sliceProfile "claims" , "lang" ;   # "claims" is an existing profile; "lang" is a new profile individual this slice introduces
...
Two honesty notes on the manifest, in the same discipline the mathematics manifesto set. First,
`"lang"` is a **new** profile individual this slice would introduce, not an existing one. Second,
`gmeow:sliceDependsOn` is gate-verified against the *computed* cross-slice reference graph, so the
final dependency set is trimmed to what `module.ttl`/`shapes.ttl` actually use — expected to be
this set or a subset. The deliberate **absences** are as load-bearing as the presences: no
`mathematics` (the one-way razor above), and no `language`/`notation`/`names`/`coreference` (those
slices migrate to depend on `lang:`, never the reverse).

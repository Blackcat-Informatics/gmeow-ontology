...
use gmeow_rdf_core::SparqlResult;
...
mod tests {
    ...
    use gmeow_rdf_core::{BlankScope, RdfDatasetBuilder, RdfQuad, RdfTerm, TermValue};
    ...
    fn populated_provenance_drops() {
        ...
        assert!(!text.contains("gmeow"), "TSV must stay pure W3C: {text}");
        assert_eq!(text, "?s\n<http://example.org/s>\n");
    }
}
